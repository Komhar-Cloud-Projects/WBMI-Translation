...
EXP_LKP_Values_Claim_Party_Occurrence AS (
	...
	IFF(v_claimant_num IS NULL AND In_CCT_CLIENT_ROLE_CD = 'CNOT LMT', v_claimant_num, 'N/A') AS out_Claimant_num,
...
EXP_SQ AS (
	...
	UPPER(UDF_DEFAULT_VALUE_FOR_STRINGS(PREFERRED_CONTACT_METHOD)) AS o_PREFERRED_CONTACT_METHOD
...
EXP_Derive_PreferredContactMethod_Description AS (
	...
	UDF_DEFAULT_VALUE_FOR_STRINGS(lkp_PreferredContactMethodDescription) AS o_PreferredContactMethodDescription,
	-- *INF*: IIF((NOT ISNULL(claim_party_occurrence_id)) AND 
	-- preferred_contact_method_target <> lkp_PreferredContactMethodDescription, 
	-- 'UPDATE', 
	-- 'NOACTION')
	IFF(
	    (claim_party_occurrence_id IS NOT NULL)
