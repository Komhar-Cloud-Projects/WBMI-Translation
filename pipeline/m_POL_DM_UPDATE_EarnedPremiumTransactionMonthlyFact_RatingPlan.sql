...
SQ_EarnedPremiumTransactionMonthlyFact AS (
	...
	EPT.EarnedPremiumTransactionMonthlyFactID,
	...
	inner Join @{pipeline().parameters.SOURCE_TABLE_OWNER}.EarnedPremiumTransactionMonthlyFact EPT
	on P.pol_dim_id=EPT.PolicyDimID
	inner join @{pipeline().parameters.SOURCE_TABLE_OWNER}.InsuranceReferenceDim IRD
	on EPT.InsuranceReferenceDimId=IRD.InsuranceReferenceDimId
	where P.edw_pol_ak_id in (SELECT
		P.edw_pol_ak_id
	FROM policy_dim P
	INNER JOIN EarnedPremiumTransactionMonthlyFact PTCTF
	...
	@{pipeline().parameters.WHERE_CLAUSE_EPTMF}
	order by 1
),
EXP_EarnedPremiumTransactionMonthlyFact_DataCollect AS (
	SELECT
	EDW_Pol_AK_id,
...
LKP_RatingPlan_DCT AS (
	...
	FROM (
		...
		INNER JOIN @{pipeline().parameters.WC_DATA_MART_DATABASE_NAME}.DBO.EarnedPremiumTransactionMonthlyFact PTCTF
			ON P.pol_dim_id = PTCTF.PolicyDimId
...
EXP_RatingPlanDerivation AS (
	...
	ON LKP_RatingPlan_DCT.PolicyAKID = EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.EDW_Pol_AK_id
	LEFT JOIN LKP_RatingPlan_PMS
	ON LKP_RatingPlan_PMS.PolicyAKID = EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.EDW_Pol_AK_id
),
EXP_EarnedPremTrans AS (
	SELECT
	EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.EarnedPremiumTransactionMonthlyFactID,
	EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.EnterpriseGroupCode,
	EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.InsuranceReferenceLegalEntityCode,
	EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.StrategicProfitCenterCode,
	EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.InsuranceSegmentCode,
	EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.PolicyOfferingCode,
	EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.ProductCode,
	EXP_EarnedPremiumTransactionMonthlyFact_DataCollect.InsuranceReferenceLineOfBusinessCode,
	...
	FROM EXP_EarnedPremiumTransactionMonthlyFact_DataCollect
...
EarnedPremiumTransactionMonthlyFact AS (
	MERGE INTO EarnedPremiumTransactionMonthlyFact AS T
	USING UPD_EPTMF AS S
	ON T.EarnedPremiumTransactionMonthlyFactID = S.EarnedPremiumTransactionMonthlyFactID
