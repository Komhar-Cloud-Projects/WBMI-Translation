...
EXP_Default_Value AS (
	...
	UDF_DEFAULT_VALUE_TO_BLANKS(trans_ctgry_code) AS trans_ctgry_code_out,
	EXP_Source.trans_kind_code,
	EXP_Source.type_disability,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(type_disability)
	UDF_DEFAULT_VALUE_TO_BLANKS(type_disability) AS type_disability_Out,
	EXP_Source.pol_sym,
	EXP_Source.pol_num,
	EXP_Source.pol_mod,
	EXP_Source.mco,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(mco)
	UDF_DEFAULT_VALUE_TO_BLANKS(mco) AS mco_Out,
	EXP_Source.producer_code,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(producer_code)
	UDF_DEFAULT_VALUE_TO_BLANKS(producer_code) AS producer_code_Out,
	EXP_Source.pol_co_num,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(pol_co_num)
	UDF_DEFAULT_VALUE_TO_BLANKS(pol_co_num) AS pol_co_num_Out,
	...
	UDF_DEFAULT_VALUE_TO_BLANKS(pms_pol_lob_code) AS pms_pol_lob_code_Out,
	EXP_Source.pol_co_line_code,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(pol_co_line_code)
	UDF_DEFAULT_VALUE_TO_BLANKS(pol_co_line_code) AS pol_co_line_code_Out,
	EXP_Source.ClassOfBusinessCode,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(ClassOfBusinessCode)
	UDF_DEFAULT_VALUE_TO_BLANKS(ClassOfBusinessCode) AS ClassOfBusinessCode_Out,
	...
	UDF_DEFAULT_VALUE_TO_BLANKS(ins_line) AS ins_line_Out,
	EXP_Source.major_peril_code,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(major_peril_code)
	UDF_DEFAULT_VALUE_TO_BLANKS(major_peril_code) AS major_peril_code_Out,
	EXP_Source.major_peril_seq_num,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(major_peril_seq_num)
	UDF_DEFAULT_VALUE_TO_BLANKS(major_peril_seq_num) AS major_peril_seq_num_out,
	EXP_Source.type_bureau_code,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(type_bureau_code)
	UDF_DEFAULT_VALUE_TO_BLANKS(type_bureau_code) AS type_bureau_code_Out,
	EXP_Source.risk_unit_grp,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(risk_unit_grp)
	UDF_DEFAULT_VALUE_TO_BLANKS(risk_unit_grp) AS risk_unit_grp_Out,
	...
	IFF(
	    risk_unit_grp_seq_num = 'N/A', '0',
	    SUBSTR(UDF_DEFAULT_VALUE_TO_BLANKS(risk_unit_grp_seq_num), 3, 1)
	) AS risk_unit_grp_seq_num_pos_3,
	-- *INF*: IIF(risk_unit_grp_seq_num='N/A','0',
	-- SUBSTR(:UDF.DEFAULT_VALUE_TO_BLANKS(risk_unit_grp_seq_num),2,1))
	IFF(
	    risk_unit_grp_seq_num = 'N/A', '0',
	    SUBSTR(UDF_DEFAULT_VALUE_TO_BLANKS(risk_unit_grp_seq_num), 2, 1)
	) AS risk_unit_grp_seq_num_pos_2,
	EXP_Source.risk_unit,
	-- *INF*: SUBSTR(:UDF.DEFAULT_VALUE_TO_BLANKS(risk_unit),1,3)
	SUBSTR(UDF_DEFAULT_VALUE_TO_BLANKS(risk_unit), 1, 3) AS risk_unit_Out,
	...
	UDF_DEFAULT_VALUE_TO_BLANKS(claimant_full_name) AS claimant_full_name_Out,
	EXP_Source.claimant_num,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(claimant_num)
	UDF_DEFAULT_VALUE_TO_BLANKS(claimant_num) AS claimant_num_Out,
	...
	UDF_DEFAULT_VALUE_TO_BLANKS(body_part_code) || UDF_DEFAULT_VALUE_TO_BLANKS(nature_inj_code) || UDF_DEFAULT_VALUE_TO_BLANKS(cause_inj_code) AS accident_injury_anal_code,
	EXP_Source.cust_num,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(cust_num)
	UDF_DEFAULT_VALUE_TO_BLANKS(cust_num) AS cust_num_Out,
	EXP_Source.name,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(name)
	UDF_DEFAULT_VALUE_TO_BLANKS(name) AS name_Out,
	EXP_Source.sort_name,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(sort_name)
	UDF_DEFAULT_VALUE_TO_BLANKS(sort_name) AS sort_name_Out,
	EXP_Source.suit_status_code,
	-- *INF*: :UDF.DEFAULT_VALUE_TO_BLANKS(suit_status_code)
	UDF_DEFAULT_VALUE_TO_BLANKS(suit_status_code) AS suit_status_code_Out,
	...
	ON LKP_SUP_STATE__UDF_DEFAULT_VALUE_TO_BLANKS_loss_loc_state.state_code = UDF_DEFAULT_VALUE_TO_BLANKS(loss_loc_state)
...
EDW_Loss_Master_txtFile AS (
	INSERT INTO EDW_Loss_Master_txtFile
	(LM_TRANSACTION_CODE, LM_PIF_LOCATION, LM_MASTER_COMPANY_NUMBER, LM_POLICY_COMPANY_NUMBER, LM_POLICY_SYMBOL, LM_POLICY_NUMBER, LM_CERTIFICATE_NUMBER, LM_POLICY_ANNIV_EFF_DATE, LM_POLICY_ANNIV_EXP_DATE, LM_ACCOUNT_NUMBER, LM_ACCOUNT_ENTERED_DATE, LM_LOSS_ID_OR_MAJ_PER_SEQ, LM_MAJ_PER_OR_UNIT_NO, Buffer1, LM_AGENCY_NUMBER, LM_AGENCY_STATE_PROVINCE, LM_AGENCY_DEPT_CODE, LM_STAT_BREAKDOWN_LINE, LM_COMPANY_LINE, LM_ANNUAL_STATEMENT_LINE, LM_EOM_IBNR_IND, LM_CATASTROPHE_NUMBER, LM_LOSS_CAUSE, LM_NEW_CLAIM_COUNT, LM_LOSS_OCCURRENCE_NUMBER, LM_DATE_REPORTED, LM_DATE_OF_LOSS, LM_EOM_REINS_ID, LM_EXPENSE_RESV_IND, LM_BRANCH_NUMBER, LM_ADJUSTOR_BRANCH_NUMBER, LM_ADJUSTING_COMPANY_NUMBER, LM_KIND_CODE, LM_TYPE_REINSURANCE, LM_REINSURANCE_COMPANY_NUMBER, LM_CEDING_COMPANY_NUMBER, LM_REINSURANCE_CESSION_NUMBER, LM_REINSURANCE_RATIO, LM_TREATY_NUMBER, LM_BANK_NUMBER, LM_DRAFT_NUMBER, LM_AMOUNT_OUTSTANDING, LM_AMOUNT_PAID_LOSSES, LM_AMOUNT_PAID_EXPENSES, LM_EOM_UNPAID_LOSS_ADJ_EXP, LM_AUTO_AND_GEN_LIAB_COV_CODE, LM_COMM_REPORT_FREQ, LM_ACCIDENT_LOCATION, LM_ACCIDENT_STATE_PROVINCE, LM_RISK_STATE_PROVINCE, LM_REGION_CODE, Buffer2, LM_TAX_LOCATION, LM_RISK_ZIP_CODE_POSTAL_ZONE, LM_TYPE_BUREAU, LM_MAJOR_PERIL, LM_CLASS, LM_EXPOSURE, LM_CSP_SUB_LINE, LM_STATISTICAL_CODES, LM_STATISTICAL_FILLER, LM_CSP_TYPE_OF_LOSS_CODE, LM_CSP_INCEPTION_DATE, LM_LINE_OF_BUSINESS, LM_CLAIMS_OFFICE, LM_PRODUCT_LINE, LM_STATISTICAL_LINE, LM_CLAIM_CONVERSION_NUMBER, LM_CLAIMANT_NUMBER, LM_CLAIMANT_NAME, LM_INSUREDS_NAME, LM_AVERAGE_RESERVE_CODE, LM_ORIGINAL_RESERVE, LM_EOM_RESERVE_ESTABLISH_DATE, LM_WEEKLY_RATE, LM_PAYMENT_FREQUENCY, LM_PERIOD_PAY, LM_PAY_CYCLE, LM_EMPLOYEE_AGE, LM_TIME_EMPLOYED_YEARS, LM_TIME_EMPLOYED_MONTHS, LM_TYPE_DISABILITY, LM_ACCIDENT_INJURY_ANAL_CODE, LM_CUSTOMER_NUMBER, Buffer3, LM_PRODUCER_CODE, LM_AUTO_REINSURANCE_FACILITY, LM_GROUP_NUMBER, LM_VARIATION_CODE, LM_SORT_NAME, LM_1099_FIELD, LM_EXAMINER_CODE, LM_ACCIDENT_DESCRIPTION, LM_CLAIMS_SEQ, Buffer4, LM_CLAIMS_ZPCD_INJ_LOC, LM_ATTORNEY_CODE, LM_CLAIMS_SPECIAL_USE_2, LM_SPECIAL_USE_SAR, Buffer5, LM_CLASS_OF_BUSINESS, Buffer6, LM_POLICY_TYPE, LM_SPECIAL_USE_PIF_B, LM_SUIT_INDICATION, LM_DRAFT_MONTH, LM_DRAFT_DAY, LM_DRAFT_YEAR, LM_COST_CENTER, LM_COST_CONTAINMENT, Buffer7, LM_VARIABLE_STAT_AREA2, LM_VARIABLE_STAT_AREA3, LM_FUTURE_STAT_AREA, LM_RISK_STREET_ADDRESS, LM_RISK_CITY_STATE_ADDRESS, LM_MODULE_NUMBER, LM_ZONE_TERRITORY, LM_CLAIMS_MADE_IND, LM_CM_RETRO_DATE, LM_SAR_INSURANCE_LINE, LM_SAR_LOCATION_NUMBER, LM_SAR_SUB_LOCATION_NUMBER, LM_SAR_RISK_UNIT_GROUP, LM_SAR_CLASS_CODE_GROUP, LM_SAR_CLASS_CODE_MEMBER, Buffer8, LM_SAR_SEQUENCE_RISK_UNIT_N, LM_SAR_SEQUENCE_RISK_UNIT_A, LM_SAR_TYPE_EXPOSURE, LM_SAR_ADDT_INT_TYP, LM_SAR_SEQ_ADDIT_INT_TYP, LM_SAR_PEAK_SEASON_INDICATOR, LM_EXCESS_CATASTROPHE, LM_ORIGINAL_RESERVE_PART_1, LM_RESERVE_CATEGORY, LM_TRANSACTION_CATEGORY, LM_OTHER_ASSIGNMENT, LM_NOTICE_OF_OCCURRENCE, Buffer9, LM_PASP_DATE_LIC_MM, LM_PASP_DATE_LIC_YY, Buffer10, LM_PASP_VC_2POS, BUffer11)
	SELECT 
	...
	Default_Spaces AS LM_EOM_IBNR_IND, 
	...
	Default_Spaces AS LM_EOM_REINS_ID, 
	Default_Spaces AS LM_EXPENSE_RESV_IND, 
	...
	Default_Spaces AS LM_COMM_REPORT_FREQ, 
	Default_Spaces AS LM_ACCIDENT_LOCATION, 
	...
	Default_Spaces AS LM_STATISTICAL_FILLER, 
	Default_Spaces AS LM_CSP_TYPE_OF_LOSS_CODE, 
	...
	Default_Spaces AS LM_EOM_RESERVE_ESTABLISH_DATE, 
	Default_Spaces AS LM_WEEKLY_RATE, 
	Default_Spaces AS LM_PAYMENT_FREQUENCY, 
	Default_Spaces AS LM_PERIOD_PAY, 
	Default_Spaces AS LM_PAY_CYCLE, 
	Default_Spaces AS LM_EMPLOYEE_AGE, 
	Default_Spaces AS LM_TIME_EMPLOYED_YEARS, 
	Default_Spaces AS LM_TIME_EMPLOYED_MONTHS, 
	...
	Default_Spaces AS LM_CLAIMS_MADE_IND, 
	Default_Spaces AS LM_CM_RETRO_DATE, 
	...
	Default_Spaces AS BUFFER8, 
	risk_unit_seq_num_out AS LM_SAR_SEQUENCE_RISK_UNIT_N, 
	Default_Spaces AS LM_SAR_SEQUENCE_RISK_UNIT_A, 
	Default_Spaces AS LM_SAR_TYPE_EXPOSURE, 
	Default_Spaces AS LM_SAR_ADDT_INT_TYP, 
	Default_0 AS LM_SAR_SEQ_ADDIT_INT_TYP, 
	Default_Spaces AS LM_SAR_PEAK_SEASON_INDICATOR, 
	Default_Spaces AS LM_EXCESS_CATASTROPHE, 
	trans_ctgry_code_out AS LM_ORIGINAL_RESERVE_PART_1, 
	reserve_ctgry_Out AS LM_RESERVE_CATEGORY, 
	trans_ctgry_code_out AS LM_TRANSACTION_CATEGORY, 
	Default_Spaces AS LM_OTHER_ASSIGNMENT, 
	Default_Spaces AS LM_NOTICE_OF_OCCURRENCE, 
	Default_Spaces AS BUFFER9, 
	Default_Spaces AS LM_PASP_DATE_LIC_MM, 
	Default_Spaces AS LM_PASP_DATE_LIC_YY, 
	Default_Spaces AS BUFFER10, 
	Default_Spaces AS LM_PASP_VC_2POS, 
	Default_Spaces AS BUFFER11
