...
EXP_Values AS (
	...
	IFF(
	    i_PhoneNumber IS NULL
	    or LENGTH(i_PhoneNumber)>0
	    and TRIM(i_PhoneNumber)=''
	    or LENGTH(i_PhoneNumber) = 0,
	    'N/A',
	    REGEXP_REPLACE(v_ph_num_full_trim, '\D', '')
	) AS v_ph_num_full,
	-- *INF*: IIF(ISNULL(i_FirstName) or IS_SPACES(i_FirstName)  or LENGTH(i_FirstName)=0,'N/A',LTRIM(RTRIM(i_FirstName)))
	IFF(
	    i_FirstName IS NULL
	    or LENGTH(i_FirstName)>0
	    and TRIM(i_FirstName)=''
	    or LENGTH(i_FirstName) = 0,
	    'N/A',
	...
	IFF(
	    i_LastName IS NULL or LENGTH(i_LastName)>0 AND TRIM(i_LastName)='' or LENGTH(i_LastName) = 0,
	    'N/A',
	...
	IFF(
	    i_MiddleName IS NULL
	    or LENGTH(i_MiddleName)>0
	    and TRIM(i_MiddleName)=''
	    or LENGTH(i_MiddleName) = 0,
	    'N/A',
	...
	IFF(
	    i_Name IS NULL or LENGTH(i_Name)>0 AND TRIM(i_Name)='' or LENGTH(i_Name) = 0, 'N/A',
	    LTRIM(RTRIM(i_Name))
	) AS o_name,
	...
	IFF(
	    i_FederalEmployeeIDNumber IS NULL
	    or LENGTH(i_FederalEmployeeIDNumber)>0
	    and TRIM(i_FederalEmployeeIDNumber)=''
	    or LENGTH(i_FederalEmployeeIDNumber) = 0,
	    'N/A',
	    LTRIM(RTRIM(i_FederalEmployeeIDNumber))
	) AS o_fed_tax_id,
	...
	IFF(
	    i_DoingBusinessAs IS NULL
	    or LENGTH(i_DoingBusinessAs)>0
	    and TRIM(i_DoingBusinessAs)=''
	    or LENGTH(i_DoingBusinessAs) = 0,
	    'N/A',
	...
	IFF(
	    i_SICCode IS NULL or LENGTH(i_SICCode)>0 AND TRIM(i_SICCode)='' or LENGTH(i_SICCode) = 0,
	    'N/A',
	...
	IFF(
	    i_NAICSCode IS NULL
	    or LENGTH(i_NAICSCode)>0
	    and TRIM(i_NAICSCode)=''
	    or LENGTH(i_NAICSCode) = 0,
	    'N/A',
	...
	IFF(
	    i_EntityType IS NULL
	    or LENGTH(i_EntityType)>0
	    and TRIM(i_EntityType)=''
	    or LENGTH(i_EntityType) = 0,
	    'N/A',
	...
	IFF(
	    v_ph_num_full = 'N/A' or LENGTH(v_ph_num_full) != 10, 'N/A',
	    v_ph_num_full_first || SUBSTR(v_ph_num_full, 1, 3)
	) AS o_ph_area_code,
	-- *INF*: IIF(v_ph_num_full='N/A' or LENGTH(v_ph_num_full)  !=  10,'N/A',SUBSTR(v_ph_num_full,4,3))
	IFF(
	    v_ph_num_full = 'N/A' or LENGTH(v_ph_num_full) != 10, 'N/A', SUBSTR(v_ph_num_full, 4, 3)
	) AS o_ph_exchange,
	-- *INF*: IIF(v_ph_num_full='N/A' or LENGTH(v_ph_num_full) != 10,'N/A',SUBSTR(v_ph_num_full,7,4))
	IFF(
	    v_ph_num_full = 'N/A' or LENGTH(v_ph_num_full) != 10, 'N/A', SUBSTR(v_ph_num_full, 7, 4)
	) AS o_ph_num,
	...
	IFF(
	    i_PhoneExtension IS NULL
	    or LENGTH(i_PhoneExtension)>0
	    and TRIM(i_PhoneExtension)=''
	    or LENGTH(i_PhoneExtension) = 0,
	    'N/A',
	    LTRIM(RTRIM(i_PhoneExtension))
	) AS o_ph_extension,
	-- *INF*: IIF(ISNULL(i_Email) or IS_SPACES(i_Email)  or LENGTH(i_Email)=0,'N/A',LTRIM(RTRIM(i_Email)))
	IFF(
	    i_Email IS NULL or LENGTH(i_Email)>0 AND TRIM(i_Email)='' or LENGTH(i_Email) = 0, 'N/A',
	    LTRIM(RTRIM(i_Email))
	) AS o_bus_email_addr,
...
EXP_Detect_Changes AS (
	...
	IFF(
	    ...
	    IFF(
	        ...
	        or LTRIM(RTRIM(lkp_sort_name)) != sort_name
