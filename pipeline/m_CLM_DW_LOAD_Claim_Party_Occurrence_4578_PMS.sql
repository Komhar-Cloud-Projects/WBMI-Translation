...
EXP_LKP_Values AS (
	...
	-- )
	IFF(
	    LKP_CLAIM_CASE_AK_ID_v_claim_case_key.claim_case_ak_id IS NULL, - 1,
	    LKP_CLAIM_CASE_AK_ID_v_claim_case_key.claim_case_ak_id
	) AS claim_case_ak_id,
	-- *INF*: TO_DATE('1/1/1800','MM/DD/YYYY')
	TO_TIMESTAMP('1/1/1800', 'MM/DD/YYYY') AS denial_date
	FROM EXP_Source
...
EXP_Detect_Changes AS (
	...
	IFF(
	    ...
	    IFF(
	        (lkp_denial_date <> DENIAL_DATE), 'UPDATE', 'NOCHANGE'
	    )
	) AS v_Changed_Flag,
...
EXP_Lag_eff_from_date AS (
	...
	DECODE(
	    TRUE,
	    claim_occurrence_ak_id = v_PREV_ROW_claim_occurrence_ak_id AND claim_party_ak_id = v_PREV_ROW_claim_party_ak_id AND claim_party_role_code = v_PREV_ROW_claim_party_role_code AND claim_case_ak_id = v_PREV_ROW_claim_case_ak_id, DATEADD(SECOND,- 1,v_PREV_ROW_eff_from_date),
	    orig_eff_to_date
	) AS v_eff_to_date,
