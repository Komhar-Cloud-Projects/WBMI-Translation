...
AGG_Remove_Duplicates AS (
	...
	IFF(
	    in_AgencyCode IS NULL
	    or LENGTH(in_AgencyCode)>0
	    and TRIM(in_AgencyCode)=''
	    or LENGTH(in_AgencyCode) = 0,
	    'N/A',
	...
	-- )
	IFF(
	    in_CustomerNumber IS NULL
	    or LENGTH(in_CustomerNumber)>0
	    and TRIM(in_CustomerNumber)=''
	    or LENGTH(in_CustomerNumber) = 0,
	    'N/A',
	    LTRIM(RTRIM(in_CustomerNumber))
	...
	-- )
	IFF(
	    in_PolicyNumber IS NULL
	    or LENGTH(in_PolicyNumber)>0
	    and TRIM(in_PolicyNumber)=''
	    or LENGTH(in_PolicyNumber) = 0,
	    'N/A',
	    LTRIM(RTRIM(in_PolicyNumber))
	) AS out_PolicyNumber,
	...
	IFF(RTRIM(WBProduct) = 'Commercial Package', 'N/A', DCLineType) AS out_SourceLineType,
	-- *INF*: IIF(
	...
	-- )
	IFF(
	    WBProduct IS NULL OR LENGTH(WBProduct)>0 AND TRIM(WBProduct)='' OR LENGTH(WBProduct) = 0,
	    'N/A',
	    LTRIM(RTRIM(WBProduct))
	) AS out_WBProduct,
	-- *INF*: IIF(
	...
	-- )
	IFF(
	    WBProductType IS NULL
	    or LENGTH(WBProductType)>0
	    and TRIM(WBProductType)=''
	    or LENGTH(WBProductType) = 0,
	    'N/A',
	    LTRIM(RTRIM(WBProductType))
...
EXP_ConvertPolicyOffering AS (
	...
	IFF(PolicyOfferingCode IS NULL, '000', PolicyOfferingCode) AS o_PolicyOfferingCode
...
EXP_values AS (
	...
	IFF(
	    Type IS NULL OR LENGTH(Type)>0 AND TRIM(Type)='' OR LENGTH(Type) = 0, 'N/A',
	    LTRIM(RTRIM(Type))
	) AS v_Type,
	...
	IFF(
	    in_CancellationDate IS NULL, TO_TIMESTAMP('2100-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS'),
	    in_CancellationDate
	...
	IFF(
	    v_Type <> 'Cancel', TO_TIMESTAMP('21001231235959', 'YYYYMMDDHH24MISS'),
	...
	IFF(
	    v_seq = 1,
	    IFF(
	        NOT v_Type IN ('Reinstate','Cancel')
	        and lkp_pol_ak_id IS NULL
	        and to_char(lkp_pol_cancellation_date, 'YYYYMM') <> '210NOT 012',
	        lkp_pol_cancellation_date,
	        v_pol_cancellation_date
	    ),
	    IFF(
	        v_seq <> 1
	    and v_Type IN ('Reinstate','Cancel'), v_pol_cancellation_date,
	        v_pol_cancellation_date_persist
	    )
	) AS v_pol_cancellation_date_persist,
	-- *INF*: IIF(v_pol_cancellation_date_persist<TO_DATE('21001231','YYYYMMDD'),  'Y','N' )
	-- --prod 11524 
	IFF(v_pol_cancellation_date_persist < TO_TIMESTAMP('21001231', 'YYYYMMDD'), 'Y', 'N') AS v_pol_cancellation_ind,
	...
	IFF(
	    in_PreviousPolicyNumber IS NULL
	    or LENGTH(in_PreviousPolicyNumber)>0
	    and TRIM(in_PreviousPolicyNumber)=''
	    or LENGTH(in_PreviousPolicyNumber) = 0,
	    'N/A',
	...
	IFF(
	    in_EffectiveDate IS NULL, TO_TIMESTAMP('2100-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS'),
	    in_EffectiveDate
	) AS v_EffectiveDate,
	-- *INF*: IIF(ISNULL(in_ExpirationDate),TO_DATE('2100-12-31 23:59:59 ','YYYY-MM-DD HH24:MI:SS') , in_ExpirationDate)
	IFF(
	    in_ExpirationDate IS NULL, TO_TIMESTAMP('2100-12-31 23:59:59 ', 'YYYY-MM-DD HH24:MI:SS'),
	    in_ExpirationDate
	) AS v_ExpirationDate,
	-- *INF*: IIF(ISNULL(in_InceptionDate),TO_DATE('2100-12-31 23:59:59 ','YYYY-MM-DD HH24:MI:SS') , in_InceptionDate)
	IFF(
	    in_InceptionDate IS NULL, TO_TIMESTAMP('2100-12-31 23:59:59 ', 'YYYY-MM-DD HH24:MI:SS'),
	    in_InceptionDate
	) AS v_InceptionDate,
	...
	-- 
	IFF(
	    v_EffectiveDate <= in_CreatedDate
	    and in_CreatedDate < (
	        IFF(
	            v_ExpirationDate < v_pol_cancellation_date_persist, v_ExpirationDate,
	            v_pol_cancellation_date_persist
	        )),
	    'I',
	    IFF(
	        v_pol_cancellation_date_persist <= in_CreatedDate
	...
	DECODE(
	    TRUE,
	    v_seq = 1 and v_lkp_pol_issue_code IS NOT NULL, v_lkp_pol_issue_code,
	    v_seq <> 1, v_pol_issue_code,
	    Type IN ('New') and in_PreviousPolicyNumber IS NULL, 'N',
	...
	-- )
	IFF(
	    in_PolicyIssueCodeDesc IS NULL
	    or LENGTH(in_PolicyIssueCodeDesc)>0
	    and TRIM(in_PolicyIssueCodeDesc)=''
	    or LTRIM(RTRIM(in_PolicyIssueCodeDesc)) = '',
	    v_lkp_pol_issue_code,
	    IFF(
	        LTRIM(RTRIM(in_PolicyIssueCodeDesc)) = 'New', 'N', 'R'
	    )
	...
	-- --Note: This logic is obsolete after 5/1/2018. Get the Policy Issue code from v_pol_issue_code_new port
	IFF(
	    TO_TIMESTAMP(TO_CHAR(v_EffectiveDate, 'YYYY-MM-DD'), 'YYYY-MM-DD') < TO_TIMESTAMP('2018-05-01', 'YYYY-MM-DD')
	...
	DECODE(
	    ...
	    in_Status IN ('InForce','Quote','Application','Bound','Cancel-Pending') and v_CurrentDate >= v_EffectiveDate and v_CurrentDate < v_ExpirationDate, '1',
	...
	IFF(
	    in_AuditPeriod IS NULL
	    or LENGTH(in_AuditPeriod)>0
	    and TRIM(in_AuditPeriod)=''
	    or LENGTH(in_AuditPeriod) = 0,
	    'N/A',
	    in_AuditPeriod
	) AS v_AuditPeriod,
	...
	IFF(
	    in_County IS NULL OR LENGTH(in_County)>0 AND TRIM(in_County)='' OR LENGTH(in_County) = 0,
	    'N/A',
	    in_County
	) AS v_County,
	-- *INF*: IIF(
	...
	--   'CommercialLines' 
	-- )
	IFF(LTRIM(RTRIM(in_Division)) = 'WCPool', 'PoolService', 'CommercialLines') AS v_InsuranceSegmentCode,
	...
	IFF(
	    in_BCCCode IS NULL OR LENGTH(in_BCCCode)>0 AND TRIM(in_BCCCode)='' OR LENGTH(in_BCCCode) = 0,
	    'N/A',
	    in_BCCCode
	...
	Decode(
	    True,
	    ltrim(rtrim(CustomerCare)) IN ('0'), 'N',
	    ltrim(rtrim(CustomerCare)) IN ('1'), 'Y',
	    'N/A'
	...
	DECODE(
	    TRUE,
	    in_Terrorism IS NULL OR LENGTH(in_Terrorism)>0 AND TRIM(in_Terrorism)='' OR LENGTH(in_Terrorism) = 0, 'N/A',
	    in_Terrorism = '1', 'Y',
	    in_Terrorism = '0', 'N',
	    'N/A'
	...
	IFF(
	    in_RiskGrade IS NULL
	    or LENGTH(in_RiskGrade)>0
	    and TRIM(in_RiskGrade)=''
	    or LENGTH(in_RiskGrade) = 0,
	    'N/A',
	    in_RiskGrade
	...
	TO_TIMESTAMP('2100-12-31 23:59:59 ', 'YYYY-MM-DD HH24:MI:SS') AS out_nonrenewal_flag_date,
	-- *INF*: TO_DATE('2100-12-31 23:59:59 ','YYYY-MM-DD HH24:MI:SS')
	TO_TIMESTAMP('2100-12-31 23:59:59 ', 'YYYY-MM-DD HH24:MI:SS') AS out_audit_complt_date,
	-- *INF*: TO_DATE('2100-12-31 23:59:59 ','YYYY-MM-DD HH24:MI:SS')
	TO_TIMESTAMP('2100-12-31 23:59:59 ', 'YYYY-MM-DD HH24:MI:SS') AS out_orig_acct_date,
	-- *INF*: IIF(NOT ISNULL(in_TransactionDate),in_TransactionDate,TO_DATE('1800-01-01 00:00:00','YYYY-MM-DD HH24:MI:SS'))
	IFF(
	    in_TransactionDate IS NOT NULL, in_TransactionDate,
	...
	IFF(in_bus_seg_code IS NOT NULL, in_bus_seg_code, 'N/A') AS out_bus_seg_code,
	...
	IFF(
	    in_Program IS NULL OR LENGTH(in_Program)>0 AND TRIM(in_Program)='' OR LENGTH(in_Program) = 0,
	    'N/A',
	...
	-- 
	IFF(
	    in_Association IS NULL
	    or LENGTH(in_Association)>0
	    and TRIM(in_Association)=''
	    or LENGTH(in_Association) = 0
	...
	IFF(
	    in_PirorCarrierNameOther IS NULL, 'N/A',
	    SUBSTR(LTRIM(RTRIM(in_PirorCarrierNameOther)), 1, 50)
	...
	IFF(
	    in_IsRollover = 'T',
	    IFF(
	        v_PriorCarrierName = 'Other', v_PriorCarrierNameOther, v_PriorCarrierName
	    ),
	    'N/A'
	...
	IFF(in_PoolCode IS NULL, 'N/A', in_PoolCode) AS o_PoolCode,
	...
	IFF(
	    in_IssuedUWID IS NULL
	    or LENGTH(in_IssuedUWID)>0
	    and TRIM(in_IssuedUWID)=''
	    or LENGTH(in_IssuedUWID) = 0
	    or in_IssuedUWID = '0',
	    'N/A',
	    in_IssuedUWID
	...
	IFF(
	    in_IssuedUnderwriter IS NULL
	    or LENGTH(in_IssuedUnderwriter)>0
	    and TRIM(in_IssuedUnderwriter)=''
	    or LENGTH(in_IssuedUnderwriter) = 0,
	    'N/A',
...
EXP_Detect_Changes AS (
	...
	IFF(
	    v_strtgc_bus_dvsn_ak_id_1 IS NULL, LKP_STRATEGIC_BUSINESS_DIVISION__6.strtgc_bus_dvsn_ak_id,
	...
	IFF(v_strtgc_bus_dvsn_ak_id_2 IS NULL, - 1, v_strtgc_bus_dvsn_ak_id_2) AS v_strtgc_bus_dvsn_ak_id,
	-- *INF*: IIF(
	...
	IFF(in_IssuedUWID IS NULL, 'N/A', in_IssuedUWID) AS o_IssuedUWID,
	-- *INF*: IIF(ISNULL(in_IssuedUnderwriter),'N/A',in_IssuedUnderwriter)
	IFF(in_IssuedUnderwriter IS NULL, 'N/A', in_IssuedUnderwriter) AS o_IssuedUnderwriter
	FROM EXP_values
