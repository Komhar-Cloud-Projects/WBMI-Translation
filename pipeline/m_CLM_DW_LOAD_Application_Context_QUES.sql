...
EXP_VALIDATE AS (
	...
	IFF(
	    LTRIM(RTRIM(IN_app_guid)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_app_guid)))>0
	    and TRIM(LTRIM(RTRIM(IN_app_guid)))=''
	    or LENGTH(LTRIM(RTRIM(IN_app_guid))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_app_context_ent_name)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_app_context_ent_name)))>0
	    and TRIM(LTRIM(RTRIM(IN_app_context_ent_name)))=''
	    or LENGTH(LTRIM(RTRIM(IN_app_context_ent_name))) = 0,
	    'N/A',
...
EXP_DETECT_CHANGES1 AS (
	...
	-- 
	IFF(
	    LKP_app_context_ak_id IS NULL, 'NEW',
	    IFF(
	        LKP_app_ak_id <> v_app_ak_id
	        or LTRIM(RTRIM(LKP_app_context_guid)) <> LTRIM(RTRIM(app_context_guid))
	        or LTRIM(RTRIM(LKP_display_name)) <> LTRIM(RTRIM(display_name))
	        or LTRIM(RTRIM(LKP_app_context_entity_name)) <> LTRIM(RTRIM(app_context_entity_name)),
	        'UPDATE',
	        'NOCHANGE'
	    )
	) AS v_changed_flag,
