...
AGG_txn AS (
	...
	SUM(ctx_cost_cont_sav
	) AS ctx_cost_cont_sav_out
...
EXP_set_defaults_txn AS (
	...
	IFF(LTRIM(RTRIM(IN_CTB_FIN_TYPE_CD
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_CTB_FIN_TYPE_CD
				)
			)
		) = 0,
		'N/A',
	...
	IFF(IN_CTB_COST_CONT_SAV IS NULL,
		0,
		IN_CTB_COST_CONT_SAV
	) AS CTB_COST_CONT_SAV,
	0 AS AMT_default,
...
EXP_get_values_benefit AS (
	...
	IFF(IN_CTB_DRAFT_NBR IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_CTB_DRAFT_NBR
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_CTB_DRAFT_NBR
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_CTB_DRAFT_NBR
				)
	...
	-- 
	-- 
	IFF(IN_ctb_bene_litigated IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_ctb_bene_litigated
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_ctb_bene_litigated
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_ctb_bene_litigated
				)
	...
	-- 
	-- 
	IFF(IN_ctb_lump_sum_ind IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_ctb_lump_sum_ind
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_ctb_lump_sum_ind
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_ctb_lump_sum_ind
				)
	...
	-- --'N/A
	IFF(ctb_benefit_offset_cd IS NULL,
		'0',
		ctb_benefit_offset_cd
	) AS o_ctb_benefit_offset_cd,
	ctb_benefit_offset_amt,
	-- *INF*: IIF(ISNULL(ctb_benefit_offset_amt), 0.00, ctb_benefit_offset_amt)
	IFF(ctb_benefit_offset_amt IS NULL,
		0.00,
		ctb_benefit_offset_amt
	) AS o_ctb_benefit_offset_amt
...
EXP_set_defaults_benefit AS (
	...
	IFF(IN_CTB_BENEFIT_TYPE IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_CTB_BENEFIT_TYPE
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_CTB_BENEFIT_TYPE
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_CTB_BENEFIT_TYPE
				)
	...
	IFF(IN_CTB_BENEFIT_START IS NULL,
		TO_DATE('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS'
		),
	...
	IFF(IN_CTB_BENEFIT_END IS NULL,
		TO_DATE('12/31/2100 23:59:59', 'MM/DD/YYYY HH24:MI:SS'
		),
		IN_CTB_BENEFIT_END
	) AS CTB_BENEFIT_END,
	...
	-- 
	-- 
	IFF(LTRIM(RTRIM(IN_CTB_INVOICE_NBR
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_CTB_INVOICE_NBR
				)
			)
		) = 0,
		'N/A',
	...
	IFF(IN_CTB_COST_CONT_RED IS NULL,
		0,
		IN_CTB_COST_CONT_RED
	) AS CTB_COST_CONT_RED,
	...
	IFF(IN_CTB_ATY_COSTS_AMT IS NULL,
		0,
		IN_CTB_ATY_COSTS_AMT
	) AS CTB_ATY_COSTS_AMT,
	...
	-- 
	-- 
	IFF(LTRIM(RTRIM(IN_CTB_ATY_FILE_NUM
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_CTB_ATY_FILE_NUM
				)
			)
		) = 0,
		'N/A',
	...
	IFF(IN_CTB_TPD_RATE_FCTR IS NULL,
		0,
		IN_CTB_TPD_RATE_FCTR
	) AS CTB_TPD_RATE_FCTR,
	...
	IFF(IN_CTB_TPD_WAGE_LOSS IS NULL,
		0,
		IN_CTB_TPD_WAGE_LOSS
	) AS CTB_TPD_WAGE_LOSS,
	...
	IFF(IN_CTB_TPD_WKLY_WAGE IS NULL,
		0,
		IN_CTB_TPD_WKLY_WAGE
	) AS CTB_TPD_WKLY_WAGE,
	...
	-- 
	-- 
	IFF(LTRIM(RTRIM(IN_CTB_BENE_MIS_CMT
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_CTB_BENE_MIS_CMT
				)
			)
		) = 0,
		'N/A',
	...
	IFF(LTRIM(RTRIM(IN_CLAIM_PAY_CTGRY_LITIGATED_IND
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_CLAIM_PAY_CTGRY_LITIGATED_IND
				)
			)
		) = 0,
		'N/A',
	...
	IFF(LTRIM(RTRIM(cov_ctgry_code
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(cov_ctgry_code
				)
			)
		) = 0,
		'N/A',
