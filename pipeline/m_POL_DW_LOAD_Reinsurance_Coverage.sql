...
EXP_Value AS (
	...
	UDF_DEFAULT_VALUE_FOR_STRINGS(in_reins_insurance_line) AS reins_insurance_line,
	reins_location_number AS in_reins_location_number,
	-- *INF*: :UDF.DEFAULT_VALUE_FOR_STRINGS(TO_CHAR(in_reins_location_number))
	UDF_DEFAULT_VALUE_FOR_STRINGS(TO_CHAR(in_reins_location_number)) AS reins_location_number,
	...
	-- )
	IFF(
	    (reins_eff_month IS NULL OR reins_eff_day IS NULL OR reins_eff_year IS NULL),
	    TO_TIMESTAMP('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS'),
	    TO_TIMESTAMP(v_reins_eff_date, 'MM/DD/YYYY')
	) AS reins_eff_date,
	reins_exp_year,
	reins_exp_month,
	reins_exp_day,
	...
	-- )
	IFF(
	    (reins_exp_month IS NULL OR reins_exp_day IS NULL OR reins_exp_year IS NULL),
	    TO_TIMESTAMP('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS'),
	    TO_TIMESTAMP(v_reins_exp_date, 'MM/DD/YYYY')
	) AS reins_exp_date,
	reins_ent_year,
	reins_ent_month,
	reins_ent_day,
	...
	-- )
	IFF(
	    (reins_ent_month IS NULL OR reins_ent_day IS NULL OR reins_ent_year IS NULL),
	    TO_TIMESTAMP('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS'),
	    TO_TIMESTAMP(v_reins_ent_date, 'MM/DD/YYYY')
	) AS reins_ent_date,
	reins_type AS in_reins_type,
	-- *INF*: :UDF.DEFAULT_VALUE_FOR_STRINGS(in_reins_type)
	-- 
	-- --'N/A' (backed out this change for April release)
	UDF_DEFAULT_VALUE_FOR_STRINGS(in_reins_type) AS reins_type,
...
EXP_Detect_Changes AS (
	...
	IFF(
	    ...
	    IFF(
	        ltrim(rtrim(lkp_reins_co_name)) <> ltrim(rtrim(reins_co_name))
	        or (lkp_reins_exp_date <> reins_exp_date)
	        or (ltrim(rtrim(lkp_reins_type)) <> ltrim(rtrim(reins_type)))
