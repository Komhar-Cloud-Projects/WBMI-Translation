...
EXP_vendor_change AS (
	...
	IFF(in_vendor_type_code IS NULL OR IS_SPACES(in_vendor_type_code) OR LENGTH(in_vendor_type_code) = 0, 'N/A', in_vendor_type_code) AS v_vendor_type_code,
	...
	IFF(in_vendor_type_desc IS NULL OR IS_SPACES(in_vendor_type_desc) OR LENGTH(in_vendor_type_desc) = 0, 'N/A', ltrim(rtrim(in_vendor_type_desc))) AS v_vendor_type_desc,
	...
	-- 'UPDATE', 'NOCHANGE'))
	IFF(in_LKP_sup_claim_vendor_1099_type_id IS NULL, 'NEW', IFF(ltrim(rtrim(in_LKP_vendor_type_code_descript)) != ltrim(rtrim(v_vendor_type_desc)), 'UPDATE', 'NOCHANGE')) AS v_changed_flag,
...
EXP_vendor_1099_upadte AS (
	...
	-- 
	DECODE(TRUE,
	vendor_type_code = v_prev_vendor_type_code, ADD_TO_DATE(v_prev_eff_from_date, 'SS', - 1),
