...
EXP_CLAIMS_TRANSACTION_DEFAULT AS (
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_CLAIM_NUMBER)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_CLAIM_NUMBER)>0
	    and TRIM(IN_LOSS_CLAIM_NUMBER)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_loss_claim_payee_name)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_loss_claim_payee_name)>0
	    and TRIM(IN_loss_claim_payee_name)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_NOTES_DRAFT_PAYEE)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_NOTES_DRAFT_PAYEE)>0
	    and TRIM(IN_LOSS_NOTES_DRAFT_PAYEE)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_ADJUSTOR_NO)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_ADJUSTOR_NO)>0
	    and TRIM(IN_LOSS_ADJUSTOR_NO)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_REINS_KEY_ID)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_REINS_KEY_ID)>0
	    and TRIM(IN_LOSS_REINS_KEY_ID)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(In_LOSS_REINS_CO_NO)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(In_LOSS_REINS_CO_NO)>0
	    and TRIM(In_LOSS_REINS_CO_NO)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_ID)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_ID)>0 AND TRIM(IN_LOSS_ID)='', 'N/A', LTRIM(RTRIM(IN_LOSS_ID))
	    )
	) AS LOSS_ID,
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE)) IS NULL
	    or LENGTH(IN_LOSS_INSURANCE_LINE)>0
	    and TRIM(IN_LOSS_INSURANCE_LINE)='',
	    'N/A',
	...
	IFF(
	    IN_LOSS_LOCATION_NUMBER IS NULL, '0000',
	    LTRIM(RTRIM(LPAD(TO_CHAR(IN_LOSS_LOCATION_NUMBER), 4, '0')))
	...
	IFF(
	    IN_LOSS_SUB_LOCATION_NUMBER IS NULL, '000',
	    LTRIM(RTRIM(LPAD(TO_CHAR(IN_LOSS_SUB_LOCATION_NUMBER), 3, '0')))
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_RISK_UNIT_GROUP)) IS NULL
	    or LENGTH(IN_LOSS_RISK_UNIT_GROUP)>0
	    and TRIM(IN_LOSS_RISK_UNIT_GROUP)='',
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_RISK_UNIT_GROUP)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_RISK_UNIT_GROUP)>0
	    and TRIM(IN_LOSS_RISK_UNIT_GROUP)='', '0',
	        LTRIM(RTRIM(IN_LOSS_RISK_UNIT_GROUP))
	    )
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_UNIT)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_UNIT)>0
	    and TRIM(IN_LOSS_UNIT)='', 'N/A', LTRIM(RTRIM(IN_LOSS_UNIT))
	    )
	) AS LOSS_UNIT,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOSS_UNIT))) OR IS_SPACES(IN_LOSS_UNIT),'N/A',LTRIM(RTRIM(IN_LOSS_UNIT)))
	IFF(
	    LTRIM(RTRIM(IN_LOSS_UNIT)) IS NULL OR LENGTH(IN_LOSS_UNIT)>0 AND TRIM(IN_LOSS_UNIT)='',
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_SEQUENCE_RISK_UNIT)) IS NULL
	    or LENGTH(IN_LOSS_SEQUENCE_RISK_UNIT)>0
	    and TRIM(IN_LOSS_SEQUENCE_RISK_UNIT)='',
	    '0',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_SEQUENCE_RISK_UNIT)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_SEQUENCE_RISK_UNIT)>0
	    and TRIM(IN_LOSS_SEQUENCE_RISK_UNIT)='', '0',
	        LTRIM(RTRIM(IN_LOSS_SEQUENCE_RISK_UNIT))
	    )
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_TYPE_EXPOSURE)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_TYPE_EXPOSURE)>0
	    and TRIM(IN_LOSS_TYPE_EXPOSURE)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_MAJOR_PERIL)>0
	    and TRIM(IN_LOSS_MAJOR_PERIL)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_MAJOR_PERIL)>0
	    and TRIM(IN_LOSS_MAJOR_PERIL)='', '0',
	        LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL))
	    )
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL_SEQ)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_MAJOR_PERIL_SEQ)>0
	    and TRIM(IN_LOSS_MAJOR_PERIL_SEQ)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL_SEQ)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_MAJOR_PERIL_SEQ)>0
	    and TRIM(IN_LOSS_MAJOR_PERIL_SEQ)='', '0',
	        LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL_SEQ))
	    )
	...
	-- 
	IFF(
	    LENGTH(IN_LOSS_OCCURENCE) > 2, SUBSTR(IN_LOSS_OCCURENCE, 2, 2), TO_CHAR(IN_LOSS_OCCURENCE)
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_MEMBER)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_MEMBER)>0
	    and TRIM(IN_LOSS_MEMBER)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_MEMBER)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_MEMBER)>0
	    and TRIM(IN_LOSS_MEMBER)='', '0',
	        LTRIM(RTRIM(IN_LOSS_MEMBER))
	    )
	) AS LOSS_MEMBER_lkp,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOSS_DISABILITY))),'N/A',IIF(IS_SPACES(IN_LOSS_DISABILITY),'N/A',LTRIM(RTRIM(IN_LOSS_DISABILITY))))
	IFF(
	    LTRIM(RTRIM(IN_LOSS_DISABILITY)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_DISABILITY)>0
	    and TRIM(IN_LOSS_DISABILITY)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_DISABILITY)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_DISABILITY)>0
	    and TRIM(IN_LOSS_DISABILITY)='', '0',
	        LTRIM(RTRIM(IN_LOSS_DISABILITY))
	    )
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_RESERVE_CATEGORY)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_RESERVE_CATEGORY)>0
	    and TRIM(IN_LOSS_RESERVE_CATEGORY)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_RESERVE_CATEGORY)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_RESERVE_CATEGORY)>0
	    and TRIM(IN_LOSS_RESERVE_CATEGORY)='', '0',
	        LTRIM(RTRIM(IN_LOSS_RESERVE_CATEGORY))
	    )
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_TRANSACTION)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_TRANSACTION)>0
	    and TRIM(IN_LOSS_TRANSACTION)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_ENTRY_OPERATOR)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_ENTRY_OPERATOR)>0
	    and TRIM(IN_LOSS_ENTRY_OPERATOR)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_TRANSACTION_CATEGORY)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_TRANSACTION_CATEGORY)>0
	    and TRIM(IN_LOSS_TRANSACTION_CATEGORY)='',
	        'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_CAUSE)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_CAUSE)>0
	    and TRIM(IN_LOSS_CAUSE)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_CAUSE)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_CAUSE)>0
	    and TRIM(IN_LOSS_CAUSE)='', '0', LTRIM(RTRIM(IN_LOSS_CAUSE))
	    )
	) AS LOSS_CAUSE_lkp,
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_DRAFT_NO)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_DRAFT_NO)>0
	    and TRIM(IN_LOSS_DRAFT_NO)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_ACCOUNT_ENTERED_DATE)) IS NULL, '180001',
	    IFF(
	        LENGTH(IN_LOSS_ACCOUNT_ENTERED_DATE)>0
	    and TRIM(IN_LOSS_ACCOUNT_ENTERED_DATE)='',
	        '180001',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_TYPE_DISABILITY)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_TYPE_DISABILITY)>0
	    and TRIM(IN_LOSS_TYPE_DISABILITY)='', 'N/A',
	        LTRIM(RTRIM(IN_LOSS_TYPE_DISABILITY))
	    )
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_OFFSET_ONSET_IND)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_OFFSET_ONSET_IND)>0
	    and TRIM(IN_LOSS_OFFSET_ONSET_IND)='', 'N/A',
	...
	DECODE(
	    ...
	    LENGTH(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE)))>0 AND TRIM(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE)))='', substr(ltrim(rtrim(IN_LOSS_ACCOUNT_ENTERED_DATE)), 5, 2),
	    length(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE))) = 0, substr(ltrim(rtrim(IN_LOSS_ACCOUNT_ENTERED_DATE)), 5, 2),
	    substr(IN_LOSS_TRANSACTION_DATE, 5, 2)
	...
	DECODE(
	    ...
	    LENGTH(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE)))>0 AND TRIM(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE)))='', '01',
	    length(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE))) = 0, '01',
	    substr(IN_LOSS_TRANSACTION_DATE, 7, 2)
	...
	DECODE(
	    ...
	    LENGTH(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE)))>0 AND TRIM(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE)))='', substr(ltrim(rtrim(IN_LOSS_ACCOUNT_ENTERED_DATE)), 1, 4),
	    length(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE))) = 0, substr(ltrim(rtrim(IN_LOSS_ACCOUNT_ENTERED_DATE)), 1, 4),
	    substr(IN_LOSS_TRANSACTION_DATE, 1, 4)
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_TIME)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_TIME)>0
	    and TRIM(IN_LOSS_TIME)='', 'N/A', LTRIM(RTRIM(IN_LOSS_TIME))
	    )
	) AS LOSS_TIME,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOGICAL_FLAG))),'N/A',IIF(IS_SPACES(IN_LOGICAL_FLAG),'N/A',LTRIM(RTRIM(IN_LOGICAL_FLAG))))
	IFF(
	    LTRIM(RTRIM(IN_LOGICAL_FLAG)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOGICAL_FLAG)>0
	    and TRIM(IN_LOGICAL_FLAG)='', 'N/A',
...
EXP_CLAIM_TRANSACTION_DETECT_CHANGES AS (
	...
	-- 
	IFF(
	    CAST(LOSS_SEQUENCE_RISK_UNIT AS FLOAT) = 0, '0',
	...
	IFF(LOSS_TRANSACTION IN ('90','91','92','95','97','98','99'), 0, LOSS_PAID_OR_RESV_AMT) AS OUT_LOSS_PAID_OR_RESV_AMT_FOR_LKP,
	...
	IFF(LOSS_TRANSACTION IN ('91','92'), '90', LOSS_TRANSACTION) AS OUT_LOSS_TRANSACTION_FOR_LKP,
	...
	IFF(
	    LOSS_TRANSACTION_CATEGORY = 'WD' OR LOSS_TRANSACTION_CATEGORY = 'DR',
	    LOSS_TRANSACTION_CATEGORY,
	    IFF(
	        LKP_trans_ctgry_code IS NULL, 'N/A', LKP_trans_ctgry_code
	    )
	...
	TO_TIMESTAMP(LOSS_TRANSACTION_DATE, 'MM/DD/YYYY') AS V_LOSS_TRANSACTION_DATE,
	V_LOSS_TRANSACTION_DATE AS LOSS_TRANSACTION_DATE_OP,
	-- *INF*: TO_DATE('01/01/1800 01:00:00','MM/DD/YYYY HH24:MI:SS')
	TO_TIMESTAMP('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS') AS S3P_UPDATED_DATE_OP,
	-- *INF*: TO_DATE('01/01/1800 01:00:00','MM/DD/YYYY HH24:MI:SS')
	TO_TIMESTAMP('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS') AS S3P_TO_PMS_TRANS_DATE_OP,
	EXP_CLAIMS_TRANSACTION_DEFAULT.LOSS_ACCOUNT_ENTERED_DATE,
	-- *INF*: TO_DATE(LOSS_ACCOUNT_ENTERED_DATE,'YYYYMM')
	TO_TIMESTAMP(LOSS_ACCOUNT_ENTERED_DATE, 'YYYYMM') AS LOSS_ACCOUNT_ENTERED_DATE_OP,
	...
	IFF(
	    claim_occurrence_ak_id_EXCEED IS NULL,
	...
	IFF(
	    claim_occurrence_ak_id_EXCEED IS NULL, LKP_CLAIM_PARTY_V_LOSS_PARTY_KEY.claim_party_ak_id, 0
	...
	IFF(
	    claim_occurrence_ak_id_EXCEED IS NULL,
	    IFF(
	        V_CLAIM_PARTY_AK_ID_LKP IS NULL, 0, V_CLAIM_PARTY_AK_ID_LKP
	    ),
	    0
	...
	-- 
	IFF(
	    claim_occurrence_ak_id_EXCEED IS NOT NULL,
	...
	-- 
	IFF(
	    claim_occurrence_ak_id_EXCEED IS NULL, V_CLAIM_PARTY_OCCURRENCE_AK_ID_PMS,
	...
	IFF(
	    LENGTH(to_char(ipfcx6_month_process)) = 1, '0' || to_char(ipfcx6_month_process),
	...
	IFF(
	    LENGTH(to_char(ipfcx6_day_process)) = 1, '0' || to_char(ipfcx6_day_process),
	...
	IFF(
	    v_reprocess_date = '00000', TO_TIMESTAMP('01011800', 'MMDDYYYY'),
	    TO_TIMESTAMP(LTRIM(RTRIM(v_reprocess_date)), 'MMDDYYYY')
	...
	IFF(
	    LOSS_OFFSET_ONSET_IND = 'N/A' OR v_reprocess_date_out IS NULL,
