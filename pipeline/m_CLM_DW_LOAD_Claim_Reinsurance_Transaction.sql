...
AGG_amts_on_90_91 AS (
	...
	SUM(in_loss_paid_or_resv_amt
	) AS loss_paid_or_resv_amt,
	loss_bank_number,
	loss_draft_amount,
	...
	SUM(in_loss_net_change_dollars
	) AS loss_net_change_dollars,
...
EXP_CLAIMS_TRANSACTION_DEFAULT AS (
	...
	IFF(LTRIM(RTRIM(IN_LOSS_CLAIM_NUMBER
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_CLAIM_NUMBER)>0 AND TRIM(IN_LOSS_CLAIM_NUMBER)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_CLAIM_NUMBER
				)
			)
		)
	) AS LOSS_CLAIM_NUMBER,
	...
	IFF(LTRIM(RTRIM(IN_loss_claim_payee_name
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_loss_claim_payee_name)>0 AND TRIM(IN_loss_claim_payee_name)='',
			'N/A',
			LTRIM(RTRIM(IN_loss_claim_payee_name
				)
			)
		)
	) AS Claim_Reins_Broker_name,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_NOTES_DRAFT_PAYEE
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_NOTES_DRAFT_PAYEE)>0 AND TRIM(IN_LOSS_NOTES_DRAFT_PAYEE)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_NOTES_DRAFT_PAYEE
				)
			)
		)
	) AS LOSS_NOTES_DRAFT_PAYEE,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOSS_ADJUSTOR_NO))),'N/A',IIF(IS_SPACES(IN_LOSS_ADJUSTOR_NO),'N/A',LTRIM(RTRIM(IN_LOSS_ADJUSTOR_NO))))
	IFF(LTRIM(RTRIM(IN_LOSS_ADJUSTOR_NO
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_ADJUSTOR_NO)>0 AND TRIM(IN_LOSS_ADJUSTOR_NO)='',
			'N/A',
	...
	-- 
	IFF(IN_LOSS_BASE_TRANSACTION IS NULL,
		'N/A',
		TO_CHAR(IN_LOSS_BASE_TRANSACTION
		)
	) AS LOSS_BASE_TRANSACTION,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_REINS_KEY_ID
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_REINS_KEY_ID)>0 AND TRIM(IN_LOSS_REINS_KEY_ID)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_REINS_KEY_ID
				)
			)
		)
	) AS LOSS_REINS_KEY_ID,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(In_LOSS_REINS_CO_NO))),'N/A',IIF(IS_SPACES(In_LOSS_REINS_CO_NO),'N/A',LTRIM(RTRIM(In_LOSS_REINS_CO_NO))))
	IFF(LTRIM(RTRIM(In_LOSS_REINS_CO_NO
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(In_LOSS_REINS_CO_NO)>0 AND TRIM(In_LOSS_REINS_CO_NO)='',
			'N/A',
			LTRIM(RTRIM(In_LOSS_REINS_CO_NO
				)
			)
		)
	) AS LOSS_REINS_CO_NO,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_ID
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_ID)>0 AND TRIM(IN_LOSS_ID)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_ID
				)
			)
		)
	) AS LOSS_ID,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE
			)
		) IS NULL 
		OR LENGTH(IN_LOSS_INSURANCE_LINE)>0 AND TRIM(IN_LOSS_INSURANCE_LINE)='',
		'N/A',
		LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE
			)
		)
	) AS LOSS_INSURANCE_LINE,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE))) OR IS_SPACES(IN_LOSS_INSURANCE_LINE),'N/A',LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE)))
	IFF(LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE
			)
		) IS NULL 
	...
	IFF(IN_LOSS_LOCATION_NUMBER IS NULL,
		'0000',
		LTRIM(RTRIM(LPAD(TO_CHAR(IN_LOSS_LOCATION_NUMBER
					), 4, '0'
				)
			)
		)
	) AS LOSS_LOCATION_NUMBER,
	-- *INF*: IIF(ISNULL(IN_LOSS_SUB_LOCATION_NUMBER),0,IN_LOSS_SUB_LOCATION_NUMBER)
	IFF(IN_LOSS_SUB_LOCATION_NUMBER IS NULL,
		0,
	...
	IFF(IN_LOSS_SUB_LOCATION_NUMBER IS NULL,
		'000',
		LTRIM(RTRIM(LPAD(TO_CHAR(IN_LOSS_SUB_LOCATION_NUMBER
					), 3, '0'
				)
			)
		)
	) AS LOSS_SUB_LOCATION_NUMBER,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_RISK_UNIT_GROUP
			)
		) IS NULL 
		OR LENGTH(IN_LOSS_RISK_UNIT_GROUP)>0 AND TRIM(IN_LOSS_RISK_UNIT_GROUP)='',
		'N/A',
		LTRIM(RTRIM(IN_LOSS_RISK_UNIT_GROUP
			)
		)
	) AS LOSS_RISK_UNIT_GROUP,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_UNIT
		...
		OR LENGTH(IN_LOSS_UNIT)>0 AND TRIM(IN_LOSS_UNIT)='',
		'N/A',
		LTRIM(RTRIM(IN_LOSS_UNIT
			)
		)
	...
	IFF(LTRIM(RTRIM(IN_LOSS_SEQUENCE_RISK_UNIT
			)
		) IS NULL 
		OR LENGTH(IN_LOSS_SEQUENCE_RISK_UNIT)>0 AND TRIM(IN_LOSS_SEQUENCE_RISK_UNIT)='',
		'0',
		LTRIM(RTRIM(IN_LOSS_SEQUENCE_RISK_UNIT
			)
		)
	) AS LOSS_SEQUENCE_RISK_UNIT,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_SEQUENCE_RISK_UNIT
		...
		'0',
		IFF(LENGTH(IN_LOSS_SEQUENCE_RISK_UNIT)>0 AND TRIM(IN_LOSS_SEQUENCE_RISK_UNIT)='',
			'0',
			LTRIM(RTRIM(IN_LOSS_SEQUENCE_RISK_UNIT
				)
			)
		)
	) AS LOSS_SEQUENCE_RISK_UNIT_lkp,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_TYPE_EXPOSURE
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_TYPE_EXPOSURE)>0 AND TRIM(IN_LOSS_TYPE_EXPOSURE)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_TYPE_EXPOSURE
				)
			)
		)
	) AS LOSS_TYPE_EXPOSURE,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL))),'N/A',IIF(IS_SPACES(IN_LOSS_MAJOR_PERIL),'N/A',LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL))))
	IFF(LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_MAJOR_PERIL)>0 AND TRIM(IN_LOSS_MAJOR_PERIL)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL
				)
			)
		)
	) AS LOSS_MAJOR_PERIL,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL_SEQ
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_MAJOR_PERIL_SEQ)>0 AND TRIM(IN_LOSS_MAJOR_PERIL_SEQ)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_MAJOR_PERIL_SEQ
				)
			)
		)
	) AS LOSS_MAJOR_PERIL_SEQ,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_DISABILITY
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_DISABILITY)>0 AND TRIM(IN_LOSS_DISABILITY)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_DISABILITY
				)
			)
		)
	) AS LOSS_DISABILITY,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_RESERVE_CATEGORY
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_RESERVE_CATEGORY)>0 AND TRIM(IN_LOSS_RESERVE_CATEGORY)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_RESERVE_CATEGORY
				)
			)
		)
	) AS LOSS_RESERVE_CATEGORY,
	...
	) AS LOSS_RESERVE_CATEGORY_lkp,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOSS_TRANSACTION))),'N/A',IIF(IS_SPACES(IN_LOSS_TRANSACTION),'N/A',LTRIM(RTRIM(IN_LOSS_TRANSACTION))))
	IFF(LTRIM(RTRIM(IN_LOSS_TRANSACTION
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_TRANSACTION)>0 AND TRIM(IN_LOSS_TRANSACTION)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_TRANSACTION
				)
			)
		)
	) AS LOSS_TRANSACTION,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_ENTRY_OPERATOR
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_ENTRY_OPERATOR)>0 AND TRIM(IN_LOSS_ENTRY_OPERATOR)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_ENTRY_OPERATOR
				)
			)
		)
	...
	IFF(LTRIM(RTRIM(IN_LOSS_TRANSACTION_CATEGORY
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_TRANSACTION_CATEGORY)>0 AND TRIM(IN_LOSS_TRANSACTION_CATEGORY)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_TRANSACTION_CATEGORY
				)
			)
		)
	) AS LOSS_TRANSACTION_CATEGORY,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOSS_CAUSE))),'N/A',IIF(IS_SPACES(IN_LOSS_CAUSE),'N/A',LTRIM(RTRIM(IN_LOSS_CAUSE))))
	IFF(LTRIM(RTRIM(IN_LOSS_CAUSE
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_CAUSE)>0 AND TRIM(IN_LOSS_CAUSE)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_CAUSE
				)
			)
		)
	) AS LOSS_CAUSE,
	...
	IFF(IN_LOSS_PAID_OR_RESV_AMT IS NULL,
		0,
		IN_LOSS_PAID_OR_RESV_AMT
	) AS LOSS_PAID_OR_RESV_AMT,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOSS_DRAFT_NO))),'N/A',IIF(IS_SPACES(IN_LOSS_DRAFT_NO),'N/A',LTRIM(RTRIM(IN_LOSS_DRAFT_NO))))
	IFF(LTRIM(RTRIM(IN_LOSS_DRAFT_NO
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_DRAFT_NO)>0 AND TRIM(IN_LOSS_DRAFT_NO)='',
			'N/A',
	...
	IFF(IN_LOSS_NET_CHANGE_DOLLARS IS NULL,
		0,
		IN_LOSS_NET_CHANGE_DOLLARS
	) AS LOSS_NET_CHANGE_DOLLARS,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_ACCOUNT_ENTERED_DATE
			)
		) IS NULL,
		'180001',
		IFF(LENGTH(IN_LOSS_ACCOUNT_ENTERED_DATE)>0 AND TRIM(IN_LOSS_ACCOUNT_ENTERED_DATE)='',
			'180001',
			LTRIM(RTRIM(IN_LOSS_ACCOUNT_ENTERED_DATE
				)
			)
		)
	) AS LOSS_ACCOUNT_ENTERED_DATE,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_TYPE_DISABILITY
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_TYPE_DISABILITY)>0 AND TRIM(IN_LOSS_TYPE_DISABILITY)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_TYPE_DISABILITY
				)
			)
		)
	) AS LOSS_TYPE_DISABILITY,
	...
	IFF(LTRIM(RTRIM(IN_LOSS_OFFSET_ONSET_IND
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_OFFSET_ONSET_IND)>0 AND TRIM(IN_LOSS_OFFSET_ONSET_IND)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_OFFSET_ONSET_IND
				)
			)
		)
	) AS LOSS_OFFSET_ONSET_IND,
	...
	-- 
	DECODE(TRUE,
		LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE
			)
		) IS NULL, substr(ltrim(rtrim(IN_LOSS_ACCOUNT_ENTERED_DATE
				)
			), 5, 2
		),
		LENGTH(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE
			)
		))='', substr(ltrim(rtrim(IN_LOSS_ACCOUNT_ENTERED_DATE
				)
			), 5, 2
		),
		length(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE
		...
		) = 0, substr(ltrim(rtrim(IN_LOSS_ACCOUNT_ENTERED_DATE
				)
			), 5, 2
		),
		substr(IN_LOSS_TRANSACTION_DATE, 5, 2
		)
	) AS V_Loss_transaction_month,
	-- *INF*: DECODE(TRUE,
	...
	) AS V_loss_trasaction_year,
	-- *INF*: Concat (V_Loss_transaction_month ,Concat ('/' , Concat (V_loss_transaction_day,Concat ('/', V_loss_trasaction_year))))
	Concat(V_Loss_transaction_month, Concat('/', Concat(V_loss_transaction_day, Concat('/', V_loss_trasaction_year
				)
			)
		)
	...
	IFF(LTRIM(RTRIM(IN_LOSS_TIME
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOSS_TIME)>0 AND TRIM(IN_LOSS_TIME)='',
			'N/A',
			LTRIM(RTRIM(IN_LOSS_TIME
				)
			)
		)
	) AS LOSS_TIME,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOGICAL_FLAG))),'N/A',IIF(IS_SPACES(IN_LOGICAL_FLAG),'N/A',LTRIM(RTRIM(IN_LOGICAL_FLAG))))
	IFF(LTRIM(RTRIM(IN_LOGICAL_FLAG
			)
		) IS NULL,
		'N/A',
		IFF(LENGTH(IN_LOGICAL_FLAG)>0 AND TRIM(IN_LOGICAL_FLAG)='',
			'N/A',
			LTRIM(RTRIM(IN_LOGICAL_FLAG
				)
			)
		)
	) AS LOGICAL_FLAG
...
EXP_CLAIM_TRANSACTION_DETECT_CHANGES AS (
	...
	IFF(TO_CHAR(LOSS_CLASS_CODE_GROUP
		...
		'N/A',
		LPAD(TO_CHAR(LOSS_CLASS_CODE_GROUP
			) || TO_CHAR(LOSS_CLASS_CODE_MEMBER
			), 3, '0'
		)
	...
	PIF_SYMBOL || PIF_POLICY_NUMBER || PIF_MODULE || V_LOSS_DATE || TO_CHAR(LOSS_OCCURENCE
	) AS V_LOSS_OCCURENCE_KEY,
	...
	LTRIM(RTRIM(LOSS_REINS_CO_NO
		)
	) AS v_LOSS_REINS_CO_NO,
	...
	IFF(LOSS_NET_CHANGE_DOLLARS = 0,
		'39',
		'38'
	) AS TRANS_TYPE_CODE_1,
	...
	IFF(LOSS_TRANSACTION IN ('90','91','92','95','97','98','99'),
		0,
		LOSS_PAID_OR_RESV_AMT
	) AS OUT_LOSS_PAID_OR_RESV_AMT_FOR_LKP,
	-- *INF*: IIF(LOSS_PAID_OR_RESV_AMT=0,'40','30')
	IFF(LOSS_PAID_OR_RESV_AMT = 0,
		'40',
		'30'
	) AS TRANS_TYPE_CODE_2,
	EXP_CLAIMS_TRANSACTION_DEFAULT.LOSS_TRANSACTION,
	-- *INF*: IIF(IN(LOSS_TRANSACTION, '91', '92'), '90', LOSS_TRANSACTION) 
	IFF(LOSS_TRANSACTION IN ('91','92'),
		'90',
		LOSS_TRANSACTION
	) AS OUT_LOSS_TRANSACTION_FOR_LKP,
	...
	IFF(LKP_trans_ctgry_code IS NULL,
		'N/A',
		LKP_trans_ctgry_code
	) AS trans_ctgry_code,
	...
	IFF(LOSS_TRANSACTION_CATEGORY = 'WD' 
		OR LOSS_TRANSACTION_CATEGORY = 'DR',
		LOSS_TRANSACTION_CATEGORY,
		IFF(LKP_trans_ctgry_code IS NULL,
			'N/A',
			LKP_trans_ctgry_code
		)
	) AS V_LOSS_TRANSACTION_CATEGORY,
	...
	TO_DATE(LOSS_TRANSACTION_DATE, 'MM/DD/YYYY'
	) AS V_LOSS_TRANSACTION_DATE,
	V_LOSS_TRANSACTION_DATE AS LOSS_TRANSACTION_DATE_OP,
	-- *INF*: TO_DATE('01/01/1800 01:00:00','MM/DD/YYYY HH24:MI:SS')
	TO_DATE('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS'
	) AS S3P_UPDATED_DATE_OP,
	-- *INF*: TO_DATE('01/01/1800 01:00:00','MM/DD/YYYY HH24:MI:SS')
	TO_DATE('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS'
	) AS S3P_TO_PMS_TRANS_DATE_OP,
	EXP_CLAIMS_TRANSACTION_DEFAULT.LOSS_ACCOUNT_ENTERED_DATE,
	-- *INF*: TO_DATE(LOSS_ACCOUNT_ENTERED_DATE,'YYYYMM')
	TO_DATE(LOSS_ACCOUNT_ENTERED_DATE, 'YYYYMM'
	) AS LOSS_ACCOUNT_ENTERED_DATE_OP,
	...
	IFF(claim_occurrence_ak_id_EXCEED IS NULL,
		LKP_CLAIM_OCCURRENCE_V_LOSS_OCCURENCE_KEY.claim_occurrence_ak_id,
		claim_occurrence_ak_id_EXCEED
	) AS V_CLAIM_OCCURRENCE_AK_ID_LKP,
	-- *INF*: IIF(ISNULL(V_CLAIM_OCCURRENCE_AK_ID_LKP), 0, V_CLAIM_OCCURRENCE_AK_ID_LKP)
	-- 
	-- 
	-- 
	IFF(V_CLAIM_OCCURRENCE_AK_ID_LKP IS NULL,
		0,
		V_CLAIM_OCCURRENCE_AK_ID_LKP
	) AS V_CLAIM_OCCURENCE_AK_ID,
	...
	IFF(claim_occurrence_ak_id_EXCEED IS NULL,
		LKP_CLAIM_PARTY_V_LOSS_PARTY_KEY.claim_party_ak_id,
		0
	) AS V_CLAIM_PARTY_AK_ID_LKP,
	...
	-- 
	IFF(claim_occurrence_ak_id_EXCEED IS NULL,
		IFF(V_CLAIM_PARTY_AK_ID_LKP IS NULL,
			0,
			V_CLAIM_PARTY_AK_ID_LKP
		),
		0
	...
	-- 
	IFF(claim_occurrence_ak_id_EXCEED IS NULL,
		LKP_CLAIM_PARTY_OCCURRENCE_V_CLAIM_OCCURENCE_AK_ID_V_CLAIM_PARTY_AK_ID_V_PARTY_ROLE_CODE.claim_party_occurrence_ak_id,
		0
	) AS V_CLAIM_PARTY_OCCURRENCE_AK_ID_PMS,
	...
	-- 
	IFF(claim_occurrence_ak_id_EXCEED IS NOT NULL,
		LKP_CLAIM_PARTY_OCCURRENCE_EXCEED_claim_occurrence_ak_id_EXCEED_LTRIM_RTRIM_LOSS_CLAIMANT.claim_party_occurrence_ak_id,
		0
	) AS V_CLAIM_PARTY_OCCURRENCE_AK_ID_EXCEED,
	-- *INF*: IIF(ISNULL(claim_occurrence_ak_id_EXCEED),V_CLAIM_PARTY_OCCURRENCE_AK_ID_PMS,V_CLAIM_PARTY_OCCURRENCE_AK_ID_EXCEED)
	-- 
	-- 
	-- 
	IFF(claim_occurrence_ak_id_EXCEED IS NULL,
		V_CLAIM_PARTY_OCCURRENCE_AK_ID_PMS,
		V_CLAIM_PARTY_OCCURRENCE_AK_ID_EXCEED
	) AS V_CLAIM_PARTY_OCCURENCE_AK_ID,
	...
	-- 
	IFF(claim_occurrence_ak_id_EXCEED IS NULL,
		LKP_CLAIMANT_DETAIL_COVERAGE_PMS_V_CLAIM_PARTY_OCCURENCE_AK_ID_V_LOSS_LOCATION_NUMBER_V_LOSS_SUB_LOCATION_NUMBER_LOSS_INSURANCE_LINE_LOSS_RISK_UNIT_GROUP_V_RISK_UNIT_GRP_SEQ_NUM_V_LOSS_UNIT_V_LOSS_SEQUENCE_RISK_UNIT_LOSS_MAJOR_PERIL_V_LOSS_MAJOR_PERIL_SEQ_LOSS_DISABILITY_LOSS_RESERVE_CATEGORY_v_LOSS_CAUSE_LOSS_MEMBER.claimant_cov_det_ak_id,
		LKP_CLAIMANT_COVERAGE_DETAIL_EXCEED_V_CLAIM_PARTY_OCCURENCE_AK_ID_LOSS_INSURANCE_LINE_V_LOSS_LOCATION_NUMBER_V_LOSS_SUB_LOCATION_NUMBER_LOSS_RISK_UNIT_GROUP_V_RISK_UNIT_GRP_SEQ_NUM_V_LOSS_UNIT_SUBSTR_V_LOSS_SEQUENCE_RISK_UNIT_1_1_LOSS_MAJOR_PERIL_LOSS_RESERVE_CATEGORY_LOSS_CAUSE_LOSS_MAJOR_PERIL_SEQ.claimant_cov_det_ak_id
	...
	IFF(V_CLAIMANT_COV_DET_AK_ID IS NULL,
		- 1,
		V_CLAIMANT_COV_DET_AK_ID
	) AS OUT_CLAIMANT_COV_DET_AK_ID,
	LKP_pif_42x6_stage.ipfcx6_year_process,
	-- *INF*: to_char(ipfcx6_year_process)
	to_char(ipfcx6_year_process
	) AS V_ipfcx6_year_process,
	...
	IFF(LENGTH(to_char(ipfcx6_month_process
			)
		) = 1,
		'0' || to_char(ipfcx6_month_process
		),
		to_char(ipfcx6_month_process
		)
	) AS v_ipfcx6_month_process,
	...
	IFF(LENGTH(to_char(ipfcx6_day_process
			)
		) = 1,
		'0' || to_char(ipfcx6_day_process
		),
		to_char(ipfcx6_day_process
		)
	) AS v_ipfcx6_day_process,
	...
	IFF(v_reprocess_date = '00000',
		TO_DATE('01011800', 'MMDDYYYY'
		),
		to_date(LTRIM(RTRIM(v_reprocess_date
				)
			), 'MMDDYYYY'
		)
	) AS v_reprocess_date_out,
	...
	IFF(LOSS_OFFSET_ONSET_IND = 'N/A' 
		OR v_reprocess_date_out IS NULL,
		TO_DATE('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS'
		),
		v_reprocess_date_out
	) AS reprocess_date_out,
	...
	IFF(LKP_CLAIM_TRANSACTION_V_CLAIMANT_COV_DET_AK_ID.claim_trans_id IS NULL,
		1,
		0
	) AS MISSING_FLAG,
