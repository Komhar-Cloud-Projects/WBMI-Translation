...
EXP_PremiumMaster_DCT AS (
	...
	-- i_WrittenExposure)
	DECODE(TRUE,
		i_InsuranceLine = 'WorkersCompensation' 
		AND i_PremiumTransactionAmount < 0 
		AND i_WrittenExposure > 0, - 1 * i_WrittenExposure,
		i_WrittenExposure
	...
	IFF(v_ReasonAmendedCode IS NOT NULL,
		v_ReasonAmendedCode,
		'N/A'
...
EXP_DirectTransactions AS (
	...
	DECODE(TRUE,
		...
		AND PremiumTransactionAmount < 0 
		AND i_WrittenExposure > 0, - 1 * i_WrittenExposure,
		i_WrittenExposure
	) AS v_WrittenExposure,
	-- *INF*: DECODE(TRUE,
...
EXP_Evaluate_fields_for_OnsetOffset AS (
	...
	-- 
	-- -- SEE COMMENTS
	IFF(pol_sym = v_prev_row_pol_sym 
	...
	SUBSTR(TO_CHAR(( DATEADD(MONTH,@{pipeline().parameters.NO_OF_MONTHS},CURRENT_TIMESTAMP(
				)) 
			), 'YYYYMMDD'
		), 1, 6
	) AS v_Account_Date,
	...
	IFF(substr(TO_CHAR(PremiumTransactionBookedDate, 'YYYYMMDD'
			), 1, 6
		) = v_Account_Date,
		'VALID',
		'INVALID'
	...
	ROUND(DATEDIFF(DAY,PremiumTransactionEffectiveDate,PremiumTransactionExpirationDate) / DATEDIFF(DAY,pol_eff_date,pol_exp_date), 4
	) AS v_Pro_Rata_Factor,
	...
	IFF(v_Prior_Transaction_Found = 'NOPRIOR_TRANS',
		0.0,
		( v_Accum_Total_Original_Premium + v_prev_row_FullTermPremium 
		)
	...
	-- 
	-- --'PRIOR_TRANS_FOUND','NOPRIOR_TRANS'
	IFF(v_Prior_Transaction_Found = 'PRIOR_TRANS_FOUND',
		v_Total_Original_Premium,
		0.0
	) AS v_Accum_Total_Original_Premium,
	-- *INF*: ROUND(((-1) *  v_Total_Original_Premium * v_Pro_Rata_Factor),4)
	ROUND(( ( - 1 
			) * v_Total_Original_Premium * v_Pro_Rata_Factor 
		), 4
	) AS v_Offset_Premium,
	...
	-- 
	IFF(v_Valid_Record_Generation = 'VALID',
		IFF(v_valid_transaction_codes = 'VALID_TRANS_CODES' 
			AND v_Prior_Transaction_Found = 'PRIOR_TRANS_FOUND' 
			AND NOT pms_pol_lob_code IN ('WC','WCP'),
			IFF(v_Onset_Original_Premium = 0.00,
				'GENERATE_PREMIUM',
				'ONSET_OFFSET'
			),
			'GENERATE_PREMIUM'
...
EXP_ReinsCov_Prejoin_ops AS (
	...
	ltrim(rtrim(reins_section_code
		)
	) AS reins_section_code_out,
	reins_eff_date,
	reins_exp_date,
...
EXP_Reinsurance_PreJoin_ops AS (
	...
	LTRIM(RTRIM(ReinsuranceSectionCode3
		)
	) AS ReinsuranceSectionCode_Out,
...
EXP_Generate_DirectPremium_Reinsurance AS (
	...
	TO_CHAR(reins_risk_unit_group
	) AS v_reins_risk_unit_group,
	...
	-- 
	-- -- see comments
	IFF(( v_pif_number_installments > 1 
		),
		IFF(( reins_expiration_date > Hold_sar_exp_date 
			),
			Hold_sar_exp_date,
	...
	-- -- see comments
	-- 
	IFF(NOT LTRIM(RTRIM(pms_pol_lob_code3
			)
		) IN ('HP'),
		IFF(( reins_expiration_date >= reins_effective_date 
			) 
	...
	DECODE(v_reins_validity,
		...
		'VALID', IFF(LTRIM(RTRIM(reins_insurance_line
				)
			) = 'GL' 
			AND v_reins_location_number <> v_location_number,
			'NOREINSURANCE',
			IFF(RTRIM(reins_location
				) = RiskUnit3 
				OR RTRIM(reins_location
				) = 'N/A',
				'REINSURANCE',
				'NOREINSURANCE'
			)
	...
	DECODE(v_reinsurance_flag,
		...
		'REINSURANCE', IFF(( MajorPerilCode3 IN ('088','089','127','184','128','188') 
	...
	IFF(PremiumTransactionCode3 IN ('14','24'),
		StatisticalCoverageEffectiveDate3,
		PremiumTransactionEffectiveDate3
	...
	IFF(PremiumTransactionEffectiveDate3 >= reins_effective_date 
		AND PremiumTransactionEffectiveDate3 < reins_expiration_date,
		DATEDIFF(DAY,PremiumTransactionEffectiveDate3,reins_expiration_date),
		IFF(PremiumTransactionExpirationDate3 > reins_effective_date 
			AND PremiumTransactionExpirationDate3 <= reins_expiration_date,
	...
	IFF(PremiumTransactionExpirationDate3 < reins_effective_date 
		...
		IFF(PremiumTransactionEffectiveDate3 >= reins_effective_date 
			AND PremiumTransactionExpirationDate3 <= reins_expiration_date,
			IFF(( Hold_sar_eff_date >= reins_effective_date 
	...
	ROUND(PremiumTransactionAmount3 * reins_percent_prem_ceded * I_Pro_Rata_Factor, 3
	) AS V_REINS_CEDED_PREMIUM,
	V_REINS_CEDED_PREMIUM AS OUT_REINS_CEDED_PREMIUM,
	-- *INF*: ROUND(FullTermPremium3 * reins_percent_prem_ceded  * I_Pro_Rata_Factor ,3)
	ROUND(FullTermPremium3 * reins_percent_prem_ceded * I_Pro_Rata_Factor, 3
	) AS V_REINS_CEDED_ORIG_PREMIUM,
...
mplt_PremiumMasterStatisticalCodeDerivation AS (WITH
	...
	EXP_Pre_StatCode_ops AS (
		...
		IFF(InsuranceLine5 = 'N/A',
			'  ',
			LTRIM(RTRIM(InsuranceLine5
				)
			)
		) AS ins_line,
		pif02_audit_code,
		...
		IFF(SourceSystemID = 'PMS',
			IFF(v_pif43ld_audit_code IS NULL,
				pif02_audit_code,
				v_pif43ld_audit_code
			),
			'N/A'
		) AS AuditCode,
		...
		IFF(SUBSTR(v_comments_area, 1, 2
			) = 'CS',
			SUBSTR(v_comments_area, 3, 12
			),
			''
		) AS comments_cs,
		pol_eff_date5,
		...
		CAST(TO_CHAR(pol_exp_date5, 'YYYY'
		) AS INTEGER) AS v_pif_exp_yr,
		-- *INF*: to_integer(TO_CHAR(pol_exp_date5, 'MM'))
		CAST(TO_CHAR(pol_exp_date5, 'MM'
		) AS INTEGER) AS v_pif_exp_mm,
		-- *INF*: to_integer(TO_CHAR(pol_eff_date5, 'YYYY'))
		CAST(TO_CHAR(pol_eff_date5, 'YYYY'
		) AS INTEGER) AS v_pif_eff_yr,
		-- *INF*: to_integer(TO_CHAR(pol_eff_date5, 'MM'))
		CAST(TO_CHAR(pol_eff_date5, 'MM'
		) AS INTEGER) AS v_pif_eff_mm,
		...
		( CAST(DATEDIFF(MONTH,PremiumTransactionEffectiveDate,pol_eff_date5) AS INTEGER) / v_pif_installment_term_n 
		) + 1 AS v_calc_installments,
		-- *INF*: v_pif_eff_yr+TO_INTEGER((v_pif_installment_term_n * v_calc_installments)/12,TRUE)+1
		v_pif_eff_yr + CAST(( v_pif_installment_term_n * v_calc_installments 
		) / 12 AS INTEGER) + 1 AS v_calc_yr,
		...
		IFF(v_pif_installment_term_n <> 0,
			IFF(Pol_Term = '0' 
				OR Pol_Term = '999',
				v_pif_exp_yr,
				IFF(v_calc_yr < v_pif_exp_yr,
					v_calc_yr,
					v_pif_exp_yr
				)
			),
		...
		substr(to_char(v_PremiumMasterPolicyExpirationYear
			), 3, 2
		) AS PremiumMasterPolicyExpirationYear,
		-- *INF*: to_char(v_pol_term)
		to_char(v_pol_term
		) AS PremiumMasterPolicyTerm,
		-- *INF*: IIF(IN(v_pif_installment_term,'0','999'),v_pif_installment_term,
		-- to_char(v_calc_installments))
		IFF(v_pif_installment_term IN ('0','999'),
			v_pif_installment_term,
			to_char(v_calc_installments
			)
		...
		ltrim(rtrim(BureauStatisticalUserLine5
			)
		) AS ProductLine,
		SubLineCode,
		ReasonAmendedCode,
	...
	EXP_Derive_CSP_Fields AS (
		...
		SUBSTR(Formatted_Stat_Codes_26_34, 1, 3
		) AS csp_rate_mod,
		-- *INF*: SUBSTR(Formatted_Stat_Codes_26_34,4,3)
		SUBSTR(Formatted_Stat_Codes_26_34, 4, 3
		) AS csp_rate_dep
	...
	EXP_PassThrough_Preoutput AS (
		...
		DECODE(TRUE,
			substr(ProductLine, 1, 2
			) IN ('87','88','89','90','91','92','93','94','95','96','97'), 'NB',
			TypeBureauCodeIn
...
EXP_Insert_DirectTransactions AS (
	...
	DATEADD(SECOND,59-DATE_PART(SECOND,DATEADD(MINUTE,59-DATE_PART(MINUTE,DATEADD(HOUR,23-DATE_PART(HOUR,last_day(DATEADD(MONTH,@{pipeline().parameters.NO_OF_MONTHS},sysdate)
	)),last_day(DATEADD(MONTH,@{pipeline().parameters.NO_OF_MONTHS},sysdate)
	))),DATEADD(HOUR,23-DATE_PART(HOUR,last_day(DATEADD(MONTH,@{pipeline().parameters.NO_OF_MONTHS},sysdate)
	)),last_day(DATEADD(MONTH,@{pipeline().parameters.NO_OF_MONTHS},sysdate)
	)))),DATEADD(MINUTE,59-DATE_PART(MINUTE,DATEADD(HOUR,23-DATE_PART(HOUR,last_day(DATEADD(MONTH,@{pipeline().parameters.NO_OF_MONTHS},sysdate)
	)),last_day(DATEADD(MONTH,@{pipeline().parameters.NO_OF_MONTHS},sysdate)
	))),DATEADD(HOUR,23-DATE_PART(HOUR,last_day(DATEADD(MONTH,@{pipeline().parameters.NO_OF_MONTHS},sysdate)
	)),last_day(DATEADD(MONTH,@{pipeline().parameters.NO_OF_MONTHS},sysdate)
	)))) AS o_PremiumMasterBookedDateOut,
	...
	IFF(i_PremiumMasterPolicyExpirationYear IS NULL,
		'N/A',
		i_PremiumMasterPolicyExpirationYear
	) AS o_PremiumMasterPolicyExpirationYear,
	...
	IFF(( ltrim(rtrim(PremiumMasterTransactionCode
				)
			) IN ('55','65') 
		),
		TO_DATE('12/31/2099 00:00:00', 'MM/DD/YYYY HH24:MI:SS'
		),
	...
	IFF(i_PremiumMasterPolicyTerm IS NULL,
		'N/A',
		i_PremiumMasterPolicyTerm
	) AS o_PremiumMasterPolicyTerm,
	-- *INF*: IIF(ISNULL(ltrim(rtrim(i_PremiumMasterBureauStatisticalLine))),'N/A',i_PremiumMasterBureauStatisticalLine)
	IFF(ltrim(rtrim(i_PremiumMasterBureauStatisticalLine
			)
		) IS NULL,
		'N/A',
	...
	IFF(SUBSTR(i_comments_cs, 1, 1
		) IS NULL,
		'   ',
		SUBSTR(i_comments_cs, 1, 1
		)
	) AS o_PremiumMasterCountersignAgencyType,
	-- *INF*: IIF(ISNULL(SUBSTR(i_comments_cs,2,7)),'   ',SUBSTR(i_comments_cs,2,7))
	IFF(SUBSTR(i_comments_cs, 2, 7
		) IS NULL,
		'   ',
		SUBSTR(i_comments_cs, 2, 7
		)
	) AS o_PremiumMasterCountersignAgencyCode,
	-- *INF*: IIF(ISNULL(SUBSTR(i_comments_cs,9,2)),'   ',SUBSTR(i_comments_cs,9,2))
	IFF(SUBSTR(i_comments_cs, 9, 2
		) IS NULL,
		'   ',
		SUBSTR(i_comments_cs, 9, 2
		)
	) AS o_PremiumMasterCountersignAgencyState,
	-- *INF*: IIF(ISNULL(SUBSTR(i_comments_cs,11,2)),'   ',REPLACECHR(0, SUBSTR(i_comments_cs,11,2), ',', '0'))
	IFF(SUBSTR(i_comments_cs, 11, 2
		) IS NULL,
		'   ',
		REGEXP_REPLACE(SUBSTR(i_comments_cs, 11, 2
		),',','0','i')
	) AS o_PremiumMasterCountersignAgencyRate,
	-- *INF*: IIF(LTRIM(RTRIM(PremiumMasterPremiumType)) = 'D','0','50')
	IFF(LTRIM(RTRIM(PremiumMasterPremiumType
			)
		) = 'D',
		'0',
		'50'
	) AS o_TaxBoardPercentage,
	-- *INF*: IIF(ISNULL(i_PremiumMasterCustomerCareCommissionRate),1,i_PremiumMasterCustomerCareCommissionRate)
	IFF(i_PremiumMasterCustomerCareCommissionRate IS NULL,
		1,
