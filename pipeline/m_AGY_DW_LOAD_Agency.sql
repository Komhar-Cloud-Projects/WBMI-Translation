...
EXP_Lkp_Values AS (
	...
	IFF(in_rsm_terr_sym IS NULL, 'N/A', IFF(is_spaces(in_rsm_terr_sym), 'N/A', in_rsm_terr_sym)) AS out_rsm_TERR_SYM,
	...
	IFF(in_rsm_TERR_NAME IS NULL, 'Not Found', IFF(is_spaces(in_rsm_TERR_NAME), 'Not Found', ltrim(rtrim(in_rsm_TERR_NAME)))) AS out_rsm_TERR_NAME,
	...
	IFF(STATE_CODE IS NULL, 'N/A', IFF(is_spaces(STATE_CODE), 'N/A', STATE_CODE)) AS v_STATE_CODE,
	...
	-- 
	-- 
	IFF(in_agency_state_abbrev IS NULL, 'N/A', IFF(IS_SPACES(in_agency_state_abbrev), 'N/A', in_agency_state_abbrev)) AS agency_state_abbrev,
	...
	IFF(in_agency_state_descript IS NULL, 'Not Available', IFF(is_spaces(in_agency_state_descript), 'Not Available', in_agency_state_descript)) AS out_agency_state_descript,
	EXP_Values.AGENCY_NUM AS in_AGENCY_NUM,
	-- *INF*: iif(isnull(in_AGENCY_NUM),'N/A',
	-- iif(is_spaces(in_AGENCY_NUM),'N/A',in_AGENCY_NUM))
	IFF(in_AGENCY_NUM IS NULL, 'N/A', IFF(is_spaces(in_AGENCY_NUM), 'N/A', in_AGENCY_NUM)) AS out_AGENCY_NUM,
	...
	IFF(in_AGENCY_NAME IS NULL, 'Not Available', IFF(is_spaces(in_AGENCY_NAME), 'Not Available', ltrim(rtrim(in_AGENCY_NAME)))) AS out_AGENCY_NAME,
	...
	IFF(in_DIRCONN_PER_DATE IS NULL, TO_DATE('1/1/1800', 'MM/DD/YYYY'), in_DIRCONN_PER_DATE) AS out_DIRCONN_PER_DATE,
	...
	IFF(in_DIRCONN_COMM_DATE IS NULL, TO_DATE('1/1/1800', 'MM/DD/YYYY'), in_DIRCONN_COMM_DATE) AS out_DIRCONN_COMM_DATE,
	...
	IFF(RSM_LAST_NAME IS NULL, 'Not Available', IFF(is_spaces(RSM_LAST_NAME), 'Not Available', ( ltrim(rtrim(RSM_LAST_NAME)) ))) AS v_RSM_Last_Name,
	...
	IFF(RSM_FIRST_NAME IS NULL, 'N/A', IFF(is_spaces(RSM_FIRST_NAME), 'N/A', ( ltrim(rtrim(RSM_FIRST_NAME)) ))) AS v_RSM_FIRST_NAME,
	-- *INF*: iif(isnull
	-- (ltrim(rtrim(RSM_FIRST_NAME))
	-- ||' ' || (ltrim(rtrim(RSM_LAST_NAME)))),'Not Available',(ltrim(rtrim(RSM_FIRST_NAME)))||' ' || (ltrim(rtrim(RSM_LAST_NAME))))
	-- 
	IFF(ltrim(rtrim(RSM_FIRST_NAME)) || ' ' || ( ltrim(rtrim(RSM_LAST_NAME)) ) IS NULL, 'Not Available', ( ltrim(rtrim(RSM_FIRST_NAME)) ) || ' ' || ( ltrim(rtrim(RSM_LAST_NAME)) )) AS out_rsm_full_name,
...
EXP_Detect_Changes AS (
	...
	IFF(in_prim_agency_state_descript IS NULL, agency_state_descript, in_prim_agency_state_descript) AS v_prim_agency_state_descript,
	...
	RTRIM(IFF(in_prim_AGENCY_NAME IS NULL, AGENCY_NAME, in_prim_AGENCY_NAME)) AS v_prim_AGENCY_NAME,
	...
	IFF(in_pay_code_exp_date IS NULL, TO_DATE('1/1/1800', 'MM/DD/YYYY'), in_pay_code_exp_date) AS v_pay_code_exp_date,
	...
	IFF(in_pay_code_eff_date IS NULL, TO_DATE('1/1/1800', 'MM/DD/YYYY'), in_pay_code_eff_date) AS v_pay_code_eff_date,
	...
	-- 'UPDATE',
	-- 'NOCHANGE'))
	IFF(agency_id_old IS NULL, 'NEW', IFF(( rsm_TERR_SYM <> rsm_terr_sym_old ) OR ( in_rsm_TERR_CODE <> RTrim(rsm_terr_code_old, '  ') ) OR ( rsm_TERR_NAME <> rsm_terr_name_old ) OR ( out_rsm_full_name <> rsm_full_name_old ) OR ( out_rsm_last_name <> rsm_last_name_old ) OR ( LPAD(v_prim_agency_state_code, 2) <> LPAD(prim_agency_state_code_old, 2) ) OR ( v_prim_agency_state_abbrev <> prim_agency_state_abbrev_old ) OR ( v_prim_agency_state_descript <> prim_agency_state_descript_old ) OR ( v_prim_AGENCY_NUM <> prim_agency_num_old ) OR ( V_out_prim_agency_key <> prim_agency_key_old ) OR ( v_prim_AGENCY_NAME <> prim_agency_name_old ) OR ( LPAD(agency_state_code, 2) <> LPAD(agency_state_code_old, 2) ) OR ( agency_state_abbrev <> agency_state_abbrev_old ) OR ( agency_state_descript <> agency_state_descript_old ) OR ( AGENCY_NUM <> agency_num_old ) OR ( out_Agency_Key <> agency_key_old ) OR ( AGENCY_NAME <> agency_name_old ) OR ( out_Audit_Terr_Name_Rule_id <> rsm_terr_name_rule_id_old ) OR ( DIRCONN_PER_DATE <> dirconn_per_date_old ) OR ( DIRCONN_COMM_DATE <> dirconn_comm_date_old ) OR ( v_pay_code <> agency_pay_code ) OR ( v_pay_code_exp_date <> agency_pay_code_eff_to_date ) OR ( v_pay_code_eff_date <> agency_pay_code_eff_from_date ), 'UPDATE', 'NOCHANGE')) AS v_changed_flag,
