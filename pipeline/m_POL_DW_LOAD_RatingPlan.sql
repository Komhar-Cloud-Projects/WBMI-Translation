...
EXP_NumericValues AS (
	...
	IFF(i_RatingPlanId IS NULL,
		- 1,
		i_RatingPlanId
	) AS o_RatingPlanId,
	-- *INF*: IIF(ISNULL(i_RatingPlanAKId),-1,i_RatingPlanAKId)
	IFF(i_RatingPlanAKId IS NULL,
		- 1,
		i_RatingPlanAKId
	) AS o_RatingPlanAKId
	FROM SQ_RatingPlan
...
EXP_StringValues AS (
	...
	IFF(i_RatingPlanDescription IS NULL 
		OR LENGTH(i_RatingPlanDescription
		) = 0 
		OR LENGTH(i_RatingPlanDescription)>0 AND TRIM(i_RatingPlanDescription)='',
		'N/A',
