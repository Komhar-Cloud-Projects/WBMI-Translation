...
EXPTRANS AS (
	...
	TO_TIMESTAMP('1/1/1800', 'MM/DD/YYYY') AS DEFAULT_DATE
	FROM SQ_agency
),
...
EXP_DET_MRG_REGION AS (
	...
	--  
	IFF(
	    LKP_agency_uw_id IS NULL, 'N/A',
	    IFF(
	        LKP_insurance_line = 'D', in_uw_mgr_full_name, LKP_uw_mgr
	    )
...
EXP_EFF_TO_DATE AS (
	...
	IFF(eff_to_date IS NOT NULL, DATEADD(SECOND,- 1,eff_to_date)) AS out_eff_to_date,
