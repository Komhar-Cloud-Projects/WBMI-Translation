...
EXP_VALIDATE AS (
	...
	--                                                                                                                
	IFF(
	    LTRIM(RTRIM(tch_claim_nbr_ccd)) IS NULL
	    or LENGTH(LTRIM(RTRIM(tch_claim_nbr_ccd)))>0
	    and TRIM(LTRIM(RTRIM(tch_claim_nbr_ccd)))=''
	    or LENGTH(LTRIM(RTRIM(tch_claim_nbr_ccd))) = 0,
	    'N/A',
	...
	--                                                                                                              
	IFF(
	    LTRIM(RTRIM(tch_client_id_ccd)) IS NULL
	    or LENGTH(LTRIM(RTRIM(tch_client_id_ccd)))>0
	    and TRIM(LTRIM(RTRIM(tch_client_id_ccd)))=''
	    or LENGTH(LTRIM(RTRIM(tch_client_id_ccd))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_damage_cd)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_damage_cd)))>0
	    and TRIM(LTRIM(RTRIM(IN_damage_cd)))=''
	    or LENGTH(LTRIM(RTRIM(IN_damage_cd))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_damage_desc)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_damage_desc)))>0
	    and TRIM(LTRIM(RTRIM(IN_damage_desc)))=''
	    or LENGTH(LTRIM(RTRIM(IN_damage_desc))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_damage_type)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_damage_type)))>0
	    and TRIM(LTRIM(RTRIM(IN_damage_type)))=''
	    or LENGTH(LTRIM(RTRIM(IN_damage_type))) = 0,
	    'N/A',
...
EXP_DETECT_CHANGES AS (
	...
	-- 
	IFF(
	    old_claim_case_dam_det_id IS NULL, 'NEW',
	    IFF(
	        damage_amt <> old_dam_low_amt
	        or damage_high_amt <> old_dam_high_amt
	        or LTRIM(RTRIM(damage_desc)) <> LTRIM(RTRIM(old_dam_comment))
	        or LTRIM(RTRIM(damage_type)) <> LTRIM(RTRIM(old_dam_type_code)),
	        'UPDATE',
	        'NOCHANGE'
	    )
	) AS v_changed_flag,
...
EXP_Lag_eff_from_date AS (
	...
	DECODE(
	    TRUE,
	    claim_case_ak_id = v_prev_row_claim_case_ak_id and dam_seq_num = v_prev_row_dam_seq_num and dam_code = v_prev_row_dam_code, DATEADD(SECOND,- 1,v_prev_row_eff_from_date),
	    orig_eff_to_date
	) AS v_eff_to_date,
