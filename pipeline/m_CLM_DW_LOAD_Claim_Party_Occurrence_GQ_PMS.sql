...
EXP_Lkp_Values AS (
	...
	IFF(
	    LKP_CLAIM_OCCURRENCE_CLAIM_OCCURRENCE_KEY.claim_occurrence_ak_id IS NULL, 0,
	...
	-- 
	IFF(
	    (in_IPFCGQ_OFFSET_ONSET_IND IS NULL
	    or LENGTH(in_IPFCGQ_OFFSET_ONSET_IND)>0
	    and TRIM(in_IPFCGQ_OFFSET_ONSET_IND)=''
	    or LENGTH(in_IPFCGQ_OFFSET_ONSET_IND) = 0),
	    'N/A',
	    in_IPFCGQ_OFFSET_ONSET_IND
...
EXP_Detect_Changes AS (
	...
	IFF(
	    ...
	    IFF(
	        ...
	        or ltrim(rtrim(lkp_offset_onset_ind)) <> ltrim(rtrim(IPFCGQ_OFFSET_ONSET_IND)),
	        'UPDATE',
	        'NOCHANGE'
	    )
	) AS v_Changed_Flag,
