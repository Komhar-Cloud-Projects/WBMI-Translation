...
EXP_NumericValues AS (
	...
	IFF(i_SupProductId IS NULL,
		- 1,
		i_SupProductId
	) AS o_SupProductId,
	-- *INF*: IIF(ISNULL(i_SupProductAKId),-1,i_SupProductAKId)
	IFF(i_SupProductAKId IS NULL,
		- 1,
		i_SupProductAKId
	) AS o_SupProductAKId
	FROM SQ_SupProduct
...
EXP_StringValues AS (
	...
	IFF(i_SourceProductCode IS NULL 
		OR LENGTH(i_SourceProductCode
		) = 0 
		OR LENGTH(i_SourceProductCode)>0 AND TRIM(i_SourceProductCode)='',
		'N/A',
