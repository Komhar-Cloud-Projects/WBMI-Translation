...
EXP_Target AS (
	...
	IFF(
	    lkp_pol_exp_date > DATEADD(MONTH,- 1,v_RunDate) AND lkp_pol_exp_date <= v_RunDate,
	...
	IFF(
	    lkp_PolicyStatusDescription = 'Cancelled'
	    and DATE_PART(lkp_PolicyCancellationDate, 'MM') = DATE_PART(v_RunDate, 'MM'),
	    v_DistinctPolicyKeyCounter,
	    NULL
	) AS o_CancelledPolicyCounter,
	-- *INF*: IIF(TransactionInforceFlag='Y' AND lkp_pol_issue_code='N' ,v_DistinctPolicyKeyCounter,NULL)
	IFF(
	    TransactionInforceFlag = 'Y' AND lkp_pol_issue_code = 'N', v_DistinctPolicyKeyCounter, NULL
	...
	-- )
	IFF(
	    ((CAST(TRUNC(lkp_pol_eff_date, 'MONTH') AS TIMESTAMP_NTZ(0)) <= CAST(TRUNC(v_RunDate, 'MONTH') AS TIMESTAMP_NTZ(0))
