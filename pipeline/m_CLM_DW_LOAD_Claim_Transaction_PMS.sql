...
EXP_CLAIMS_TRANSACTION_DEFAULT AS (
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_INSURANCE_LINE)>0
	    and TRIM(IN_LOSS_INSURANCE_LINE)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_INSURANCE_LINE)>0
	    and TRIM(IN_LOSS_INSURANCE_LINE)='', '0',
	        LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE))
	    )
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_RISK_UNIT_GROUP)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_LOSS_RISK_UNIT_GROUP)>0
	    and TRIM(IN_LOSS_RISK_UNIT_GROUP)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_UNIT)) IS NULL, '0',
	    IFF(
	        LENGTH(IN_LOSS_UNIT)>0 AND TRIM(IN_LOSS_UNIT)='', '0', LTRIM(RTRIM(IN_LOSS_UNIT))
	    )
	) AS LOSS_UNIT_lkp,
	...
	IFF(
	    LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE)) IS NULL, '18000101',
	...
	IFF(
	    IN_LOSS_TRANSACTION_DATE IS NULL,
	    TO_TIMESTAMP('01/01/1800 00:00:00', 'MM/DD/YYYY HH24:MI:SS'),
	    TO_TIMESTAMP(IN_LOSS_TRANSACTION_DATE, 'YYYYMMDD')
...
EXP_Claim_Transaction_Detect_Changes AS (
	...
	IFF(
	    IN_LOSS_TRANSACTION IN ('90','91','92','95','97','98','99'), 0.0, IN_LOSS_PAID_OR_RESV_AMT
	...
	-- 
	-- 
	IFF(IN_LOSS_TRANSACTION IN ('91','92'), '90', IN_LOSS_TRANSACTION) AS LOSS_TRANSACTION,
	...
	TO_TIMESTAMP(LOSS_TRANSACTION_DATE, 'YYYYMMDD') AS V_LOSS_TRANSACTION_DATE,
	...
	IFF(
	    ...
	    TO_TIMESTAMP(v_reprocess_date, 'MMDDYYYY')
	...
	IFF(
	    LKP_adnm_taxid_ssn IS NULL
	    or length(LKP_adnm_taxid_ssn) = 0
	    or LENGTH(LKP_adnm_taxid_ssn)>0
	    and TRIM(LKP_adnm_taxid_ssn)='',
	    '000000000',
	...
	IFF(
	    V_TRIM_LKP_TAXID_SSN != '000000000',
	    IFF(
	        SUBSTR(V_TRIM_LKP_TAXID_SSN, 3, 1) = '-'
	        and REGEXP_INSTR(V_TRIM_LKP_TAXID_SSN, ' ') = 0
	        and LENGTH(V_TRIM_LKP_TAXID_SSN) = 10,
	        V_TRIM_LKP_TAXID_SSN,
	        IFF(
	            SUBSTR(V_TRIM_LKP_TAXID_SSN, 4, 1) = '-'
	            and SUBSTR(V_TRIM_LKP_TAXID_SSN, 7, 1) = '-'
	            and REGEXP_INSTR(V_TRIM_LKP_TAXID_SSN, ' ') = 0
	            and LENGTH(V_TRIM_LKP_TAXID_SSN) = 11,
	            V_TRIM_LKP_TAXID_SSN,
	            '000000000'
	        )
	    ),
	...
	-- *INF*: TO_DATE('01/01/1800 01:00:00','MM/DD/YYYY HH24:MI:SS')
	TO_TIMESTAMP('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS') AS s3p_created_date,
...
EXP_Evaluate AS (
	...
	IFF(
	    LOSS_TRANSACTION IN ('27','40','83','84') AND inf_action = '4',
