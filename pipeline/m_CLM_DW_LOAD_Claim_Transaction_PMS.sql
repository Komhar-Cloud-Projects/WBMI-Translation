...
EXP_CLAIMS_TRANSACTION_DEFAULT AS (
	...
	IFF(LTRIM(RTRIM(IN_LOSS_INSURANCE_LINE
		...
		'N/A',
		IFF(LENGTH(IN_LOSS_INSURANCE_LINE)>0 AND TRIM(IN_LOSS_INSURANCE_LINE)='',
			'N/A',
	...
	IFF(IN_LOSS_DRAFT_AMOUNT IS NULL,
		0,
		IN_LOSS_DRAFT_AMOUNT
	) AS LOSS_DRAFT_AMOUNT,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE))),'18000101',LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE)))
	IFF(LTRIM(RTRIM(IN_LOSS_TRANSACTION_DATE
			)
		) IS NULL,
		'18000101',
	...
	IFF(IN_LOSS_TRANSACTION_DATE IS NULL,
		TO_DATE('01/01/1800 00:00:00', 'MM/DD/YYYY HH24:MI:SS'
		),
		to_date(IN_LOSS_TRANSACTION_DATE, 'YYYYMMDD'
		)
	) AS LOSS_TRANSACTION_DATE_payment,
	...
	LTRIM(RTRIM(IN_LOSS_DRAFT_PAY_TO_1
		)
	) || LTRIM(RTRIM(IN_LOSS_DRAFT_PAY_TO_2
		)
	) || LTRIM(RTRIM(IN_LOSS_DRAFT_PAY_TO_3
		)
	) AS v_pay_to_code,
	...
	IFF(IN_LOSS_DRAFT_MAIL_TO IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_LOSS_DRAFT_MAIL_TO
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_LOSS_DRAFT_MAIL_TO
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_LOSS_DRAFT_MAIL_TO
				)
...
EXP_Claim_Transaction_Detect_Changes AS (
	...
	IFF(LOSS_SUB_LOCATION_NUMBER = 0,
		'0',
	...
	IFF(LOSS_NET_CHANGE_DOLLARS = 0.0,
		'39',
		'38'
	...
	IFF(IN_LOSS_TRANSACTION IN ('90','91','92','95','97','98','99'),
		0.0,
		IN_LOSS_PAID_OR_RESV_AMT
	) AS LOSS_PAID_OR_RESV_AMT,
	-- *INF*: IIF(IN_LOSS_PAID_OR_RESV_AMT=0.0,'40','30')
	IFF(IN_LOSS_PAID_OR_RESV_AMT = 0.0,
		'40',
		'30'
	...
	-- 
	-- 
	IFF(IN_LOSS_TRANSACTION IN ('91','92'),
		'90',
		IN_LOSS_TRANSACTION
	) AS LOSS_TRANSACTION,
	...
	DECODE(IN_LOSS_TRANSACTION,
		...
		'99', '84',
		IN_LOSS_TRANSACTION
	) AS LOSS_TRANSACTION_83_84_40_27,
	...
	TO_DATE(LOSS_TRANSACTION_DATE, 'YYYYMMDD'
	...
	v_ipfcx6_month_process || v_ipfcx6_day_process || to_char(ipfcx6_year_process
	) AS v_reprocess_date,
	...
	IFF(LKP_adnm_taxid_ssn IS NULL 
		OR length(LKP_adnm_taxid_ssn
		) = 0 
		OR LENGTH(LKP_adnm_taxid_ssn)>0 AND TRIM(LKP_adnm_taxid_ssn)='',
		'000000000',
		ltrim(rtrim(LKP_adnm_taxid_ssn
			)
		)
	) AS V_LKP_adnm_taxid_ssn,
	-- *INF*: LTRIM(RTRIM(V_LKP_adnm_taxid_ssn))
	LTRIM(RTRIM(V_LKP_adnm_taxid_ssn
		)
	) AS V_TRIM_LKP_TAXID_SSN,
	...
	IFF(V_TRIM_LKP_TAXID_SSN != '000000000',
		IFF(SUBSTR(V_TRIM_LKP_TAXID_SSN, 3, 1
			) = '-' 
			AND REGEXP_INSTR(V_TRIM_LKP_TAXID_SSN, ' '
			) = 0 
			AND LENGTH(V_TRIM_LKP_TAXID_SSN
			) = 10,
			V_TRIM_LKP_TAXID_SSN,
			IFF(SUBSTR(V_TRIM_LKP_TAXID_SSN, 4, 1
				) = '-' 
				AND SUBSTR(V_TRIM_LKP_TAXID_SSN, 7, 1
				) = '-' 
				AND REGEXP_INSTR(V_TRIM_LKP_TAXID_SSN, ' '
				) = 0 
				AND LENGTH(V_TRIM_LKP_TAXID_SSN
...
EXP_Evaluate AS (
	...
	IFF(LOSS_TRANSACTION IN ('27','40','83','84') 
		AND inf_action = '4',
		claim_trans_id_lkp_83_84_27_40,
		LKP_claim_trans_id
	) AS V_Claim_Trans_ID,
	...
	IFF(CLAIMANT_COV_DET_AK_ID_OP IS NULL,
		- 1,
		CLAIMANT_COV_DET_AK_ID_OP
	) AS o_CLAIMANT_COV_DET_AK_ID_OP,
	...
	) AS o_SupTransactionCategoryCode
