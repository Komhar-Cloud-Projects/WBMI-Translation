...
EXP_Src_Values AS (
	...
	IFF(
	    ltrim(rtrim(referral_date)) IS NULL, TO_TIMESTAMP('1/1/1800', 'MM/DD/YYYY'), referral_date
	) AS o_referral_date
...
EXP_TargetLkp_Detect_Changes AS (
	...
	--    )
	IFF(
	    Lkp_NurseReferralId IS NULL, 'NEW',
	    IFF(
	        ltrim(rtrim(Lkp_NurseCaseAkId)) != ltrim(rtrim(NurseCaseAkId))
	        or ltrim(rtrim(Lkp_claim_party_ak_id)) != ltrim(rtrim(claim_party_ak_id))
	        or ltrim(rtrim(LKp_nurse_referral_id)) != ltrim(rtrim(nurse_referral_id))
	        or ltrim(rtrim(Lkp_ReferralDate)) != ltrim(rtrim(ReferralDate)),
	        'UPDATE',
	        'NOCHANGE'
	    )
	) AS v_ChangedFlag,
