...
EXP_DefaultValues AS (
	...
	IFF(i_SupStrategicProfitCenterInsuranceSegmentId IS NULL, - 1, i_SupStrategicProfitCenterInsuranceSegmentId) AS o_SupStrategicProfitCenterInsuranceSegmentId,
	...
	IFF(i_SupStrategicProfitCenterInsuranceSegmentAKId IS NULL, - 1, i_SupStrategicProfitCenterInsuranceSegmentAKId) AS o_SupStrategicProfitCenterInsuranceSegmentAKId,
	...
	IFF(i_PolicyNumber1 IS NULL OR LENGTH(i_PolicyNumber1) = 0 OR IS_SPACES(i_PolicyNumber1), 'N/A', LTRIM(RTRIM(i_PolicyNumber1))) AS o_PolicyNumber1,
	-- *INF*: IIF(ISNULL(i_PolicySymbol1) OR LENGTH(i_PolicySymbol1)=0 OR IS_SPACES(i_PolicySymbol1),'N/A',LTRIM(RTRIM(i_PolicySymbol1)))
	IFF(i_PolicySymbol1 IS NULL OR LENGTH(i_PolicySymbol1) = 0 OR IS_SPACES(i_PolicySymbol1), 'N/A', LTRIM(RTRIM(i_PolicySymbol1))) AS o_PolicySymbol1,
	-- *INF*: IIF(ISNULL(i_Division) OR LENGTH(i_Division)=0 OR IS_SPACES(i_Division),'N/A',LTRIM(RTRIM(i_Division)))
	IFF(i_Division IS NULL OR LENGTH(i_Division) = 0 OR IS_SPACES(i_Division), 'N/A', LTRIM(RTRIM(i_Division))) AS o_Division,
