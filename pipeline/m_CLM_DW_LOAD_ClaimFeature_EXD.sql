...
EXP_Validate_Source_Values AS (
	...
	UDF_DEFAULT_VALUE_FOR_STRINGS(in_Adjuster_Client_Id) AS out_Adjuster_Client_Id,
	Created_TS,
	Modified_TS,
	SourceSystemId
...
EXP_Lookup_Values AS (
	...
	UDF_DEFAULT_VALUE_FOR_STRINGS(SUBSTR(in_Bur_Cause_Loss, 1, 2)) AS CauseOfLoss,
	-- *INF*: :UDF.DEFAULT_VALUE_FOR_STRINGS(SUBSTR(in_Bur_Cause_Loss,3,1))
	UDF_DEFAULT_VALUE_FOR_STRINGS(SUBSTR(in_Bur_Cause_Loss, 3, 1)) AS ReserveCategory,
	...
	DECODE(
	    TRUE,
	    in_FeatureRepresentativeAkId IS NULL, TO_TIMESTAMP('01/01/1800 00:00:00', 'MM/DD/YYYY HH24:MI:SS'),
...
EXP_Detect_Changes AS (
	...
	-- 		'NOCHANGE'))
	IFF(
	    lkp_ClaimFeatureAKId IS NULL, 'NEW',
	    IFF(
	        lkp_ClaimRepresentativeAkId != FeatureRepresentativeAkId, 'UPDATE', 'NOCHANGE'
	    )
	) AS v_ChangedFlag,
...
EXP_Insert_Target AS (
	...
	-- 	SYSDATE)
	IFF(
	    ChangedFlag = 'NEW', TO_TIMESTAMP('01/01/1800 01:00:00', 'MM/DD/YYYY HH24:MI:SS'),
	    CURRENT_TIMESTAMP
	) AS EffectiveDate,
