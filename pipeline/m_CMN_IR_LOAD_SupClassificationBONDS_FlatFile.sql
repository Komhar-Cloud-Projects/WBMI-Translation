...
EXP_Detect_Changes AS (
	...
	IFF(LTRIM(RTRIM(i_EffectiveDate
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(i_EffectiveDate
			)
		))>0 AND TRIM(LTRIM(RTRIM(i_EffectiveDate
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(i_EffectiveDate
				)
		...
		OR LTRIM(RTRIM(SUBSTR(i_EffectiveDate, 1, 10
				)
			)
		) = '1900-01-01',
		TO_DATE('1800-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS'
	...
	IFF(LTRIM(RTRIM(i_ExpirationDate
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(i_ExpirationDate
			)
		))>0 AND TRIM(LTRIM(RTRIM(i_ExpirationDate
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(i_ExpirationDate
				)
		...
		OR LTRIM(RTRIM(SUBSTR(i_ExpirationDate, 1, 10
				)
			)
		) = '2999-01-01',
		TO_DATE('2100-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS'
