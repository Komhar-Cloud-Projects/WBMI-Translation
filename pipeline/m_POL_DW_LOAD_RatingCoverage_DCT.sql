...
EXP_PolicyKey AS (
	...
	IFF(
	    i_PolicyEffectiveDate IS NULL, TO_TIMESTAMP('21001231235959', 'YYYYMMDDHH24MISS'),
...
EXP_GetValues AS (
	...
	IFF(
	    i_PEffectiveDate IS NULL, TO_TIMESTAMP('21001231235959', 'YYYYMMDDHH24MISS'),
	    i_PEffectiveDate
	) AS v_PEffectiveDate,
	-- *INF*: IIF(ISNULL(i_PExpirationDate),TO_DATE('21001231235959' , 'YYYYMMDDHH24MISS'),i_PExpirationDate)
	IFF(
	    i_PExpirationDate IS NULL, TO_TIMESTAMP('21001231235959', 'YYYYMMDDHH24MISS'),
	    i_PExpirationDate
	...
	IFF(
	    i_RiskType IS NULL OR LENGTH(i_RiskType)>0 AND TRIM(i_RiskType)='' OR LENGTH(i_RiskType) = 0,
	    'N/A',
	...
	DECODE(
	    ...
	    i_LineType = 'CommercialAuto' and LTRIM(RTRIM(i_ParentCoverageObjectName)) = 'DC_CA_State' and REGEXP_INSTR(i_CoverageType, 'HiredAndBorrowed', 1, 1) > 0, '6625',
	    i_LineType = 'CommercialAuto' and LTRIM(RTRIM(i_ParentCoverageObjectName)) = 'DC_CA_State' and REGEXP_INSTR(i_CoverageType, 'DriveOtherCar', 1, 1) > 0, LKP_DCCLASSCODESTAGING_CA_DRIVEOTHERCAR_i_ParentCoverageObjectId_i_SessionId.Value,
	    i_LineType = 'CommercialAuto' and LTRIM(RTRIM(i_ParentCoverageObjectName)) = 'DC_Line' and REGEXP_INSTR(i_CoverageType, 'NonOwned', 1, 1) > 0, LKP_DCCLASSCODESTAGING_CA_NONOWNED_i_ParentCoverageObjectId_i_SessionId.Value,
	    ...
	    i_LineType = 'CommercialAuto' and LTRIM(RTRIM(i_ParentCoverageObjectName)) = 'DC_CA_State' and REGEXP_INSTR(i_CoverageType, 'MotorJunkLicense', 1, 1) > 0, LKP_DCCLASSCODESTAGING_CA_MOTORJUNKLICENSE_i_ParentCoverageObjectId_i_SessionId.Value,
	...
	DECODE(
	    ...
	    LENGTH(v_ClassCode_lkp) = 0, 'N/A',
	    REGEXP_INSTR(i_CoverageType, 'NonOwned', 1, 1) > 0 AND REGEXP_INSTR(v_ClassCode_lkp, ',', 1, 1) > 0, SUBSTR(v_ClassCode_lkp, REGEXP_INSTR(v_ClassCode_lkp, ',', 1, 1) + 1),
	    NOT REGEXP_INSTR(i_CoverageType, 'NonOwned', 1, 1) > 0 AND REGEXP_INSTR(v_ClassCode_lkp, ',', 1, 1) > 0, SUBSTR(v_ClassCode_lkp, 1, REGEXP_INSTR(v_ClassCode_lkp, ',', 1, 1) - 1),
	    v_ClassCode_lkp
	) AS v_ClassCode,
	...
	IFF(
	    v_SubLineCode_lkp IS NULL
	    or LENGTH(v_SubLineCode_lkp)>0
	    and TRIM(v_SubLineCode_lkp)=''
	    or LENGTH(v_SubLineCode_lkp) = 0,
	    'N/A',
	...
	IFF(
	    i_ParentCoverageType IS NULL OR LENGTH(LTRIM(RTRIM(i_ParentCoverageType))) = 0, 'N/A',
	...
	-- 
	IFF(
	    i_LineType = 'WorkersCompensation' AND ltrim(rtrim(v_ClassCode)) != 'N/A',
	    LPAD(LTRIM(RTRIM(v_ClassCode)), 4, '0'),
	...
	IFF(
	    v_TType = 'Cancel', v_TCancellationDate, TO_TIMESTAMP('21001231235959', 'YYYYMMDDHH24MISS')
	...
	IFF(
	    i_SpecialClassLevel1 IS NULL
	    or LENGTH(i_SpecialClassLevel1)>0
	    and TRIM(i_SpecialClassLevel1)=''
	    or LENGTH(i_SpecialClassLevel1) = 0,
	    'N/A',
	...
	IFF(
	    (LOWER(i_LineType) IN ('property','sbopproperty')
	    or LTRIM(RTRIM(i_WBProduct)) = 'SMARTbusiness')
	...
	IFF(
	    i_PolicyCoverage IS NULL
	    or LENGTH(i_PolicyCoverage)>0
	    and TRIM(i_PolicyCoverage)=''
	    or LENGTH(i_PolicyCoverage) = 0,
	    'N/A',
	...
	IFF(
	    i_PerilGroup IS NULL
	    or LENGTH(i_PerilGroup)>0
	    and TRIM(i_PerilGroup)=''
	    or LENGTH(i_PerilGroup) = 0,
	    'N/A',
	...
	DECODE(
	    TRUE,
	    v_LineType = 'BusinessOwners' and LTRIM(RTRIM(i_WBProduct)) = 'SMARTbusiness' and LTRIM(RTRIM(i_Division)) = 'CommercialLines' and REGEXP_INSTR(v_PType, 'Spoilage') > 0, 'Spoilage',
	    (i_WBProduct = 'SMARTbusiness' and v_LineType = 'BusinessOwners') Or (v_PType IN ('EquipmentBreakdown','MineSubsidence','DataCompromise','CyberComputerAttack','CyberExtendedReportingPeriod','CyberExtortionExpenses','CyberNetworkSecurity','CyberSuite','EmploymentPracticesLiability','WB516','WB516CA','WB516CANC','WB516GL','NS0279','NS0313','ActsErrorsOmissionsLiabilityCoverages','WB1429','WB2086','WB1144','WB1430','WB1146','NS0320','WB2525','WB2525NC','WB2499','WB2216','WB1372','NS0453','NS0453NC','NS0321','CA2562','CA2564','CA2565')), v_PType,
	    'N/A'
	...
	IFF(
	    i_SubCoverageType IS NULL
	    and NOT LENGTH(i_SubCoverageType) = 0
	    and NOT LENGTH(i_SubCoverageType)>0
	    and TRIM(i_SubCoverageTypeNOT )='',
	    i_SubCoverageType,
	    'N/A'
	...
	IFF(
	    i_OccupancyClassDescription IS NULL
	    or LENGTH(i_OccupancyClassDescription)>0
	    and TRIM(i_OccupancyClassDescription)=''
	    or LENGTH(i_OccupancyClassDescription) = 0,
	    'N/A',
...
EXP_CalPolicyCoverageHashKey AS (
	...
	IFF(
	    i_StandardSpecialClassGroupCode IS NULL, 'N/A',
...
EXP_RatingCoverageHashKey AS (
	...
	DECODE(
	    ...
	    CoverageForm = 'AutoDealers' AND i_InsuranceReferenceLineOfBusinessCode = '360' AND i_ProductCode = '200', '360',
	    CoverageForm = 'AutoDealers', '341',
	    i_InsuranceReferenceLineOfBusinessCode = '200' AND i_ProductCode = '200' AND i_CoverageSummaryDescription IS NOT NULL, '340',
	    i_InsuranceReferenceLineOfBusinessCode
	...
	DECODE(
	    TRUE,
	    CoverageForm = 'AutoDealers', '341',
	    i_ProductCode = '200' AND i_CoverageSummaryDescription IS NOT NULL, '340',
	    i_ProductCode
	) AS o_ProductCode,
...
EXP_CoverageCancellationDate AS (
	...
	IFF(
	    v_RatingCoverageCancellationFlag = 1, LEAST(TEffectiveDate, i_TCancellationDate),
	    TO_TIMESTAMP('21001231235959', 'YYYYMMDDHH24MISS')
	) AS o_RatingCoverageCancellationDate,
	-- *INF*: DECODE(TRUE,
...
EXP_CalValues AS (
	...
	IFF(
	    PolicyAKID = v_prev_PolicyAKID and i_CoverageGUID = v_prev_CoverageGUID, v_prev_NEXTVAL,
	    i_NEXTVAL
	) AS v_NEXTVAL,
	...
	IFF(
	    v_ChangeFlag = 'New' AND lkp_RatingCoverageAKID IS NULL, v_NEXTVAL, lkp_RatingCoverageAKID
	) AS o_RatingCoverageAKID,
	...
	DECODE(
	    TRUE,
	    v_Seq = 1 AND lkp_RatingCoverageAKID IS NOT NULL, lkp_RatingCoverageEffectivedate,
	...
	DECODE(
	    TRUE,
	    v_Seq = 1 AND lkp_RatingCoverageAKID IS NOT NULL, lkp_RatingCoverageExpirationdate,
	...
	-- 
	IFF(
	    v_RatingCoverageExpirationDate IS NULL, i_PExpirationDate, v_RatingCoverageExpirationDate
	) AS o_RatingCoverageExpirationDate
