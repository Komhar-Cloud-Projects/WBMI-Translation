...
AGG_Remove_Duplicates AS (
	...
	IFF(
	    i_DriversLicenseNumber IS NULL
	    or LENGTH(i_DriversLicenseNumber)>0
	    and TRIM(i_DriversLicenseNumber)=''
	    or LENGTH(i_DriversLicenseNumber) = 0,
	    'N/A',
	...
	IFF(
	    i_StateLicensed IS NULL
	    or LENGTH(i_StateLicensed)>0
	    and TRIM(i_StateLicensed)=''
	    or LENGTH(i_StateLicensed) = 0,
	    'N/A',
...
EXP_Pre_Target_Lookup AS (
	...
	IFF(
	    LastName IS NULL or LENGTH(LastName)>0 AND TRIM(LastName)='' or LENGTH(LastName) = 0, 'N/A',
	    LTRIM(RTRIM(LastName))
	) AS o_LastName,
	-- *INF*: IIF(ISNULL(FirstName) or IS_SPACES(FirstName)  or LENGTH(FirstName)=0,'N/A',LTRIM(RTRIM(FirstName)))
	IFF(
	    FirstName IS NULL or LENGTH(FirstName)>0 AND TRIM(FirstName)='' or LENGTH(FirstName) = 0,
	    'N/A',
	...
	IFF(
	    ...
	    or LENGTH(MiddleInitial)>0
	    and TRIM(MiddleInitial)=''
	    or LENGTH(MiddleInitial) = 0,
	    'N/A',
	...
	IFF(
	    ExcludeDriver IS NULL
	    or LENGTH(ExcludeDriver)>0
	    and TRIM(ExcludeDriver)=''
	    or LENGTH(ExcludeDriver) = 0,
	    'N/A',
	...
	IFF(
	    MVRStatus IS NULL or LENGTH(MVRStatus)>0 AND TRIM(MVRStatus)='' or LENGTH(MVRStatus) = 0,
	    'N/A',
...
EXP_Detect_Changes AS (
	...
	-- 'UPDATE', 'NOCHANGE'))
	IFF(
	    lkp_DriverAKId IS NULL, 'NEW',
	    IFF(
	        LTRIM(RTRIM(lkp_LastName)) != LTRIM(RTRIM(i_LastName))
	        or LTRIM(RTRIM(lkp_FirstName)) != LTRIM(RTRIM(i_FirstName))
	        or LTRIM(RTRIM(lkp_MiddleName)) != LTRIM(RTRIM(i_MiddleInitial))
	        or lkp_Birthdate != i_DateOfBirth
	        or LTRIM(RTRIM(lkp_GenderCode)) != LTRIM(RTRIM(i_Gender))
	        or LTRIM(RTRIM(lkp_ExcludeDriver)) != LTRIM(RTRIM(i_ExcludeDriver))
	        or LTRIM(RTRIM(lkp_MVRStatus)) != LTRIM(RTRIM(i_MVRStatus)),
	        'UPDATE',
	        'NOCHANGE'
	    )
	) AS v_Changed_Flag,
