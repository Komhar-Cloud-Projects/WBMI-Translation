...
SQ_PremiumMonthlySummaryFact AS (
	select P.edw_pol_ak_id,
	PMSF.PremiumMonthlySummaryFactId,
	...
	inner Join @{pipeline().parameters.SOURCE_TABLE_OWNER}.PremiumMonthlySummaryFact PMSF
	on P.pol_dim_id=PMSF.PolicyDimId
	inner join @{pipeline().parameters.SOURCE_TABLE_OWNER}.InsuranceReferenceDim IRD
	on PMSF.InsuranceReferenceDimId=IRD.InsuranceReferenceDimId
	where P.edw_pol_ak_id in (SELECT
			P.edw_pol_ak_id
		FROM policy_dim P
		INNER JOIN PremiumMonthlySummaryFact PTCTF
	...
	@{pipeline().parameters.WHERE_CLAUSE_PMSF}
	order by 1
),
EXP_PremiumMonthlySummaryFact AS (
	SELECT
	EDW_Pol_AK_id,
	...
	ProductCode,
	InsuranceReferenceLineOfBusinessCode
	FROM SQ_PremiumMonthlySummaryFact
),
LKP_RatingPlan_DCT AS (
	...
	FROM (
		...
		and P.pol_ak_id in (SELECT
			...
			INNER JOIN @{pipeline().parameters.WC_DATA_MART_DATABASE_NAME}.DBO.PremiumMonthlySummaryFact PTCTF
				ON P.pol_dim_id = PTCTF.PolicyDimId
...
EXP_RatingPlanDerivation AS (
	...
	ON LKP_RatingPlan_DCT.PolicyAKID = EXP_PremiumMonthlySummaryFact.EDW_Pol_AK_id
	LEFT JOIN LKP_RatingPlan_PMS
	ON LKP_RatingPlan_PMS.PolicyAKID = EXP_PremiumMonthlySummaryFact.EDW_Pol_AK_id
),
EXP_PremMthlySumFact_JNR AS (
	SELECT
	EXP_PremiumMonthlySummaryFact.PremiumMonthlySummaryFactId,
	EXP_PremiumMonthlySummaryFact.EnterpriseGroupCode,
	EXP_PremiumMonthlySummaryFact.InsuranceReferenceLegalEntityCode,
	EXP_PremiumMonthlySummaryFact.StrategicProfitCenterCode,
	EXP_PremiumMonthlySummaryFact.InsuranceSegmentCode,
	EXP_PremiumMonthlySummaryFact.PolicyOfferingCode,
	EXP_PremiumMonthlySummaryFact.ProductCode,
	EXP_PremiumMonthlySummaryFact.InsuranceReferenceLineOfBusinessCode,
	...
	FROM EXP_PremiumMonthlySummaryFact
...
PremiumMonthlySummaryFact AS (
	MERGE INTO PremiumMonthlySummaryFact AS T
	USING UPD_PMSF AS S
