...
EXP_LkpValues AS (
	...
	IFF(
	    IN_uw_assistant_flag IS NULL, 'X',
	    IFF(
	        LENGTH(IN_uw_assistant_flag)>0
	    and TRIM(IN_uw_assistant_flag)='', 'X',
	...
	IFF(
	    IN_insurance_line IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_insurance_line)>0
	    and TRIM(IN_insurance_line)='', 'N/A',
	        rpad(IN_insurance_line, 3)
	    )
	) AS insurance_line,
	...
	IFF(
	    IN_in_uw_code IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_in_uw_code)>0 AND TRIM(IN_in_uw_code)='', 'N/A', IN_in_uw_code
	    )
	) AS in_uw_code,
...
EXP_DetectChanges AS (
	...
	-- 
	IFF(
	    agency_uw_id_old IS NULL, 'NEW',
	    IFF(
	        (uw_ak_id_old <> out_uw_ak_id), 'UPDATE', 'NOCHANGE'
	    )
	) AS v_changed_flag,
...
EXP_Lag_eff_from_date AS (
	...
	DECODE(
	    TRUE,
	    insurance_line = v_Prev_Row_insurance_line and uw_assistant_flag = v_Prev_Row_uw_assistant_flag and agency_ak_id = v_Prev_Row_agency_ak_id, DATEADD(SECOND,- 1,v_Prev_Row_Eff_From_Date),
	    orig_eff_to_date
	) AS v_eff_to_date,
