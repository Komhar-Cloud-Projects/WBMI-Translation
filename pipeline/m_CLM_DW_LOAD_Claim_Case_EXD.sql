...
EXP_VALIDATE AS (
	...
	IFF(
	    LTRIM(RTRIM(tch_claim_nbr_ccm)) IS NULL
	    or LENGTH(LTRIM(RTRIM(tch_claim_nbr_ccm)))>0
	    and TRIM(LTRIM(RTRIM(tch_claim_nbr_ccm)))=''
	    or LENGTH(LTRIM(RTRIM(tch_claim_nbr_ccm))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(tch_client_id_ccm)) IS NULL
	    or LENGTH(LTRIM(RTRIM(tch_client_id_ccm)))>0
	    and TRIM(LTRIM(RTRIM(tch_client_id_ccm)))=''
	    or LENGTH(LTRIM(RTRIM(tch_client_id_ccm))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_case_name)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_case_name)))>0
	    and TRIM(LTRIM(RTRIM(IN_case_name)))=''
	    or LENGTH(LTRIM(RTRIM(IN_case_name))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_case_number)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_case_number)))>0
	    and TRIM(LTRIM(RTRIM(IN_case_number)))=''
	    or LENGTH(LTRIM(RTRIM(IN_case_number))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_suit_venue)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_suit_venue)))>0
	    and TRIM(LTRIM(RTRIM(IN_suit_venue)))=''
	    or LENGTH(LTRIM(RTRIM(IN_suit_venue))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_suit_state)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_suit_state)))>0
	    and TRIM(LTRIM(RTRIM(IN_suit_state)))=''
	    or LENGTH(LTRIM(RTRIM(IN_suit_state))) = 0,
	    'N/A',
	...
	IFF(
	    IN_trial_date IS NULL, TO_TIMESTAMP('01/01/1800 00:00:00', 'MM/DD/YYYY HH24:MI:SS'),
	    IN_trial_date
	) AS trial_date,
	...
	IFF(
	    LTRIM(RTRIM(IN_first_not_law_suit)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_first_not_law_suit)))>0
	    and TRIM(LTRIM(RTRIM(IN_first_not_law_suit)))=''
	    or LENGTH(LTRIM(RTRIM(IN_first_not_law_suit))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_declaratory_act)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_declaratory_act)))>0
	    and TRIM(LTRIM(RTRIM(IN_declaratory_act)))=''
	    or LENGTH(LTRIM(RTRIM(IN_declaratory_act))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_suit_status)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_suit_status)))>0
	    and TRIM(LTRIM(RTRIM(IN_suit_status)))=''
	    or LENGTH(LTRIM(RTRIM(IN_suit_status))) = 0,
	    'N/A',
	...
	IFF(
	    IN_denial_date IS NULL, TO_TIMESTAMP('01/01/1800 00:00:00', 'MM/DD/YYYY HH24:MI:SS'),
	    IN_denial_date
	) AS denial_date,
	...
	IFF(
	    IN_litigation_date IS NULL, TO_TIMESTAMP('01/01/1800 00:00:00', 'MM/DD/YYYY HH24:MI:SS'),
	    IN_litigation_date
	) AS litigation_date,
	...
	IFF(
	    IN_litigation_closed IS NULL, TO_TIMESTAMP('01/01/1800 00:00:00', 'MM/DD/YYYY HH24:MI:SS'),
	...
	IFF(
	    LTRIM(RTRIM(IN_how_clm_closed)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_how_clm_closed)))>0
	    and TRIM(LTRIM(RTRIM(IN_how_clm_closed)))=''
	    or LENGTH(LTRIM(RTRIM(IN_how_clm_closed))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_reins_reported)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_reins_reported)))>0
	    and TRIM(LTRIM(RTRIM(IN_reins_reported)))=''
	    or LENGTH(LTRIM(RTRIM(IN_reins_reported))) = 0,
	    'N/A',
...
EXP_LKP_VALUES AS (
	...
	IFF(
	    IN_prim_lit_handler IS NULL
	    or LENGTH(IN_prim_lit_handler)>0
	    and TRIM(IN_prim_lit_handler)=''
	    or LENGTH(IN_prim_lit_handler) = 0,
	    'N/A',
	    IFF(
	        caj_emp_client_id IS NOT NULL,
	        IFF(
	            caj_adjuster_class = 'L', 'STF', 'ADJ'
	        ),
	        IFF(
	            cre_client_id IS NULL, 'REG', 'ARB'
	        )
	    )
	...
	DECODE(
	    TRUE,
	    v_pol_lim_comment IS NULL OR LENGTH(v_pol_lim_comment)>0 AND TRIM(v_pol_lim_comment)='' OR LENGTH(v_pol_lim_comment) = 0, 'N/A',
	    v_total_comments_length <= 7000, v_pol_lim_comment,
	    v_total_comments_length > 7000, SUBSTR(v_pol_lim_comment, 1, length(v_pol_lim_comment) - (FLOOR(v_extra_comments_length * pol_lmt_comment_ratio)) - 35) || '...Rest of the comments removed...'
	) AS pol_lim_comment,
	-- *INF*: DECODE (TRUE,
	-- 
	...
	DECODE(
	    TRUE,
	    v_claim_case_comment IS NULL OR LENGTH(v_claim_case_comment)>0 AND TRIM(v_claim_case_comment)='' OR LENGTH(v_claim_case_comment) = 0, 'N/A',
	    v_total_comments_length <= 7000, v_claim_case_comment,
	    v_total_comments_length > 7000, SUBSTR(v_claim_case_comment, 1, length(v_claim_case_comment) - (FLOOR(v_extra_comments_length * claim_case_comment_ratio)) - 35) || '...Rest of the comments removed...'
	...
	DECODE(
	    TRUE,
	    v_injury_dam_comment IS NULL OR LENGTH(v_injury_dam_comment)>0 AND TRIM(v_injury_dam_comment)='' OR LENGTH(v_injury_dam_comment) = 0, 'N/A',
	    v_total_comments_length <= 7000, v_injury_dam_comment,
	    v_total_comments_length > 7000, SUBSTR(v_injury_dam_comment, 1, length(v_injury_dam_comment) - (FLOOR(v_extra_comments_length * inj_dam_comment_ratio)) - 35) || '...Rest of the comments removed...'
	...
	DECODE(
	    TRUE,
	    v_subrogation_contri_comment IS NULL OR LENGTH(v_subrogation_contri_comment)>0 AND TRIM(v_subrogation_contri_comment)='' OR LENGTH(v_subrogation_contri_comment) = 0, 'N/A',
	    v_total_comments_length <= 7000, v_subrogation_contri_comment,
	    v_total_comments_length > 7000, SUBSTR(v_subrogation_contri_comment, 1, length(v_subrogation_contri_comment) - (FLOOR(v_extra_comments_length * sub_cont_comment_ratio)) - 35) || '...Rest of the comments removed...'
	...
	DECODE(
	    TRUE,
	    v_pros_comment IS NULL OR LENGTH(v_pros_comment)>0 AND TRIM(v_pros_comment)='' OR LENGTH(v_pros_comment) = 0, 'N/A',
	    v_total_comments_length <= 7000, v_pros_comment,
	    v_total_comments_length > 7000, SUBSTR(v_pros_comment, 1, length(v_pros_comment) - (FLOOR(v_extra_comments_length * pros_comment_ratio)) - 35) || '...Rest of the comments removed...'
	) AS pros_comment,
	-- *INF*: DECODE (TRUE,
	-- 
	...
	DECODE(
	    TRUE,
	    v_cons_comment IS NULL OR LENGTH(v_cons_comment)>0 AND TRIM(v_cons_comment)='' OR LENGTH(v_cons_comment) = 0, 'N/A',
	    v_total_comments_length <= 7000, v_cons_comment,
	    v_total_comments_length > 7000, SUBSTR(v_cons_comment, 1, length(v_cons_comment) - (FLOOR(v_extra_comments_length * cons_comment_ratio)) - 35) || '...Rest of the comments removed...'
	) AS cons_comment,
	-- *INF*: DECODE (TRUE,
	-- 
	...
	DECODE(
	    TRUE,
	    v_pripst_injury_comment IS NULL OR LENGTH(v_pripst_injury_comment)>0 AND TRIM(v_pripst_injury_comment)='' OR LENGTH(v_pripst_injury_comment) = 0, 'N/A',
	    v_total_comments_length <= 7000, v_pripst_injury_comment,
	    v_total_comments_length > 7000, SUBSTR(v_pripst_injury_comment, 1, length(v_pripst_injury_comment) - (FLOOR(v_extra_comments_length * prior_post_inj_comment_ratio)) - 35) || '...Rest of the comments removed...'
	...
	DECODE(
	    TRUE,
	    v_liability_comment IS NULL OR LENGTH(v_liability_comment)>0 AND TRIM(v_liability_comment)='' OR LENGTH(v_liability_comment) = 0, 'N/A',
	    v_total_comments_length <= 7000, v_liability_comment,
	    v_total_comments_length > 7000, SUBSTR(v_liability_comment, 1, length(v_liability_comment) - (FLOOR(v_extra_comments_length * liab_comment_ratio)) - 35) || '...Rest of the comments removed...'
...
EXP_DETECT_CHANGES AS (
	...
	-- 
	IFF(
	    old_claim_case_id IS NULL, 'NEW',
	    IFF(
	        LTRIM(RTRIM(case_name)) <> LTRIM(RTRIM(old_claim_case_name))
	        or LTRIM(RTRIM(case_number)) <> LTRIM(RTRIM(old_claim_case_num))
	        or LTRIM(RTRIM(suit_venue)) <> LTRIM(RTRIM(old_suit_county))
	        or LTRIM(RTRIM(suit_state)) <> LTRIM(RTRIM(old_suit_state))
	        or trial_date <> old_trial_date
	        or LTRIM(RTRIM(first_not_law_suit)) <> LTRIM(RTRIM(old_first_notice_law_suit_ind))
	        or LTRIM(RTRIM(declaratory_act)) <> LTRIM(RTRIM(old_declaratory_act))
	        or LTRIM(RTRIM(suit_status)) <> LTRIM(RTRIM(old_suit_status_code))
	        or denial_date <> old_suit_denial_date
	        or Prim_lit_handler_ak_id <> old_prim_litigation_handler_ak_id
	        or LTRIM(RTRIM(prim_lit_handler_role_code)) <> LTRIM(RTRIM(old_prim_litigation_handler_role_code))
	        or litigation_date <> old_suit_open_date
	        or litigation_closed <> old_suit_close_date
	        or LTRIM(RTRIM(how_clm_closed)) <> LTRIM(RTRIM(old_suit_how_claim_closed))
	        or LTRIM(RTRIM(reins_reported)) <> LTRIM(RTRIM(old_reins_reported_ind))
