...
EXP_VALIDATE AS (
	...
	IFF(LTRIM(RTRIM(tch_claim_nbr_ccm
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(tch_claim_nbr_ccm
			)
		))>0 AND TRIM(LTRIM(RTRIM(tch_claim_nbr_ccm
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(tch_claim_nbr_ccm
				)
	...
	IFF(LTRIM(RTRIM(tch_client_id_ccm
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(tch_client_id_ccm
			)
		))>0 AND TRIM(LTRIM(RTRIM(tch_client_id_ccm
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(tch_client_id_ccm
				)
	...
	IFF(LTRIM(RTRIM(IN_case_name
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_case_name
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_case_name
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_case_name
				)
	...
	IFF(LTRIM(RTRIM(IN_case_number
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_case_number
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_case_number
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_case_number
				)
	...
	IFF(LTRIM(RTRIM(IN_suit_venue
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_suit_venue
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_suit_venue
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_suit_venue
				)
	...
	IFF(LTRIM(RTRIM(IN_suit_state
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_suit_state
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_suit_state
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_suit_state
				)
	...
	IFF(LTRIM(RTRIM(IN_first_not_law_suit
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_first_not_law_suit
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_first_not_law_suit
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_first_not_law_suit
				)
		...
		'N/A',
		LTRIM(RTRIM(IN_first_not_law_suit
			)
		)
	) AS first_not_law_suit,
	...
	IFF(LTRIM(RTRIM(IN_declaratory_act
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_declaratory_act
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_declaratory_act
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_declaratory_act
				)
		...
		'N/A',
		LTRIM(RTRIM(IN_declaratory_act
			)
		)
	) AS declaratory_act,
	...
	IFF(LTRIM(RTRIM(IN_suit_status
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_suit_status
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_suit_status
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_suit_status
				)
	...
	IFF(IN_litigation_date IS NULL,
		TO_DATE('01/01/1800 00:00:00', 'MM/DD/YYYY HH24:MI:SS'
		),
		IN_litigation_date
	) AS litigation_date,
	...
	IFF(IN_litigation_closed IS NULL,
		TO_DATE('01/01/1800 00:00:00', 'MM/DD/YYYY HH24:MI:SS'
		),
		IN_litigation_closed
	) AS litigation_closed,
	...
	IFF(LTRIM(RTRIM(IN_how_clm_closed
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_how_clm_closed
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_how_clm_closed
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_how_clm_closed
				)
	...
	IFF(LTRIM(RTRIM(IN_reins_reported
			)
		) IS NULL 
		OR LENGTH(LTRIM(RTRIM(IN_reins_reported
			)
		))>0 AND TRIM(LTRIM(RTRIM(IN_reins_reported
			)
		))='' 
		OR LENGTH(LTRIM(RTRIM(IN_reins_reported
				)
...
EXP_LKP_VALUES AS (
	...
	rtrim(LKP_CLM_COMMENTS_STAGE_tch_claim_nbr_IN_policy_limit_id.tcc_comment_txt
	) AS v_pol_lim_comment,
	EXP_VALIDATE.IN_case_desc_id,
	-- *INF*: rtrim(:LKP.LKP_CLM_COMMENTS_STAGE(tch_claim_nbr,IN_case_desc_id))
	rtrim(LKP_CLM_COMMENTS_STAGE_tch_claim_nbr_IN_case_desc_id.tcc_comment_txt
	) AS v_claim_case_comment,
	EXP_VALIDATE.IN_inj_dam_desc_id,
	-- *INF*: rtrim(:LKP.LKP_CLM_COMMENTS_STAGE(tch_claim_nbr,IN_inj_dam_desc_id))
	rtrim(LKP_CLM_COMMENTS_STAGE_tch_claim_nbr_IN_inj_dam_desc_id.tcc_comment_txt
	) AS v_injury_dam_comment,
	EXP_VALIDATE.IN_subro_cont_id,
	-- *INF*: rtrim(:LKP.LKP_CLM_COMMENTS_STAGE(tch_claim_nbr,IN_subro_cont_id))
	rtrim(LKP_CLM_COMMENTS_STAGE_tch_claim_nbr_IN_subro_cont_id.tcc_comment_txt
	) AS v_subrogation_contri_comment,
	...
	IFF(IN_prim_lit_handler_ak_id IS NULL,
		- 1,
		IN_prim_lit_handler_ak_id
	) AS Out_prim_lit_handler_ak_id,
	...
	IFF(IN_prim_lit_handler IS NULL 
		OR LENGTH(IN_prim_lit_handler)>0 AND TRIM(IN_prim_lit_handler)='' 
		OR LENGTH(IN_prim_lit_handler
		) = 0,
		'N/A',
		IFF(caj_emp_client_id IS NOT NULL,
			IFF(caj_adjuster_class = 'L',
				'STF',
				'ADJ'
			),
			IFF(cre_client_id IS NULL,
				'REG',
				'ARB'
			)
		)
	) AS v_prim_lit_handler_role_code,
	...
	rtrim(LKP_CLM_COMMENTS_STAGE_tch_claim_nbr_IN_pros_cmt_id.tcc_comment_txt
	) AS v_pros_comment,
	EXP_VALIDATE.IN_cons_cmt_id,
	-- *INF*: rtrim(:LKP.LKP_CLM_COMMENTS_STAGE(tch_claim_nbr,IN_cons_cmt_id))
	rtrim(LKP_CLM_COMMENTS_STAGE_tch_claim_nbr_IN_cons_cmt_id.tcc_comment_txt
	) AS v_cons_comment,
	EXP_VALIDATE.IN_pripst_inj_desc_id,
	-- *INF*: rtrim(:LKP.LKP_CLM_COMMENTS_STAGE(tch_claim_nbr,IN_pripst_inj_desc_id))
	rtrim(LKP_CLM_COMMENTS_STAGE_tch_claim_nbr_IN_pripst_inj_desc_id.tcc_comment_txt
	) AS v_pripst_injury_comment,
	EXP_VALIDATE.IN_liab_cmt_id,
	-- *INF*: rtrim(:LKP.LKP_CLM_COMMENTS_STAGE(tch_claim_nbr,IN_liab_cmt_id))
	rtrim(LKP_CLM_COMMENTS_STAGE_tch_claim_nbr_IN_liab_cmt_id.tcc_comment_txt
	) AS v_liability_comment,
	...
	1 + length(v_pol_lim_comment || v_claim_case_comment || v_injury_dam_comment || v_subrogation_contri_comment || v_pros_comment || v_cons_comment || v_pripst_injury_comment || v_liability_comment
	) AS v_total_comments_length,
	...
	length(v_pol_lim_comment
	) / v_total_comments_length AS pol_lmt_comment_ratio,
	-- *INF*: length(v_claim_case_comment)/v_total_comments_length
	-- 
	-- //finding out what ratio of the total length, this particular comment field is taking
	length(v_claim_case_comment
	) / v_total_comments_length AS claim_case_comment_ratio,
	-- *INF*: length(v_injury_dam_comment)/v_total_comments_length
	-- 
	-- 
	-- //finding out what ratio of the total length, this particular comment field is taking
	length(v_injury_dam_comment
	) / v_total_comments_length AS inj_dam_comment_ratio,
	-- *INF*: length(v_subrogation_contri_comment)/v_total_comments_length
	-- 
	-- //finding out what ratio of the total length, this particular comment field is taking
	length(v_subrogation_contri_comment
	) / v_total_comments_length AS sub_cont_comment_ratio,
	-- *INF*: length(v_pros_comment)/v_total_comments_length
	-- 
	-- //finding out what ratio of the total length, this particular comment field is taking
	length(v_pros_comment
	) / v_total_comments_length AS pros_comment_ratio,
	-- *INF*: length(v_cons_comment)/v_total_comments_length
	-- 
	-- //finding out what ratio of the total length, this particular comment field is taking
	length(v_cons_comment
	) / v_total_comments_length AS cons_comment_ratio,
	-- *INF*: length(v_pripst_injury_comment)/v_total_comments_length
	-- 
	-- //finding out what ratio of the total length, this particular comment field is taking
	length(v_pripst_injury_comment
	) / v_total_comments_length AS prior_post_inj_comment_ratio,
	-- *INF*: length(v_liability_comment)/v_total_comments_length
	-- 
	-- //finding out what ratio of the total length, this particular comment field is taking
	length(v_liability_comment
	) / v_total_comments_length AS liab_comment_ratio,
	-- *INF*: DECODE (TRUE,
	-- 
	...
	DECODE(TRUE,
		v_pol_lim_comment IS NULL 
		OR LENGTH(v_pol_lim_comment)>0 AND TRIM(v_pol_lim_comment)='' 
		OR LENGTH(v_pol_lim_comment
		) = 0, 'N/A',
		v_total_comments_length <= 7000, v_pol_lim_comment,
		v_total_comments_length > 7000, SUBSTR(v_pol_lim_comment, 1, length(v_pol_lim_comment
			) - ( FLOOR(v_extra_comments_length * pol_lmt_comment_ratio
				) 
			) - 35
		) || '...Rest of the comments removed...'
	) AS pol_lim_comment,
	-- *INF*: DECODE (TRUE,
	-- 
	...
	DECODE(TRUE,
		v_claim_case_comment IS NULL 
		OR LENGTH(v_claim_case_comment)>0 AND TRIM(v_claim_case_comment)='' 
		OR LENGTH(v_claim_case_comment
		) = 0, 'N/A',
		v_total_comments_length <= 7000, v_claim_case_comment,
		v_total_comments_length > 7000, SUBSTR(v_claim_case_comment, 1, length(v_claim_case_comment
			) - ( FLOOR(v_extra_comments_length * claim_case_comment_ratio
				) 
			) - 35
		) || '...Rest of the comments removed...'
	) AS claim_case_comment,
	-- *INF*: DECODE (TRUE,
	-- 
	...
	DECODE(TRUE,
		v_injury_dam_comment IS NULL 
		OR LENGTH(v_injury_dam_comment)>0 AND TRIM(v_injury_dam_comment)='' 
		OR LENGTH(v_injury_dam_comment
		) = 0, 'N/A',
		v_total_comments_length <= 7000, v_injury_dam_comment,
		v_total_comments_length > 7000, SUBSTR(v_injury_dam_comment, 1, length(v_injury_dam_comment
			) - ( FLOOR(v_extra_comments_length * inj_dam_comment_ratio
				) 
			) - 35
		) || '...Rest of the comments removed...'
	) AS injury_dam_comment,
	-- *INF*: DECODE (TRUE,
	-- 
	...
	DECODE(TRUE,
		v_subrogation_contri_comment IS NULL 
		OR LENGTH(v_subrogation_contri_comment)>0 AND TRIM(v_subrogation_contri_comment)='' 
		OR LENGTH(v_subrogation_contri_comment
		) = 0, 'N/A',
		v_total_comments_length <= 7000, v_subrogation_contri_comment,
		v_total_comments_length > 7000, SUBSTR(v_subrogation_contri_comment, 1, length(v_subrogation_contri_comment
			) - ( FLOOR(v_extra_comments_length * sub_cont_comment_ratio
				) 
			) - 35
		) || '...Rest of the comments removed...'
	) AS subrogation_contri_comment,
	-- *INF*: DECODE (TRUE,
	-- 
	...
	DECODE(TRUE,
		v_pros_comment IS NULL 
		OR LENGTH(v_pros_comment)>0 AND TRIM(v_pros_comment)='' 
		OR LENGTH(v_pros_comment
		) = 0, 'N/A',
		v_total_comments_length <= 7000, v_pros_comment,
		v_total_comments_length > 7000, SUBSTR(v_pros_comment, 1, length(v_pros_comment
			) - ( FLOOR(v_extra_comments_length * pros_comment_ratio
				) 
			) - 35
		) || '...Rest of the comments removed...'
	) AS pros_comment,
	...
	DECODE(TRUE,
		v_cons_comment IS NULL 
		OR LENGTH(v_cons_comment)>0 AND TRIM(v_cons_comment)='' 
		OR LENGTH(v_cons_comment
		) = 0, 'N/A',
		v_total_comments_length <= 7000, v_cons_comment,
		v_total_comments_length > 7000, SUBSTR(v_cons_comment, 1, length(v_cons_comment
			) - ( FLOOR(v_extra_comments_length * cons_comment_ratio
				) 
			) - 35
		) || '...Rest of the comments removed...'
	) AS cons_comment,
	...
	DECODE(TRUE,
		v_pripst_injury_comment IS NULL 
		OR LENGTH(v_pripst_injury_comment)>0 AND TRIM(v_pripst_injury_comment)='' 
		OR LENGTH(v_pripst_injury_comment
		) = 0, 'N/A',
		v_total_comments_length <= 7000, v_pripst_injury_comment,
		v_total_comments_length > 7000, SUBSTR(v_pripst_injury_comment, 1, length(v_pripst_injury_comment
			) - ( FLOOR(v_extra_comments_length * prior_post_inj_comment_ratio
				) 
			) - 35
		) || '...Rest of the comments removed...'
	) AS pripst_injury_comment,
	-- *INF*: DECODE (TRUE,
	-- 
	...
	DECODE(TRUE,
		v_liability_comment IS NULL 
		OR LENGTH(v_liability_comment)>0 AND TRIM(v_liability_comment)='' 
		OR LENGTH(v_liability_comment
		) = 0, 'N/A',
		v_total_comments_length <= 7000, v_liability_comment,
		v_total_comments_length > 7000, SUBSTR(v_liability_comment, 1, length(v_liability_comment
			) - ( FLOOR(v_extra_comments_length * liab_comment_ratio
				) 
			) - 35
		) || '...Rest of the comments removed...'
	) AS liability_comment,
	EXP_VALIDATE.settlement_type_cd,
	-- *INF*: IIF(ISNULL(settlement_type_cd), 'N/A', settlement_type_cd)
	IFF(settlement_type_cd IS NULL,
		'N/A',
		settlement_type_cd
	) AS SettlementTypeCode
	FROM EXP_VALIDATE
...
EXP_DETECT_CHANGES AS (
	...
	IFF(old_claim_case_id IS NULL,
		...
		IFF(LTRIM(RTRIM(case_name
			...
			OR LTRIM(RTRIM(suit_venue
				)
			) <> LTRIM(RTRIM(old_suit_county
				)
			) 
			OR LTRIM(RTRIM(suit_state
				)
			) <> LTRIM(RTRIM(old_suit_state
				)
			) 
			OR trial_date <> old_trial_date 
			OR LTRIM(RTRIM(first_not_law_suit
				)
			) <> LTRIM(RTRIM(old_first_notice_law_suit_ind
				)
			) 
			OR LTRIM(RTRIM(declaratory_act
				)
			) <> LTRIM(RTRIM(old_declaratory_act
				)
			) 
			...
			OR denial_date <> old_suit_denial_date 
			OR Prim_lit_handler_ak_id <> old_prim_litigation_handler_ak_id 
			OR LTRIM(RTRIM(prim_lit_handler_role_code
				)
			) <> LTRIM(RTRIM(old_prim_litigation_handler_role_code
				)
			) 
			OR litigation_date <> old_suit_open_date 
			OR litigation_closed <> old_suit_close_date 
			OR LTRIM(RTRIM(how_clm_closed
				)
			) <> LTRIM(RTRIM(old_suit_how_claim_closed
				)
			) 
			OR LTRIM(RTRIM(reins_reported
				)
			) <> LTRIM(RTRIM(old_reins_reported_ind
				)
			) 
			OR com_umb_res <> old_commercl_umb_reserve 
			OR payment_amt <> old_suit_pay_amt 
			OR demand_at_initial_litigation <> old_Demand_at_initial_litigation 
			OR SettlementTypeCode <> Old_SettlementTypeCode,
			'UPDATE',
			'NOCHANGE'
		)
	) AS v_changed_flag,
