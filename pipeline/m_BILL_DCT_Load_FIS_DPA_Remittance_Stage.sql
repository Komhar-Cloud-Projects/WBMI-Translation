...
EXP_Load_Remittance_Stage AS (
	...
	'Vendor Policy Number number missing Transaction ID =  '  || transaction_id AS var_vendor_PolicyMissing_Message,
	-- *INF*: DECODE( TRUE,
	-- ISNULL(in_VendorAccountNumber), ERROR(var_vendor_AccountMissing_Message), 
	-- length(ltrim(rtrim(in_VendorAccountNumber))) = 12, in_VendorAccountNumber,
	-- '')
	DECODE(
	    TRUE,
	...
	-- *INF*: DECODE( TRUE,
	-- ISNULL(in_VendorAccountNumber), ERROR(var_vendor_PolicyMissing_Message), 
	-- length(ltrim(rtrim(in_VendorAccountNumber))) = 7, in_VendorAccountNumber,
	-- '')
	DECODE(
	    TRUE,
	    in_VendorAccountNumber IS NULL, ERROR(var_vendor_PolicyMissing_Message),
	    length(ltrim(rtrim(in_VendorAccountNumber))) = 7, in_VendorAccountNumber,
	    ''
	) AS out_PolicyNumber
...
FIL_TransactionID AS (
	...
	EXP_Load_Remittance_Stage.out_ModifyDate AS ModifyDate, 
	...
	EXP_Load_Remittance_Stage.customer_name, 
	...
	EXP_Load_Remittance_Stage.out_PolicyNumber AS PolicyNumber
