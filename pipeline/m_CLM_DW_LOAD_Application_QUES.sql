...
EXP_VALIDATE AS (
	...
	-- 
	IFF(
	    LTRIM(RTRIM(app_guid)) IS NULL
	    or LENGTH(LTRIM(RTRIM(app_guid)))>0
	    and TRIM(LTRIM(RTRIM(app_guid)))=''
	    or LENGTH(LTRIM(RTRIM(app_guid))) = 0,
	    'N/A',
	...
	-- 
	IFF(
	    LTRIM(RTRIM(display_name)) IS NULL
	    or LENGTH(LTRIM(RTRIM(display_name)))>0
	    and TRIM(LTRIM(RTRIM(display_name)))=''
	    or LENGTH(LTRIM(RTRIM(display_name))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(version_num)) IS NULL
	    or LENGTH(LTRIM(RTRIM(version_num)))>0
	    and TRIM(LTRIM(RTRIM(version_num)))=''
	    or LENGTH(LTRIM(RTRIM(version_num))) = 0,
	    'N/A',
	...
	IFF(eff_date IS NULL, TO_TIMESTAMP('1/1/1800', 'MM/DD/YYYY'), eff_date) AS v_eff_date,
	v_eff_date AS o_eff_date,
	exp_date,
	-- *INF*: IIF(ISNULL(exp_date),TO_DATE('1/1/1800','MM/DD/YYYY'),exp_date)
	IFF(exp_date IS NULL, TO_TIMESTAMP('1/1/1800', 'MM/DD/YYYY'), exp_date) AS v_exp_date,
...
EXP_DETECT_CHANGES AS (
	...
	-- 	'UPDATE','NOCHANGE'))
	IFF(
	    LKP_app_id IS NULL, 'NEW',
	    IFF(
	        LTRIM(RTRIM(o_display_name)) <> LTRIM(RTRIM(LKP_display_name))
	        or LTRIM(RTRIM(o_published_to_prod_flag)) <> LTRIM(RTRIM(LKP_published_to_prod_flag))
	        or LTRIM(RTRIM(o_enabled_flag)) <> LTRIM(RTRIM(LKP_enabled_flag))
	        or LTRIM(RTRIM(o_version_num)) <> LTRIM(RTRIM(LKP_version_num))
