...
EXP_source_anchor AS (
	...
	IFF(
	    LTRIM(RTRIM(vehicle_make)) IS NULL OR LENGTH(LTRIM(RTRIM(vehicle_make))) = 0, 'N/A',
...
EXP_expire_rows AS (
	...
	DECODE(
	    TRUE,
	    claim_total_loss_ak_id = v_prev_row_claim_total_loss_ak_id AND claimant_cov_det_ak_id = v_prev_row_claimant_cov_det_ak_id, DATEADD(SECOND,- 1,v_prev_row_eff_from_date),
	    eff_to_date
