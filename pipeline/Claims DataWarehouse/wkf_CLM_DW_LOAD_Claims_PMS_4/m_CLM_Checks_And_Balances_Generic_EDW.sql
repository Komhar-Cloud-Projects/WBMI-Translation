...
SQ_curr_row_count AS (
	--Check that all source recods from pif_40_stage have been populated into target Reinsurance Coverage table
	SELECT 
	   COUNT(distinct c.pol_ak_id)  as EDW_count, 
	   'Count of Records from @{pipeline().parameters.SOURCE_TABLE_NAME} which are not populated into target EDW.Reinsurance Coverage = ' + convert(varchar,count(distinct c.pol_ak_id)) as check_out_message,
	'@{pipeline().parameters.SOURCE_TABLE_NAME}' as target_name
	FROM @{pipeline().parameters.STG_DB}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.PIF_40_stage b, @{pipeline().parameters.SOURCE_TABLE_OWNER_V2}.policy c
	where convert(varchar, b.pif_symbol + b.pif_policy_number + b.pif_module) = c.pol_key 
	and not exists 
	( select 1
	   FROM @{pipeline().parameters.SOURCE_TABLE_OWNER}.@{pipeline().parameters.SOURCE_TABLE_NAME} a 
		WHERE crrnt_snpsht_flag=1
	    and c.pol_ak_id = a.pol_ak_id  )
