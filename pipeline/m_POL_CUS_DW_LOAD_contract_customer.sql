...
EXP_values AS (
	...
	IFF(in_Pmdl4w1FedEmpIdNumber IS NULL 
		OR LENGTH(in_Pmdl4w1FedEmpIdNumber)>0 AND TRIM(in_Pmdl4w1FedEmpIdNumber)='' 
		OR LENGTH(in_Pmdl4w1FedEmpIdNumber
		) = 0,
		'N/A',
	...
	IFF(LTRIM(RTRIM(in_pif_line_business
			)
		) = 'WC' 
		OR LTRIM(RTRIM(in_pif_line_business
			)
		) = 'WCP',
	...
	LTRIM(RTRIM(in_pmd4j_phone_area
		)
	) || LTRIM(RTRIM(in_pmd4j_phone_exchange
		)
	) || LTRIM(RTRIM(in_pmd4j_phone_number
		)
	) AS v_ph_num_full,
	...
	IFF(in_pif_legal_entity IS NULL 
		OR LENGTH(in_pif_legal_entity)>0 AND TRIM(in_pif_legal_entity)='' 
		OR LENGTH(in_pif_legal_entity
		) = 0,
		'N/A',
	...
	-- 
	IFF(in_pif_sic IS NULL 
		OR LENGTH(in_pif_sic)>0 AND TRIM(in_pif_sic)='' 
		OR LENGTH(in_pif_sic
		) = 0,
		'N/A',
	...
	:UDF.DEFAULT_VALUE_FOR_STRINGS(v_ph_num_full
	) AS o_ph_num_full,
...
EXP_Detect_Changes AS (
	...
	-- )
	IFF(in_sup_lgl_ent_code_id IS NULL,
		- 1,
		in_sup_lgl_ent_code_id
	) AS v_sup_lgl_ent_code_id,
	...
	IFF(lkp_cust_ak_id IS NULL,
		'NEW',
		IFF(lkp_cust_num != customer_number 
			OR lkp_name != name 
