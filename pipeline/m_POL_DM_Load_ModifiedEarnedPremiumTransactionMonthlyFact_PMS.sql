...
SQ_WC_ModifiedPremium_OtherLines_GeneratedModifiedPremium AS (
	...
	INNER JOIN @{pipeline().parameters.SOURCE_TABLE_OWNER}.StatisticalCoverage SC
	             ON WEMP.StatisticalCoverageAKID=SC.StatisticalCoverageAKID  
	...
	INNER JOIN @{pipeline().parameters.SOURCE_TABLE_OWNER}.PolicyCoverage PC
	             ON WEMP.PolicyCoverageAKID=PC.PolicyCoverageAKID 
	INNER JOIN @{pipeline().parameters.SOURCE_TABLE_OWNER}.RiskLocation RL
	             ON WEMP.RiskLocationAKID=RL.RiskLocationAKID 
	INNER JOIN @{pipeline().parameters.SOURCE_TABLE_OWNER_V2}.Policy P
			ON WEMP.PolicyAKID=P.pol_ak_id AND P.crrnt_snpsht_flag=1
			@{pipeline().parameters.JOIN_POLICY_LIST_IL}		
	...
	LEFT OUTER JOIN  @{pipeline().parameters.SOURCE_TABLE_OWNER}.InsuranceSegment ISS
			ON P.InsuranceSegmentAKId=ISS.InsuranceSegmentAKId AND ISS.CurrentSnapshotFlag=1
	LEFT OUTER JOIN  @{pipeline().parameters.SOURCE_TABLE_OWNER}.StrategicProfitCenter SPC
	            ON P.StrategicProfitCenterAKId=SPC.StrategicProfitCenterAKId AND SPC.CurrentSnapshotFlag=1
	LEFT OUTER JOIN  @{pipeline().parameters.SOURCE_TABLE_OWNER}.EnterpriseGroup EGP
	           ON SPC.EnterpriseGroupId=EGP.EnterpriseGroupId AND EGP.CurrentSnapshotFlag=1
	LEFT OUTER JOIN  @{pipeline().parameters.SOURCE_TABLE_OWNER}.InsuranceReferenceLegalEntity IRLE
	          ON SPC.InsuranceReferenceLegalEntityId=IRLE.InsuranceReferenceLegalEntityId AND IRLE.CurrentSnapshotFlag=1
...
EXP_EarnedPremiumCalculation_IN AS (
	...
	ModifiedPremiumType
	FROM SQ_WC_ModifiedPremium_OtherLines_GeneratedModifiedPremium
),
