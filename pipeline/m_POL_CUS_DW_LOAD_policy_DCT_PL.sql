...
EXP_values AS (
	...
	Decode(
	    ...
	    in_CreatedDate < v_EffectiveDate AND (v_pol_cancellation_date_persist > in_CreatedDate OR v_pol_cancellation_date_persist > v_EffectiveDate), 'F',
	    in_CreatedDate > v_ExpirationDate, 'N',
	    v_pol_cancellation_date <> TO_TIMESTAMP('21001231235959', 'YYYYMMDDHH24MISS') or v_pol_cancellation_date_persist <> TO_TIMESTAMP('21001231235959', 'YYYYMMDDHH24MISS'), 'C',
	...
	-- )
	IFF(
	    PolicyIssueCode IS NULL
	    or LENGTH(PolicyIssueCode)>0
	    and TRIM(PolicyIssueCode)=''
	    or LTRIM(RTRIM(PolicyIssueCode)) = '',
	    v_lkp_pol_issue_code,
	    IFF(
	        LTRIM(RTRIM(PolicyIssueCode)) = 'New', 'N', 'R'
	    )
