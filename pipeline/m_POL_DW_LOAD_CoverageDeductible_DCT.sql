...
EXP_DataCollectSRC AS (
	...
	IFF(
	    CoverageGuId IS NULL
	    or LENGTH(CoverageGuId)>0
	    and TRIM(CoverageGuId)=''
	    or LENGTH(CoverageGuId) = 0,
	    'N/A',
...
mplt_Load_Deductibles_IL_Layer_DCT AS (WITH
	...
	EXP_Default_Values AS (
		...
		UDF_DEFAULT_VALUE_FOR_STRINGS(i_Type) AS o_CoverageLimitType,
		-- *INF*: TO_CHAR(TO_DECIMAL(i_Value))
		TO_CHAR(CAST(i_Value AS FLOAT)) AS o_CoverageLimitValue,
		AuditId
	...
	EXP_HANDE AS (
		...
		IFF(i_CoverageDeductibleValue IS NOT NULL, i_CoverageDeductibleValue, '0') AS o_CoverageDeductibleVlaue
