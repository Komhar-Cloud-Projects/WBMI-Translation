...
EXP_EDW_Convert_Agency_Key_Id_for_Producer_code AS (
	...
	IFF(v_producer_code IS NULL, 'N/A', IFF(IS_SPACES(v_producer_code), 'N/A', v_producer_code)) AS PRODUCER_CODE,
	...
	IFF(IN_PRODUCER_DESCRIPT IS NULL, 'Not Available', IFF(IS_SPACES(IN_PRODUCER_DESCRIPT), 'Not Available', IFF(length(IN_PRODUCER_DESCRIPT) = 0, 'Not Available', ltrim(rtrim(IN_PRODUCER_DESCRIPT))))) AS PRODUCER_DESCRIPT,
...
EXP_Detect_Changes AS (
	...
	-- 
	-- 
	IFF(old_producer_code_id IS NULL, 'NEW', IFF(( PRODUCER_DESCRIPT <> OLD_PRODUCER_DESCRIPT ) OR ( OUT_AGENCY_ak_ID <> OLD_OUT_AGENCY_ak_ID ), 'UPDATE', 'NOCHANGE')) AS v_changed_flag,
...
EXP_UPD_Producer_Code AS (
	...
	-- 	orig_eff_to_date)
	DECODE(TRUE,
		producer_code = v_PREV_ROW_producer_code AND agency_key = v_PREV_ROW_agency_key AND emp_id = v_prev_row_emp_id, ADD_TO_DATE(v_PREV_ROW_eff_from_date, 'SS', - 1),
