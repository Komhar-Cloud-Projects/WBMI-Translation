...
EXP_Values AS (
	...
	IFF(LENGTH(LTRIM(RTRIM(sar_unit_complete))) = 0 OR IS_SPACES(LTRIM(RTRIM(sar_unit_complete))), '000000', sar_unit_complete) AS v_sar_unit,
	...
	:UDF.DEFAULT_VALUE_FOR_STRINGS(sar_type_bureau) AS sar_type_bureau_out,
	'N/A' AS default_NA,
...
EXP_Detect_Changes AS (
	...
	-- 	'UPDATE',
	-- 	'NOCHANGE'))
	IFF(lkp_cov_id IS NULL, 'NEW', IFF(lkp_type_bureau_code <> sar_type_bureau_out, 'UPDATE', 'NOCHANGE')) AS v_Changed_Flag,
...
EXP_Expire_Rows AS (
	...
	-- ,Orig_eff_to_date)
	DECODE(TRUE,
		ins_line = v_PREV_ROW_ins_line AND loc_unit_num = v_PREV_ROW_loc_unit_num AND sub_loc_unit_num = v_PREV_ROW_sub_loc_unit_num AND risk_unit_grp = v_PREV_ROW_risk_unit_grp AND risk_unit_grp_seq_num = v_PREV_ROW_risk_unit_grp_seq_num AND risk_unit = v_PREV_ROW_risk_unit AND risk_unit_seq_num = v_PREV_ROW_risk_unit_seq_num AND major_peril_code = v_PREV_ROW_major_peril_code AND major_peril_seq_num = v_PEV_ROW_major_peril_seq_num AND pms_type_exposure = v_PEV_ROW_pms_type_exposure AND cov_eff_date = v_PEV_ROW_cov_eff_date, ADD_TO_DATE(v_PREV_ROW_eff_from_date, 'SS', - 1),
