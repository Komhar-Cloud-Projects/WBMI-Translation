...
EXP_Lkp_Values_workers_comp_claimant_detail AS (
	...
	-- 
	-- 
	IFF(IN_wch_last_wrk_dt IS NULL, TO_TIMESTAMP('1/1/1800', 'MM/DD/YYYY'), IN_wch_last_wrk_dt) AS EMP_LAST_DAY_WORKED,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_CWC_CLAIM_NBR))),'N/A',IIF(IS_SPACES(IN_CWC_CLAIM_NBR),'N/A',LTRIM(RTRIM(IN_CWC_CLAIM_NBR))))
	IFF(
	    LTRIM(RTRIM(IN_CWC_CLAIM_NBR)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_CWC_CLAIM_NBR)>0
	    and TRIM(IN_CWC_CLAIM_NBR)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_CWC_CLIENT_ID)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_CWC_CLIENT_ID)>0
	    and TRIM(IN_CWC_CLIENT_ID)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_SOURCE_SYSTEM_ID)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_SOURCE_SYSTEM_ID)>0
	    and TRIM(IN_SOURCE_SYSTEM_ID)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_wrh_empr_pd_lit)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_wrh_empr_pd_lit)>0
	    and TRIM(IN_wrh_empr_pd_lit)='', 'N/A',
	...
	-- 
	IFF(
	    LTRIM(RTRIM(IN_wrh_restrictions)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_wrh_restrictions)>0
	    and TRIM(IN_wrh_restrictions)='', 'N/A',
	...
	IFF(IN_wch_return_dt IS NULL, TO_TIMESTAMP('1/1/1800', 'MM/DD/YYYY'), IN_wch_return_dt) AS RETURN_TO_WORK_DATE,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_wch_same_emp_ind))),'N/A',IIF(IS_SPACES(IN_wch_same_emp_ind),'N/A',LTRIM(RTRIM(IN_wch_same_emp_ind))))
	IFF(
	    LTRIM(RTRIM(IN_wch_same_emp_ind)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_wch_same_emp_ind)>0
	    and TRIM(IN_wch_same_emp_ind)='', 'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_wch_ret_type_cd)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_wch_ret_type_cd)>0
	    and TRIM(IN_wch_ret_type_cd)='', 'N/A',
	...
	-- 
	IFF(IN_wch_create_ts IS NULL, TO_TIMESTAMP('1/1/1800', 'MM/DD/YYYY'), IN_wch_create_ts) AS wch_create_ts
...
EXP_DETECT_CHANGES_workers_comp_claimant_detail AS (
	...
	-- 'NOCHANGE') )
	IFF(
	    wc_claimant_work_hist_ak_id IS NULL, 'NEW',
	    IFF(
	        LKP_return_to_work_date <> RETURN_TO_WORK_DATE
	        or LKP_emplyr_paid_amt <> EMPLYR_PAID_AMT
	        or ltrim(rtrim(LKP_emplyr_paid_litigated_ind)) <> ltrim(rtrim(EMPLYR_PAID_LITIGATED_IND))
	        or ltrim(rtrim(LKP_return_to_work_with_same_emplyr_ind)) <> ltrim(rtrim(RETURN_TO_WORK_WITH_SAME_EMPLYR))
	        or LKP_emp_last_day_worked <> EMP_LAST_DAY_WORKED
	        or ltrim(rtrim(RETURN_TO_WORK_TYPE)) <> ltrim(rtrim(LKP_return_to_work_type))
	        or ltrim(rtrim(LKP_return_to_work_with_restriction_ind)) <> ltrim(rtrim(RETURN_TO_WORK_WITH_RESTRICTION_IND)),
	        'UPDATE',
	        'NOCHANGE'
	    )
