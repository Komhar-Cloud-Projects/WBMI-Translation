...
EXP_Src_Values AS (
	...
	DECODE(TRUE,
		...
		IS_SPACES(CS01_CODE), 'N/A',
		LENGTH(CS01_CODE) = 0, 'N/A',
		LTRIM(RTRIM(CS01_CODE))) AS o_CS01_CODE,
	CS01_CODE_DES,
	-- *INF*: DECODE(TRUE,
	...
	DECODE(TRUE,
		...
		IS_SPACES(CS01_CODE_DES), 'N/A',
		LENGTH(CS01_CODE_DES) = 0, 'N/A',
		LTRIM(RTRIM(CS01_CODE_DES))) AS o_CS01_CODE_DES,
	SOURCE_SYSTEM_ID,
...
EXP_TargetLkp_Detect_Changes AS (
	...
	--    )
	--   
	IFF(Lkp_SupSurgeryTypeId IS NULL, 'NEW', IFF(LTRIM(RTRIM(CS01_CODE)) != LTRIM(RTRIM(Lkp_SurgeryTypeCode)) OR LTRIM(RTRIM(CS01_CODE_DES)) != LTRIM(RTRIM(Lkp_SurgeryTypeDescription)), 'UPDATE', 'NOCHANGE')) AS v_ChangedFlag,
...
EXP_Lag_ExpirationDate AS (
	...
	-- Orig_ExpirationDate)
	DECODE(TRUE,
		SurgeryTypeCode = v_PREV_ROW_SurgeryTypeCode, ADD_TO_DATE(v_PREV_ROW_EffectiveDate, 'SS', - 1),
