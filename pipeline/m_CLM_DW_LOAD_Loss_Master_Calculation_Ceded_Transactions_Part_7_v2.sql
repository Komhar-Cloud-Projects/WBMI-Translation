...
EXP_Derive_Values_Reserves AS (
	...
	IFF(NOT temp_pol_trans_ak_id IS NULL, 'VALID', 'NOTVALID') AS V_Valid_sar_transaction,
...
EXP_Generate_Part_7_Reinsurance_Reserves AS (
	...
	IFF(reins_insurance_line IS NULL OR LENGTH(reins_insurance_line) = 0, 'NA', reins_insurance_line) AS v_reins_insurance_line,
	...
	-- 
	IFF(reins_location_number IS NULL, '0000', LPAD(LTRIM(RTRIM(TO_CHAR(reins_location_number))), 4, '0')) AS v_reins_location_number,
	...
	IFF(reins_sub_location_number IS NULL, '000', LPAD(LTRIM(RTRIM(TO_CHAR(reins_sub_location_number))), 3, '0')) AS v_reins_sub_location_number,
	...
	TO_DECIMAL(sar_location_x) AS v_sar_location,
	...
	-- 
	-- 
	IFF(( loss_date <= reins_expiration_date ) AND ( reins_effective_date <= loss_date ) AND reins_prcnt_loss_ceded <> 0.0, 'VALID', 'INVALID') AS v_reins_validity,
	...
	DECODE(v_reins_validity,
		...
		v_reins_insurance_line = 'GL' AND v_reins_location_number <> sar_location_x, 'NOREINSURANCE',
		reins_section_code = sar_section AND reins_location = loss_unit_3pos_out OR reins_location = 'N/A', 'REINSURANCE',
		( sar_section = '2' AND IN(Reason_Amended, 'COL', 'CWO') ) AND ( reins_location = 'N/A' OR reins_location = loss_unit_3pos_out ), 'REINSURANCE')) AS v_reinsurance_flag,
	...
	DECODE(v_reinsurance_flag,
		...
		'REINSURANCE', IFF(reins_company_no = '0058' AND IN(pms_trans_code, '73', '75', '76', '77'), 'NOGENERATE', 'GENERATE')) AS v_Reins_generate_flag,
	...
	IFF(v_Reins_generate_flag = 'GENERATE', IFF(V_LM_Amount_Outstanding = 0.0 AND V_LM_Amount_Paid_Losses = 0.0 AND V_LM_Amount_Paid_Expenses = 0.0 AND V_LM_EOM_Unpaid_Loss_Adj_Exp = 0.0, 'FALSE', 'TRUE'), 'FALSE') AS Generate_flag,
	...
	DECODE(TRUE,
		IN(sar_insurance_line, 'GL', 'LI'), '000',
		sar_insurance_line != 'GA' OR ( sar_insurance_line = 'GA' AND loss_id = '51' ), sar_sub_location_x) AS sar_sub_location_calculated,
