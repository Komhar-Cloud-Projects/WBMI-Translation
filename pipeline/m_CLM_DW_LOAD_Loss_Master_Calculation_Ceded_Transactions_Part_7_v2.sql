...
EXP_Default_Reserves AS (
	...
	financial_type_code || s3p_trans_code || IFF(trans_ctgry_code = 'N/A',
		'',
		trans_ctgry_code
	) AS v_Exceed_Trans_Code,
...
mplt_loss_master_90N_91_92_pms_trans_code_conversions AS (WITH
	...
	EXP_Default_Reserves_91_92 AS (
		...
		DECODE(true,
			pms_trans_code = '90', 'O',
			pms_trans_code = '91' 
			AND trans_offset_onset_ind = 'N', 'O',
			trans_offset_onset_ind
	...
	EXP_check_transcode_lkp_output AS (
		...
		IFF(lkp_pms_trans_code IS NULL,
			'N/A',
			lkp_pms_trans_code
		) AS v_lkp_pms_trans_code,
		...
		IFF(trans_date_pms_code_66 IS NULL,
		...
		IFF(pms_trans_code_66 IS NULL,
			'N/A',
			pms_trans_code_66
		) AS v_pms_trans_code_66,
		...
		IFF(pms_trans_code_close_post_month_end IS NULL,
			'N/A',
			pms_trans_code_close_post_month_end
		) AS v_pms_trans_code_close_post_month_end,
		...
		DECODE(true,
			pms_trans_code != '92', 'N/A',
			pms_trans_code = '92' 
			AND v_pms_trans_code_66 != 'N/A' 
			AND v_trans_date_pms_code_66 <= LastDayOfLastMonth, '92',
			pms_trans_code = '92' 
			AND v_pms_trans_code_66 != 'N/A' 
			AND v_trans_date_pms_code_66 > LastDayOfLastMonth 
			AND v_pms_trans_code_close_post_month_end != 'N/A', '90',
			'91'
	...
	EXP_determine_conversion AS (
		...
		DECODE(true,
			pms_trans_code = '92' 
			AND pms_trans_code_92_out != 'N/A', pms_trans_code_92_out,
			pms_trans_code = '91' 
			AND trans_offset_onset_ind = 'N' 
			AND lkp_pms_trans_code_pre_month_end_is_close_trans = 'Y' 
			AND pms_trans_code_91_N_out != 'N/A', pms_trans_code_91_N_out,
			pms_trans_code = '91' 
			AND trans_offset_onset_ind = 'N' 
			AND lkp_pms_trans_code_pre_month_end_is_close_trans != 'Y', pms_trans_code,
			pms_trans_code = '91' 
			AND lkp_pms_trans_code_pre_month_end_is_close_trans = 'Y', '91',
			pms_trans_code = '91' 
			AND lkp_pms_trans_code_pre_month_end_is_close_trans != 'Y', '90',
			pms_trans_code = '90' 
			AND lkp_pms_trans_code_pre_month_end_is_close_trans = 'Y', pms_trans_code,
			pms_trans_code = '90' 
			AND lkp_pms_trans_code_pre_month_end = 'N/A', '91',
			pms_trans_code
		) AS v_pms_trans_code,
...
EXP_Derive_Values_Reserves AS (
	...
	SUBSTR(risk_unit_seq_num_out, 1, 1
	) AS risk_unit_seq_num_first_pos,
	-- *INF*: SUBSTR(risk_unit_seq_num_out,2,1)
	SUBSTR(risk_unit_seq_num_out, 2, 1
	) AS risk_unit_seq_num_second_pos,
	...
	IFF(temp_pol_trans_ak_id IS NOT NULL,
		'VALID',
		'NOTVALID'
	) AS V_Valid_sar_transaction,
...
EXP_Generate_Part_7_Reinsurance_Reserves AS (
	...
	IFF(reins_insurance_line IS NULL 
		OR LENGTH(reins_insurance_line
		) = 0,
		'NA',
	...
	-- 
	IFF(reins_location_number IS NULL,
		'0000',
		LPAD(LTRIM(RTRIM(TO_CHAR(reins_location_number
					)
				)
			), 4, '0'
		)
	...
	IFF(reins_sub_location_number IS NULL,
		'000',
		LPAD(LTRIM(RTRIM(TO_CHAR(reins_sub_location_number
					)
				)
			), 3, '0'
		)
	...
	IFF(reins_risk_unit_group IS NULL,
		'000',
	...
	IFF(reins_seq_rsk_unt_grp IS NULL,
		'000',
		LPAD(TO_CHAR(reins_seq_rsk_unt_grp
			), 3, '0'
		)
	) AS v_reins_seq_rsk_unt_grp,
	...
	-- 
	-- 
	IFF(( loss_date <= reins_expiration_date 
		) 
	...
	DECODE(v_reins_validity,
		...
		'VALID', DECODE(TRUE,
		v_reins_insurance_line = 'GL' 
			AND v_reins_location_number <> sar_location_x, 'NOREINSURANCE',
		reins_section_code = sar_section 
			AND reins_location = loss_unit_3pos_out 
			OR reins_location = 'N/A', 'REINSURANCE',
		( sar_section = '2' 
				AND Reason_Amended IN ('COL','CWO') 
			) 
			AND ( reins_location = 'N/A' 
				OR reins_location = loss_unit_3pos_out 
			), 'REINSURANCE'
		)
	...
	DECODE(v_reinsurance_flag,
		...
		'REINSURANCE', IFF(reins_company_no = '0058' 
			AND pms_trans_code IN ('73','75','76','77'),
			'NOGENERATE',
			'GENERATE'
		)
	...
	IFF(v_Reins_generate_flag = 'GENERATE',
		IFF(V_LM_Amount_Outstanding = 0.0 
	...
	IFF(sar_insurance_line = 'GL' 
		AND sar_location_x != '0000',
		'0000',
		sar_location_x
	) AS sar_location_calculated,
	-- *INF*: DECODE(TRUE,
