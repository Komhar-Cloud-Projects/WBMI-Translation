...
EXP_Default_Values AS (
	...
	IFF(IN_risk_unit IS NULL,
		'N/A',
		ltrim(rtrim(IN_risk_unit
			)
		)
	) AS RISK_UNIT_CODE_OUT,
	-- *INF*: IIF(ISNULL(IN_risk_unit_literal), 'N/A', IN_risk_unit_literal)
	IFF(IN_risk_unit_literal IS NULL,
		'N/A',
		IN_risk_unit_literal
	) AS RISK_UNIT_LITERAL_OUT
...
EXP_detect_changes AS (
	...
	-- 'UPDATE', 'NOCHANGE'))
	-- 
	IFF(old_sup_risk_unit_id IS NULL,
		'NEW',
		IFF(ltrim(rtrim(old_risk_unit_descript
				)
			) != ltrim(rtrim(RISK_UNIT_LITERAL_OUT
				)
			),
			'UPDATE',
			'NOCHANGE'
...
EXP_lag_eff_from_date AS (
	...
	-- 	
	DECODE(TRUE,
		risk_unit_code = v_Prev_row_risk_unit_code 
		AND ins_line = v_Prev_row_ins_line, DATEADD(SECOND,- 1,v_prev_eff_from_date),
		orig_eff_to_date
	) AS v_eff_to_date,
