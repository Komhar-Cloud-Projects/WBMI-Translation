...
EXP_LKP_Value_Workers_comp_claimant_detila AS (
	...
	IFF(
	    LTRIM(RTRIM(IN_claim_nbr)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_claim_nbr)>0
	    and TRIM(IN_claim_nbr)='', 'N/A', LTRIM(RTRIM(IN_claim_nbr))
	    )
	) AS CLAIM_NBR,
	-- *INF*: IIF(ISNULL(LTRIM(RTRIM(IN_client_id))),'N/A',IIF(IS_SPACES(IN_client_id),'N/A',LTRIM(RTRIM(IN_client_id))))
	IFF(
	    LTRIM(RTRIM(IN_client_id)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_client_id)>0
	    and TRIM(IN_client_id)='', 'N/A', LTRIM(RTRIM(IN_client_id))
	    )
	) AS CLIENT_ID,
	...
	IFF(
	    LTRIM(RTRIM(IN_body_part_code)) IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_body_part_code)>0
	    and TRIM(IN_body_part_code)='', 'N/A',
...
EXP_DETECT_CHANGES_workers_comp_claimant_detail11 AS (
	...
	-- ---'NOCHANGE') )
	IFF(
	    wc_claimant_impairment_ak_id IS NULL, 'NEW',
	    IFF(
	        LKP_IMPAIRMENT_PERCENTAGE <> IMPAIRMENT_PERCENTAGE
...
EXP_Lag_eff_from_date11 AS (
	...
	IFF(
	    wc_claimant_impairment_ak_id = v_PREV_ROW_wc_claimant_det_ak_id,
