...
SQ_PremiumMasterCalculation AS (
	...
	RPDT.RatingPlanCode,
	PC.PremiumMasterWrittenExposure,
	PC.DeclaredEventFlag
	FROM
	@{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.PremiumMasterCalculation PC with (nolock)
	Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.StatisticalCoverage STATCOV  with (nolock)
	on STATCOV.StatisticalCoverageAKID = PC.StatisticalCoverageAKID
	Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.PolicyCoverage POLCOV
	on 
	...
	Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER_V2}.Policy POL with (nolock)
	on PC.PolicyAKID = POL.pol_ak_id AND POL.crrnt_snpsht_flag = 1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.InsuranceReferenceLineOfBusiness INRB
	on STATCOV.InsuranceReferenceLineOfBusinessAKId=INRB.InsuranceReferenceLineOfBusinessAKId and INRB.CurrentSnapshotFlag=1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.Product PROD
	on STATCOV.ProductAKId=PROD.ProductAKId and PROD.CurrentSnapshotFlag=1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.PolicyOffering POFF
	on POL.PolicyOfferingAKId=POFF.PolicyOfferingAKId and POFF.CurrentSnapshotFlag=1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.InsuranceSegment INSG
	on POL.InsuranceSegmentAKId=INSG.InsuranceSegmentAKId and INSG.CurrentSnapshotFlag=1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.StrategicProfitCenter STPC
	on POL.StrategicProfitCenterAKId=STPC.StrategicProfitCenterAKId and STPC.CurrentSnapshotFlag=1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.EnterpriseGroup ENGP
	on STPC.EnterpriseGroupId=ENGP.EnterpriseGroupId and ENGP.CurrentSnapshotFlag=1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.InsuranceReferenceLegalEntity NRLE
	...
	WHERE PC.CreatedDate >= '@{pipeline().parameters.SELECTION_START_TS}' and PC.SourceSystemID='PMS'
	...
	Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.PolicyCoverage POLCOV
	on POLCOV.PolicyCoverageAKID = PC.PolicyCoverageAKID and POLCOV.CurrentSnapshotFlag=1
	Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.RiskLocation RL
	on 
	RL.RiskLocationAKID=POLCOV.RiskLocationAKID and RL.CurrentSnapshotFlag=1
	Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER_V2}.Policy POL with (nolock)
	on PC.PolicyAKID = POL.pol_ak_id AND POL.crrnt_snpsht_flag = 1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.InsuranceReferenceLineOfBusiness INRB
	on RC.InsuranceReferenceLineOfBusinessAKId=INRB.InsuranceReferenceLineOfBusinessAKId and INRB.CurrentSnapshotFlag=1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.Product PROD
	on RC.ProductAKId=PROD.ProductAKId and PROD.CurrentSnapshotFlag=1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.PolicyOffering POFF
	on POL.PolicyOfferingAKId=POFF.PolicyOfferingAKId and POFF.CurrentSnapshotFlag=1
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.InsuranceSegment INSG
	on POL.InsuranceSegmentAKId=INSG.InsuranceSegmentAKId and INSG.CurrentSnapshotFlag=1
	...
	ON SIL.ins_line_code = POLCOV.InsuranceLine
	AND SIL.crrnt_snpsht_flag = 1
	left join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.PolicyAudit PA
	on PC.PolicyAKID=PA.PolicyAKId and PA.InsuranceLine=POLCOV.InsuranceLine
	and PC.PremiumTransactionEnteredDate>= PA.EffectiveDate
	and PC.PremiumTransactionEnteredDate<= PA.ExpirationDate
	...
	Left Join @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.RatingPlan RPDT
	on POLCOV.RatingPlanAKId=RPDT.RatingPlanAKId and RPDT.CurrentSnapshotFlag=1
	WHERE PC.CreatedDate >= '@{pipeline().parameters.SELECTION_START_TS}' and PC.SourceSystemID='DCT'  
	@{pipeline().parameters.WHERE_CLAUSE}
),
EXP_Post_Source AS (
	SELECT
	...
	:UDF.DEFAULT_VALUE_FOR_STRINGS(i_PremiumType
	) AS o_PremiumType_out,
	-- *INF*: :UDF.DEFAULT_VALUE_FOR_STRINGS(PremiumMasterReasonAmendedCode)
	:UDF.DEFAULT_VALUE_FOR_STRINGS(PremiumMasterReasonAmendedCode
	) AS o_ReasonAmendedCode,
	-- *INF*: :UDF.DEFAULT_VALUE_FOR_STRINGS(i_PolicySymbol)
	:UDF.DEFAULT_VALUE_FOR_STRINGS(i_PolicySymbol
	) AS o_PolicySymbol,
	-- *INF*: :UDF.DEFAULT_VALUE_FOR_STRINGS(i_Line_of_Business)
	:UDF.DEFAULT_VALUE_FOR_STRINGS(i_Line_of_Business
	) AS o_Line_of_Business,
	-- *INF*: :UDF.DEFAULT_VALUE_FOR_STRINGS(i_Insurance_Line)
	:UDF.DEFAULT_VALUE_FOR_STRINGS(i_Insurance_Line
	) AS o_Insurance_Line,
	...
	:UDF.DEFAULT_VALUE_FOR_STRINGS(i_class_of_business
	) AS o_class_of_business,
...
LKP_CoverageDetailDim AS (
	...
	FROM (
		...
		from @{pipeline().parameters.SOURCE_DATABASE_NAME}.@{pipeline().parameters.SOURCE_TABLE_OWNER}.PremiumMasterCalculation PMC 
		...
		where PMC.CreatedDate>='@{pipeline().parameters.SELECTION_START_TS}'
	)
...
EXP_PremiumMasterCalculation_IN AS (
	...
	LKP_CoverageDetailDim.ClassCode AS i_ClassCode,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterBureauInceptionDate1 AS i_PremiumMasterBureauInceptionDate,
	mplt_Premium_ASL_Insurance_Hierarchy.O_PremiumTransactionCode AS i_PremiumMasterTransactionCode,
	mplt_Premium_ASL_Insurance_Hierarchy.O_ReasonAmendedCode AS i_ReasonAmendedCode,
	mplt_Premium_ASL_Insurance_Hierarchy.O_PremiumTransactionID AS PremiumTransactionID,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterCalculationID1 AS PremiumMasterCalculationID,
	mplt_Premium_ASL_Insurance_Hierarchy.PolicyAKID1 AS PolicyAKID,
	mplt_Premium_ASL_Insurance_Hierarchy.O_ReinsuranceCoverageAKID AS ReinsuranceCoverageAKID,
	mplt_Premium_ASL_Insurance_Hierarchy.O_PremiumTransactionBookedDate AS PremiumMasterBookedDate,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterPolicyExpirationYear1 AS PremiumMasterPolicyExpirationYear,
	mplt_Premium_ASL_Insurance_Hierarchy.O_PremiumTransactionEffectiveDate AS PremiumMasterCoverageEffectiveDate,
	mplt_Premium_ASL_Insurance_Hierarchy.O_PremiumTransactionExpirationDate AS PremiumMasterCoverageExpirationDate,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterPolicyTerm1 AS PremiumMasterPolicyTerm,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterBureauPolicyType1 AS PremiumMasterBureauPolicyType,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterAuditCode1 AS PremiumMasterAuditCode,
	mplt_Premium_ASL_Insurance_Hierarchy.O_PremiumType AS PremiumMasterPremiumType,
	mplt_Premium_ASL_Insurance_Hierarchy.o_TypeBureauCode AS PremiumMasterTypeBureauCode,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterBureauStatisticalLine1 AS PremiumMasterBureauStatisticalLine,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterProductLine1 AS PremiumMasterProductLine,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterAgencyCommissionRate1 AS PremiumMasterAgencyCommissionRate,
	mplt_Premium_ASL_Insurance_Hierarchy.ClassCode1 AS PremiumMasterClassCode,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterExposure1 AS PremiumMasterExposure,
	mplt_Premium_ASL_Insurance_Hierarchy.SubLine AS PremiumMasterSubLine,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterStatisticalCode11 AS PremiumMasterStatisticalCode1,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterStatisticalCode21 AS PremiumMasterStatisticalCode2,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterStatisticalCode31 AS PremiumMasterStatisticalCode3,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterRateModifier1 AS PremiumMasterRateModifier,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterRateDeparture1 AS PremiumMasterRateDeparture,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterCountersignAgencyType1 AS PremiumMasterCountersignAgencyType,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterCountersignAgencyCode1 AS PremiumMasterCountersignAgencyCode,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterCountersignAgencyState1 AS PremiumMasterCountersignAgencyState,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterCountersignAgencyRate1 AS PremiumMasterCountersignAgencyRate,
	mplt_Premium_ASL_Insurance_Hierarchy.o_PremiumAmount AS PremiumMasterPremium,
	mplt_Premium_ASL_Insurance_Hierarchy.o_FullTermPremiumAmount AS PremiumMasterFullTermPremium,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterRenewalIndicator1 AS PremiumMasterRenewalIndicator,
	mplt_Premium_ASL_Insurance_Hierarchy.o_asldimID AS asldimID,
	mplt_Premium_ASL_Insurance_Hierarchy.o_aslproductcodedimID AS aslproductcodedimID,
	mplt_Premium_ASL_Insurance_Hierarchy.o_AuditID AS AuditID,
	mplt_Premium_ASL_Insurance_Hierarchy.ProductCode1 AS ProductCode,
	mplt_Premium_ASL_Insurance_Hierarchy.PolicyOfferingCode1 AS PolicyOfferingCode,
	mplt_Premium_ASL_Insurance_Hierarchy.InsuranceReferenceLineOfBusinessCode1 AS InsuranceReferenceLineOfBusinessCode,
	mplt_Premium_ASL_Insurance_Hierarchy.EnterpriseGroupCode1 AS EnterpriseGroupCode,
	mplt_Premium_ASL_Insurance_Hierarchy.InsuranceReferenceLegalEntityCode1 AS InsuranceReferenceLegalEntityCode,
	mplt_Premium_ASL_Insurance_Hierarchy.StrategicProfitCenterCode1 AS StrategicProfitCenterCode,
	mplt_Premium_ASL_Insurance_Hierarchy.InsuranceSegmentCode1 AS InsuranceSegmentCode,
	mplt_Premium_ASL_Insurance_Hierarchy.SourceSystemID1 AS SourceSystemID,
	mplt_Premium_ASL_Insurance_Hierarchy.Risk_Unit_Group AS i_Risk_Unit_Group,
	mplt_Premium_ASL_Insurance_Hierarchy.StandardInsuranceLineCode1 AS i_StandardInsuranceLineCode,
	-- *INF*: :UDF.DEFAULT_VALUE_FOR_STRINGS(i_MajorPerilCode)
	:UDF.DEFAULT_VALUE_FOR_STRINGS(i_MajorPerilCode
	) AS v_MajorPerilCode,
	...
	IFF(v_StandardInsuranceLineCode IN ('CR') 
		OR v_flag = 1,
		'N/A',
		:UDF.DEFAULT_VALUE_FOR_STRINGS(i_Risk_Unit_Group
		)
	...
	mplt_Premium_ASL_Insurance_Hierarchy.RatingCoverage1 AS RatingCoverage,
	mplt_Premium_ASL_Insurance_Hierarchy.RiskType1 AS i_RiskType,
	mplt_Premium_ASL_Insurance_Hierarchy.CoverageForm1 AS i_CoverageForm,
	-- *INF*: i_RiskType
	-- 
	-- --IIF(LTRIM(RTRIM(i_CoverageForm))='BusinessAuto','N/A',i_RiskType)
	i_RiskType AS o_RiskType,
	mplt_Premium_ASL_Insurance_Hierarchy.CoverageType1 AS CoverageType,
	mplt_Premium_ASL_Insurance_Hierarchy.o_MajorPerilCode AS i_MajorPerilCode,
	...
	mplt_Premium_ASL_Insurance_Hierarchy.o_RiskUnit AS i_RiskUnit,
	mplt_Premium_ASL_Insurance_Hierarchy.o_RiskUnitSequenceNumber AS i_RiskUnitSequenceNumber,
	mplt_Premium_ASL_Insurance_Hierarchy.RiskLocationHashKey1 AS RiskLocationHashKey,
	-- *INF*: IIF(
	--   v_flag=1
	...
	rtrim(i_PremiumMasterTransactionCode
	) AS o_PremiumMasterTransactionCode,
	-- *INF*: rtrim(PremiumMasterPremiumType)
	rtrim(PremiumMasterPremiumType
	) AS o_PremiumMasterPremiumType,
	-- *INF*: TO_CHAR(i_PremiumMasterBureauInceptionDate,'YYYY')
	TO_CHAR(i_PremiumMasterBureauInceptionDate, 'YYYY'
	) AS o_PremiumMasterBureauInceptionDate,
	-- *INF*: :UDF.DEFAULT_VALUE_FOR_STRINGS(i_ReasonAmendedCode)
	:UDF.DEFAULT_VALUE_FOR_STRINGS(i_ReasonAmendedCode
	) AS o_ReasonAmendedCode,
	-- *INF*: IIF(i_StandardInsuranceLineCode='WC','Payroll',NULL)
	IFF(i_StandardInsuranceLineCode = 'WC',
		'Payroll',
		NULL
	) AS o_ExposureBasisName,
	...
	mplt_Premium_ASL_Insurance_Hierarchy.pm_risk_state1 AS pm_risk_state,
	pm_risk_state AS StateProvinceCode,
	mplt_Premium_ASL_Insurance_Hierarchy.PerilGroup1 AS PerilGroup,
	mplt_Premium_ASL_Insurance_Hierarchy.PolicyAuditAKID1 AS PolicyAuditAKId,
	mplt_Premium_ASL_Insurance_Hierarchy.PolicyAuditEffectiveDate1 AS PolicyAuditEffectiveDate,
	mplt_Premium_ASL_Insurance_Hierarchy.SubCoverageTypeCode1 AS SubCoverageTypeCode,
	mplt_Premium_ASL_Insurance_Hierarchy.CoverageVersion1 AS CoverageVersion,
	...
	mplt_Premium_ASL_Insurance_Hierarchy.CustomerCareCommissionRate1 AS CustomerCareCommissionRate,
	mplt_Premium_ASL_Insurance_Hierarchy.RatingPlanCode1 AS RatingPlanCode,
	mplt_Premium_ASL_Insurance_Hierarchy.PremiumMasterWrittenExposure1 AS PremiumMasterWrittenExposure,
	mplt_Premium_ASL_Insurance_Hierarchy.DeclaredEventFlag1 AS DeclaredEventFlag
...
EXP_Consolidate_Data_from_Lookups AS (
	...
	LKP_reinsurance_coverage_dim.reins_cov_dim_id AS i_ReinsuranceCoverageDimID,
	...
	EXP_PremiumMasterCalculation_IN.PremiumMasterPremiumType,
	EXP_PremiumMasterCalculation_IN.PremiumMasterAgencyCommissionRate,
	...
	EXP_PremiumMasterCalculation_IN.PremiumMasterExposure AS PremiumMasterExposure1,
	...
	IFF(i_AgencyDimID IS NOT NULL,
		i_AgencyDimID,
		- 1
	) AS o_AgencyDimID,
	-- *INF*: IIF(NOT ISNULL(i_PolicyDimID),i_PolicyDimID,-1)
	IFF(i_PolicyDimID IS NOT NULL,
		i_PolicyDimID,
		- 1
	) AS o_PolicyDimID,
	-- *INF*: IIF(NOT ISNULL(i_ContractCustomerDimID),i_ContractCustomerDimID,-1)
	IFF(i_ContractCustomerDimID IS NOT NULL,
		i_ContractCustomerDimID,
		- 1
	) AS o_ContractCustomerDimID,
	-- *INF*: IIF(NOT ISNULL(i_RiskLocationDimID),i_RiskLocationDimID,-1)
	IFF(i_RiskLocationDimID IS NOT NULL,
		i_RiskLocationDimID,
		- 1
	) AS o_RiskLocationDimID,
	-- *INF*: IIF(NOT ISNULL(i_ReinsuranceCoverageDimID),i_ReinsuranceCoverageDimID,-1)
	IFF(i_ReinsuranceCoverageDimID IS NOT NULL,
		i_ReinsuranceCoverageDimID,
		- 1
	) AS o_ReinsuranceCoverageDimID,
	-- *INF*: IIF(NOT ISNULL(i_PremiumTransactionTypeDimID),i_PremiumTransactionTypeDimID,-1)
	IFF(i_PremiumTransactionTypeDimID IS NOT NULL,
		i_PremiumTransactionTypeDimID,
		- 1
	) AS o_PremiumTransactionTypeDimID,
	...
	IFF(v_PremiumMasterCoverageEffectiveDateID IS NOT NULL,
		v_PremiumMasterCoverageEffectiveDateID,
		- 1
	...
	IFF(v_PremiumMasterCoverageExpirationDateID IS NOT NULL,
		v_PremiumMasterCoverageExpirationDateID,
		- 1
	) AS o_PremiumMasterCoverageExpirationDateID,
	-- *INF*: IIF(NOT ISNULL(i_ASLdimID),i_ASLdimID,-1)
	IFF(i_ASLdimID IS NOT NULL,
		i_ASLdimID,
		- 1
	) AS o_ASLdimID,
	-- *INF*: IIF(NOT ISNULL(i_ASLproductcodedimID),i_ASLproductcodedimID,-1)
	IFF(i_ASLproductcodedimID IS NOT NULL,
		i_ASLproductcodedimID,
		- 1
	) AS o_ASLproductcodedimID,
	...
	IFF(v_PremiumMasterPolicyEffectiveDateID IS NULL,
	...
	IFF(v_PremiumMasterPolicyExpirationDateID IS NULL,
		LKP_CALENDER_DIM_TO_DATE_21001231_YYYYMMDD.clndr_id,
	...
	EXP_PremiumMasterCalculation_IN.InsuranceSegmentCode,
	EXP_PremiumMasterCalculation_IN.StrategicProfitCenterCode,
	...
	EXP_PremiumMasterCalculation_IN.PremiumMasterWrittenExposure,
	EXP_PremiumMasterCalculation_IN.DeclaredEventFlag
	...
	ON LKP_PolicyAuditDim.EDWPolicyAuditAKId = EXP_PremiumMasterCalculation_IN.PolicyAuditAKId AND LKP_PolicyAuditDim.EffectiveDate = EXP_PremiumMasterCalculation_IN.PolicyAuditEffectiveDate
	...
	ON LKP_SalesDivisionDim.PolicyAkId = EXP_PremiumMasterCalculation_IN.PolicyAKID
	LEFT JOIN LKP_reinsurance_coverage_dim
	ON LKP_reinsurance_coverage_dim.edw_reins_cov_ak_id = EXP_PremiumMasterCalculation_IN.ReinsuranceCoverageAKID AND LKP_reinsurance_coverage_dim.eff_from_date <= EXP_PremiumMasterCalculation_IN.PremiumMasterBookedDate AND LKP_reinsurance_coverage_dim.eff_to_date >= EXP_PremiumMasterCalculation_IN.PremiumMasterBookedDate
	LEFT JOIN lkp_PremiumTransactionTypeDim
	ON lkp_PremiumTransactionTypeDim.PremiumTransactionCode = EXP_PremiumMasterCalculation_IN.o_PremiumMasterTransactionCode AND lkp_PremiumTransactionTypeDim.ReasonAmendedCode = EXP_PremiumMasterCalculation_IN.o_ReasonAmendedCode AND lkp_PremiumTransactionTypeDim.PremiumTypeCode = EXP_PremiumMasterCalculation_IN.o_PremiumMasterPremiumType AND lkp_PremiumTransactionTypeDim.CustomerCareCommissionRate = EXP_PremiumMasterCalculation_IN.CustomerCareCommissionRate
	LEFT JOIN LKP_CALENDER_DIM LKP_CALENDER_DIM_to_date_to_char_i_PremiumMasterBookedDate_MM_DD_YYYY_MM_DD_YYYY
	ON LKP_CALENDER_DIM_to_date_to_char_i_PremiumMasterBookedDate_MM_DD_YYYY_MM_DD_YYYY.clndr_date = to_date(to_char(i_PremiumMasterBookedDate, 'MM/DD/YYYY'
	), 'MM/DD/YYYY'
)

	LEFT JOIN LKP_CALENDER_DIM LKP_CALENDER_DIM_to_date_to_char_i_PremiumMasterCoverageEffectiveDate_MM_DD_YYYY_MM_DD_YYYY
	ON LKP_CALENDER_DIM_to_date_to_char_i_PremiumMasterCoverageEffectiveDate_MM_DD_YYYY_MM_DD_YYYY.clndr_date = to_date(to_char(i_PremiumMasterCoverageEffectiveDate, 'MM/DD/YYYY'
	), 'MM/DD/YYYY'
)

	LEFT JOIN LKP_CALENDER_DIM LKP_CALENDER_DIM_to_date_to_char_i_PremiumMasterCoverageExpirationDate_MM_DD_YYYY_MM_DD_YYYY
	ON LKP_CALENDER_DIM_to_date_to_char_i_PremiumMasterCoverageExpirationDate_MM_DD_YYYY_MM_DD_YYYY.clndr_date = to_date(to_char(i_PremiumMasterCoverageExpirationDate, 'MM/DD/YYYY'
	), 'MM/DD/YYYY'
)

	LEFT JOIN LKP_CALENDER_DIM LKP_CALENDER_DIM_to_date_to_char_i_PolicyEffectiveDate_MM_DD_YYYY_MM_DD_YYYY
	ON LKP_CALENDER_DIM_to_date_to_char_i_PolicyEffectiveDate_MM_DD_YYYY_MM_DD_YYYY.clndr_date = to_date(to_char(i_PolicyEffectiveDate, 'MM/DD/YYYY'
	), 'MM/DD/YYYY'
)

	LEFT JOIN LKP_CALENDER_DIM LKP_CALENDER_DIM_to_date_to_char_i_PolicyExpirationDate_MM_DD_YYYY_MM_DD_YYYY
	ON LKP_CALENDER_DIM_to_date_to_char_i_PolicyExpirationDate_MM_DD_YYYY_MM_DD_YYYY.clndr_date = to_date(to_char(i_PolicyExpirationDate, 'MM/DD/YYYY'
	), 'MM/DD/YYYY'
)
	...
	ON LKP_CALENDER_DIM_to_date_to_char_i_PremiumTransactionEnteredDate_MM_DD_YYYY_MM_DD_YYYY.clndr_date = to_date(to_char(i_PremiumTransactionEnteredDate, 'MM/DD/YYYY'
	), 'MM/DD/YYYY'
)
...
EXP_Evaluate_Fields AS (
	...
	o_ReinsuranceCoverageDimID AS ReinsuranceCoverageDimID,
	...
	-- *INF*: IIF(InsuranceSegmentCode = '3',0,i_PremiumMasterAgencyCommissionRate)
	-- 
	-- -- Added this as apart of  PROD-11572 as WCpool commission should be calculate in m_POL_DM_LOAD_PremiumMasterFact_WC_Commission
	IFF(InsuranceSegmentCode = '3',
		0,
		i_PremiumMasterAgencyCommissionRate
	) AS PremiumMasterAgencyCommissionRate,
	...
	-- *INF*: DECODE( TRUE,IN(RTRIM(i_PremiumMasterPremiumType),'C','D') AND IN(ReasonAmendedCode,'CWO' , 'COL', 'CWB'),0,rtrim(i_PremiumMasterPremiumType)='D' and ReasonAmendedCode != 'CWO' and ReasonAmendedCode != 'CWB',PremiumMasterPremium,0)
	-- --IIF(rtrim(i_PremiumMasterPremiumType)='D' and ReasonAmendedCode != 'CWO',PremiumMasterPremium,0)
	DECODE(TRUE,
		RTRIM(i_PremiumMasterPremiumType
		) IN ('C','D') 
		AND ReasonAmendedCode IN ('CWO','COL','CWB'), 0,
		rtrim(i_PremiumMasterPremiumType
		) = 'D' 
		AND ReasonAmendedCode != 'CWO' 
		AND ReasonAmendedCode != 'CWB', PremiumMasterPremium,
		0
	) AS v_PremiumMasterDirectWrittenPremium,
	-- *INF*: DECODE( TRUE,IN(RTRIM(i_PremiumMasterPremiumType),'C','D') AND IN(ReasonAmendedCode,'CWO' , 'COL', 'CWB'),0,rtrim(i_PremiumMasterPremiumType)='C' and ReasonAmendedCode != 'CWO' and ReasonAmendedCode !='CWB',PremiumMasterPremium,0)
	-- --IIF(rtrim(i_PremiumMasterPremiumType)='C' and ReasonAmendedCode != 'CWO',PremiumMasterPremium,0)
	DECODE(TRUE,
		RTRIM(i_PremiumMasterPremiumType
		) IN ('C','D') 
		AND ReasonAmendedCode IN ('CWO','COL','CWB'), 0,
		...
		AND ReasonAmendedCode != 'CWB', PremiumMasterPremium,
		0
	) AS v_PremiumMasterCededWrittenPremium,
	-- *INF*: IIF(ReasonAmendedCode!='CWB',
	-- IIF(rtrim(i_PremiumMasterPremiumType)='D' AND InsuranceSegmentCode <> '3', PremiumMasterPremium,0) * i_PremiumMasterAgencyCommissionRate, PremiumMasterPremium)
	-- 
	-- ---- Added  insurancesegment code of  PROD-11572 as WCpool commission should be calculate in m_POL_DM_LOAD_PremiumMasterFact_WC_Commission
	IFF(ReasonAmendedCode != 'CWB',
		IFF(rtrim(i_PremiumMasterPremiumType
			) = 'D' 
			AND InsuranceSegmentCode <> '3',
			PremiumMasterPremium,
			0
		) * i_PremiumMasterAgencyCommissionRate,
		PremiumMasterPremium
	) AS v_PremiumMasterAgencyDirectWrittenCommission,
	i_PremiumMasterBookedDate,
	i_PolicyEffectiveDate,
	StrategicProfitCenterCode,
	-- *INF*: to_date('07-01-2013 00:00:00','MM-DD-YYYY HH24:MI:SS')
	to_date('07-01-2013 00:00:00', 'MM-DD-YYYY HH24:MI:SS'
	) AS v_MineSubPLDate,
	-- *INF*: to_date('07-01-2014 00:00:00','MM-DD-YYYY HH24:MI:SS')
	-- 
	to_date('07-01-2014 00:00:00', 'MM-DD-YYYY HH24:MI:SS'
	) AS v_CLMineSubDate,
	-- *INF*: DECODE(TRUE,
	-- o_MajorPerilCode= '050' AND o_PremiumMasterPremiumType = 'C' AND StateProvinceCode = '12' AND StrategicProfitCenterCode = '1' AND i_PolicyEffectiveDate >= v_MineSubPLDate,v_PremiumMasterCededWrittenPremium * (.15),
	-- o_MajorPerilCode= '050' AND o_PremiumMasterPremiumType = 'C' AND StateProvinceCode = '12' AND StrategicProfitCenterCode = '1' AND i_PolicyEffectiveDate < v_MineSubPLDate,v_PremiumMasterCededWrittenPremium * (.20),
	-- o_MajorPerilCode= '050' AND o_PremiumMasterPremiumType = 'C' AND StateProvinceCode = '12' AND StrategicProfitCenterCode <> '1' AND i_PremiumMasterBookedDate >= v_CLMineSubDate,v_PremiumMasterCededWrittenPremium * (.15),
	-- o_MajorPerilCode= '050' AND o_PremiumMasterPremiumType = 'C' AND IN(StateProvinceCode,'13','16'), v_PremiumMasterCededWrittenPremium * (.30),
	-- v_PremiumMasterCededWrittenPremium * i_PremiumMasterAgencyCommissionRate
	-- )
	-- 
	-- --EDWP 1060 changes PL mine sub rates for policies effective on or after 07/01/2013
	DECODE(TRUE,
		o_MajorPerilCode = '050' 
		AND o_PremiumMasterPremiumType = 'C' 
		AND StateProvinceCode = '12' 
		AND StrategicProfitCenterCode = '1' 
		AND i_PolicyEffectiveDate >= v_MineSubPLDate, v_PremiumMasterCededWrittenPremium * ( .15 
		),
		o_MajorPerilCode = '050' 
		AND o_PremiumMasterPremiumType = 'C' 
		AND StateProvinceCode = '12' 
		AND StrategicProfitCenterCode = '1' 
		AND i_PolicyEffectiveDate < v_MineSubPLDate, v_PremiumMasterCededWrittenPremium * ( .20 
		),
		o_MajorPerilCode = '050' 
		AND o_PremiumMasterPremiumType = 'C' 
		AND StateProvinceCode = '12' 
		AND StrategicProfitCenterCode <> '1' 
		AND i_PremiumMasterBookedDate >= v_CLMineSubDate, v_PremiumMasterCededWrittenPremium * ( .15 
		),
		o_MajorPerilCode = '050' 
		AND o_PremiumMasterPremiumType = 'C' 
		AND StateProvinceCode IN ('13','16'), v_PremiumMasterCededWrittenPremium * ( .30 
		),
		v_PremiumMasterCededWrittenPremium * i_PremiumMasterAgencyCommissionRate
	) AS v_PremiumMasterAgencyCededWrittenCommission,
	-- *INF*: IIF(ReasonAmendedCode='CWB',0,ROUND(v_PremiumMasterAgencyCededWrittenCommission,2))
	IFF(ReasonAmendedCode = 'CWB',
		0,
		ROUND(v_PremiumMasterAgencyCededWrittenCommission, 2
		)
	) AS PremiumMasterAgencyCededWrittenCommission,
	...
	v_PremiumMasterDirectWrittenPremium AS o_PremiumMasterDirectWrittenPremium,
	v_PremiumMasterCededWrittenPremium AS o_PremiumMasterCededWrittenPremium,
	v_PremiumMasterDirectWrittenPremium - v_PremiumMasterCededWrittenPremium AS o_PremiumMasterNetWrittenPremium,
	-- *INF*: v_PremiumMasterAgencyDirectWrittenCommission
	-- --ROUND(v_PremiumMasterAgencyDirectWrittenCommission, 2)
	v_PremiumMasterAgencyDirectWrittenCommission AS o_PremiumMasterAgencyDirectWrittenCommission,
	-- *INF*: IIF(ReasonAmendedCode='CWB',0,PremiumMasterAgencyCededWrittenCommission)
	IFF(ReasonAmendedCode = 'CWB',
		0,
		PremiumMasterAgencyCededWrittenCommission
	) AS o_PremiumMasterAgencyCededWrittenCommission,
	-- *INF*: IIF(ReasonAmendedCode='CWB',0,v_PremiumMasterAgencyDirectWrittenCommission - PremiumMasterAgencyCededWrittenCommission)
	IFF(ReasonAmendedCode = 'CWB',
		0,
		v_PremiumMasterAgencyDirectWrittenCommission - PremiumMasterAgencyCededWrittenCommission
	) AS o_PremiumMasterAgencyNetWrittenCommission,
	-- *INF*: --Updated as part of PROD-18020. Removed PremiumtransactionCode values that are not related to Audit and removed ReasonAmendCode value in conditions
	-- 
	-- IIF(ReasonAmendedCode='CWB',0,IIF(IN(PremiumMasterTransactionCode,'14','24')  AND i_PremiumMasterPremiumType='D' AND IN(StrategicProfitCenterCode,'2','3','5') AND IN (InsuranceSegmentCode,'2','3'),PremiumMasterPremium,0))
	-- 
	-- 
	-- --PROD-8544: Audit Premium is only for direct transaction.
	IFF(ReasonAmendedCode = 'CWB',
		0,
		IFF(PremiumMasterTransactionCode IN ('14','24') 
			AND i_PremiumMasterPremiumType = 'D' 
			AND StrategicProfitCenterCode IN ('2','3','5') 
			AND InsuranceSegmentCode IN ('2','3'),
			PremiumMasterPremium,
			0
		)
	) AS o_PremiumMasterAuditPremium,
	-- *INF*: IIF(ReasonAmendedCode='CWB',0,IIF(IN(PremiumMasterTransactionCode,'20','21','22','23','24','25','28','29','67'),PremiumMasterPremium,0))
	IFF(ReasonAmendedCode = 'CWB',
		0,
		IFF(PremiumMasterTransactionCode IN ('20','21','22','23','24','25','28','29','67'),
			PremiumMasterPremium,
			0
		)
	) AS o_PremiumMasterReturnedPremium,
	-- *INF*: DECODE( TRUE,IN(RTRIM(i_PremiumMasterPremiumType),'C','D') AND IN(ReasonAmendedCode,'CWO' , 'COL'),PremiumMasterPremium,IN(PremiumMasterTransactionCode,'10','11','12','13','14','15','18','22','23','24','25','28') AND IN(ReasonAmendedCode,'COL', 'CWO'),PremiumMasterPremium,0)
	-- --IIF(IN(PremiumMasterTransactionCode,'10','11','12','13','14','15','18','22','23','24','25','28') AND IN(ReasonAmendedCode,'COL', 'CWO'),PremiumMasterPremium)
	DECODE(TRUE,
		RTRIM(i_PremiumMasterPremiumType
		) IN ('C','D') 
		AND ReasonAmendedCode IN ('CWO','COL'), PremiumMasterPremium,
		PremiumMasterTransactionCode IN ('10','11','12','13','14','15','18','22','23','24','25','28') 
		AND ReasonAmendedCode IN ('COL','CWO'), PremiumMasterPremium,
		0
	) AS o_PremiumMasterCollectionWriteOffPremium,
	o_PolicyAuditDimId,
	o_PremiumTransactionEnteredDateID,
	-- *INF*: DECODE(TRUE,
	-- i_StandardInsuranceLineCode<>'WC',0,
	-- IN(SUBSTR(i_ClassCode,1,4),'9887','9658'),v_PremiumMasterDirectWrittenPremium,
	-- 0
	-- )
	DECODE(TRUE,
		i_StandardInsuranceLineCode <> 'WC', 0,
		SUBSTR(i_ClassCode, 1, 4
		) IN ('9887','9658'), v_PremiumMasterDirectWrittenPremium,
		0
	) AS CreditDirectWrittenPremium,
	-- *INF*: DECODE(TRUE,
	-- i_StandardInsuranceLineCode<>'WC',0,
	-- IN(SUBSTR(i_ClassCode,1,4),'9659','9889'),v_PremiumMasterDirectWrittenPremium,
	-- 0
	-- )
	DECODE(TRUE,
		i_StandardInsuranceLineCode <> 'WC', 0,
		SUBSTR(i_ClassCode, 1, 4
		) IN ('9659','9889'), v_PremiumMasterDirectWrittenPremium,
		0
	) AS DebitDirectWrittenPremium,
	PremiumMasterWrittenExposure,
	DeclaredEventFlag,
...
LKP_PremiumMasterFact AS (
	...
	FROM (
		SELECT 
		PMF.PremiumMasterFactID as PremiumMasterFactID, 
		PMF.EDWPremiumMasterCalculationPKID as EDWPremiumMasterCalculationPKID, PMF.AnnualStatementLineDimID as AnnualStatementLineDimID, 
		PMF.StagePremiumMasterPKID as StagePremiumMasterPKID, 
		PMF.PremiumTransactionTypeDimID as PremiumTransactionTypeDimID,
		PMF.PremiumMasterRunDateID as PremiumMasterRunDateID,
		PMF.InsuranceReferenceDimId as InsuranceReferenceDimId 
		FROM @{pipeline().parameters.TARGET_TABLE_OWNER}.@{pipeline().parameters.TARGET_TABLE_NAME} PMF,
		              @{pipeline().parameters.TARGET_TABLE_OWNER}.CALENDAR_DIM CD 
		WHERE CD.clndr_id = PMF.PremiumMasterRunDateID
		          AND cd.clndr_date >= '@{pipeline().parameters.SELECTION_START_TS}'
	)
	QUALIFY ROW_NUMBER() OVER (PARTITION BY EDWPremiumMasterCalculationPKID,AnnualStatementLineDimID,StagePremiumMasterPKID,PremiumTransactionTypeDimID,PremiumMasterRunDateID,InsuranceReferenceDimId ORDER BY PremiumMasterFactID DESC) = 1
),
RTR_INSERT_UPDATE AS (
	SELECT
	LKP_PremiumMasterFact.PremiumMasterFactID,
	...
	EXP_Evaluate_Fields.PremiumMasterFullTermPremium,
	EXP_Evaluate_Fields.o_PremiumMasterDirectWrittenPremium AS PremiumMasterDirectWrittenPremium,
	EXP_Evaluate_Fields.o_PremiumMasterCededWrittenPremium AS PremiumMasterCededWrittenPremium,
	EXP_Evaluate_Fields.o_PremiumMasterNetWrittenPremium AS PremiumMasterNetWrittenPremium,
	EXP_Evaluate_Fields.PremiumMasterAgencyCommissionRate,
	EXP_Evaluate_Fields.o_PremiumMasterAgencyDirectWrittenCommission AS PremiumMasterAgencyDirectWrittenCommission,
	EXP_Evaluate_Fields.o_PremiumMasterAgencyCededWrittenCommission AS PremiumMasterAgencyCededWrittenCommission,
	EXP_Evaluate_Fields.o_PremiumMasterAgencyNetWrittenCommission AS PremiumMasterAgencyNetWrittenCommission,
	...
	EXP_Evaluate_Fields.o_PremiumMasterAuditPremium AS PremiumMasterAuditPremium,
	EXP_Evaluate_Fields.o_PremiumMasterReturnedPremium AS PremiumMasterReturnedPremium,
	EXP_Evaluate_Fields.o_PremiumMasterCollectionWriteOffPremium AS PremiumMasterCollectionWriteOffPremium,
	...
	EXP_Evaluate_Fields.CreditDirectWrittenPremium,
	EXP_Evaluate_Fields.DebitDirectWrittenPremium,
	EXP_Evaluate_Fields.PremiumMasterWrittenExposure,
	EXP_Evaluate_Fields.o_DeclaredEventFlag AS DeclaredEventFlag
	FROM EXP_Evaluate_Fields
	LEFT JOIN LKP_PremiumMasterFact
	ON LKP_PremiumMasterFact.EDWPremiumMasterCalculationPKID = EXP_Evaluate_Fields.EDWPremiumMasterCalculationPKID AND LKP_PremiumMasterFact.AnnualStatementLineDimID = EXP_Evaluate_Fields.AnnualStatementLineDimID AND LKP_PremiumMasterFact.StagePremiumMasterPKID = EXP_Evaluate_Fields.o_DEFAULT_ID AND LKP_PremiumMasterFact.PremiumTransactionTypeDimID = EXP_Evaluate_Fields.PremiumTransactionTypeDimID AND LKP_PremiumMasterFact.PremiumMasterRunDateID = EXP_Evaluate_Fields.PremiumMasterBookedDateID AND LKP_PremiumMasterFact.InsuranceReferenceDimId = EXP_Evaluate_Fields.InsuranceReferenceDimId
),
RTR_INSERT_UPDATE_INSERT AS (SELECT * FROM RTR_INSERT_UPDATE WHERE ISNULL(PremiumMasterFactID)),
RTR_INSERT_UPDATE_DEFAULT1 AS (SELECT * FROM RTR_INSERT_UPDATE WHERE NOT ( (ISNULL(PremiumMasterFactID)) )),
TGT_PremiumMasterFact_INSERT AS (
	...
	EDWPREMIUMMASTERCALCULATIONPKID, 
	EDWPREMIUMTRANSACTIONPKID, 
	DEFAULT_ID AS STAGEPREMIUMMASTERPKID, 
	...
	PremiumMasterBookedDateID AS PREMIUMMASTERRUNDATEID, 
	PREMIUMMASTERPREMIUM, 
	PREMIUMMASTERFULLTERMPREMIUM, 
	...
	COVERAGEDETAILDIMID, 
	PremiumMasterExposure AS EXPOSUREAMOUNT, 
	POLICYAUDITDIMID, 
	o_PremiumTransactionEnteredDateID AS PREMIUMTRANSACTIONENTEREDDATEID, 
	CREDITDIRECTWRITTENPREMIUM, 
	DEBITDIRECTWRITTENPREMIUM, 
	PremiumMasterWrittenExposure AS WRITTENEXPOSURE, 
	DECLAREDEVENTFLAG
	FROM RTR_INSERT_UPDATE_INSERT
),
UPD_Update AS (
	SELECT
	PremiumMasterFactID AS PremiumMasterFactID2, 
	AuditID AS AuditID2, 
	...
	ReinsuranceCoverageDimID AS ReinsuranceCoverageDimID2, 
	...
	PremiumMasterFullTermPremium AS PremiumMasterFullTermPremium2, 
	PremiumMasterDirectWrittenPremium AS PremiumMasterDirectWrittenPremium2, 
	PremiumMasterCededWrittenPremium AS PremiumMasterCededWrittenPremium2, 
	PremiumMasterNetWrittenPremium AS PremiumMasterNetWrittenPremium2, 
	PremiumMasterAgencyCommissionRate AS PremiumMasterAgencyCommissionRate2, 
	PremiumMasterAgencyDirectWrittenCommission AS PremiumMasterAgencyDirectWrittenCommission2, 
	PremiumMasterAgencyCededWrittenCommission AS PremiumMasterAgencyCededWrittenCommission2, 
	...
	CreditDirectWrittenPremium, 
	DebitDirectWrittenPremium, 
	PremiumMasterWrittenExposure AS PremiumMasterWrittenExposure2, 
	DeclaredEventFlag AS DeclaredEventFlag2
...
TGT_PremiumMasterFact_UPDATE AS (
	...
	ON T.PremiumMasterFactID = S.PremiumMasterFactID2
	WHEN MATCHED BY TARGET THEN
	UPDATE SET T.AnnualStatementLineDimID = S.AnnualStatementLineDimID2, T.AnnualStatementLineProductCodeDimID = S.AnnualStatementLineProductCodeDimID2, T.AgencyDimID = S.AgencyDimID2, T.PolicyDimID = S.PolicyDimID2, T.ContractCustomerDimID = S.ContractCustomerDimID2, T.RiskLocationDimID = S.RiskLocationDimID2, T.ReinsuranceCoverageDimID = S.ReinsuranceCoverageDimID2, T.PremiumTransactionTypeDimID = S.PremiumTransactionTypeDimID2, T.EDWPremiumMasterCalculationPKID = S.EDWPremiumMasterCalculationPKID2, T.EDWPremiumTransactionPKID = S.EDWPremiumTransactionPKID2, T.StagePremiumMasterPKID = S.DEFAULT_ID2, T.PremiumMasterPolicyEffectiveDateID = S.PremiumMasterPolicyEffectiveDateID2, T.PremiumMasterPolicyExpirationDateID = S.PremiumMasterPolicyExpirationDateID2, T.PremiumMasterCoverageEffectiveDateID = S.PremiumMasterCoverageEffectiveDateID2, T.PremiumMasterCoverageExpirationDateID = S.PremiumMasterCoverageExpirationDateID2, T.PremiumMasterRunDateID = S.PremiumMasterRunDateID2, T.PremiumMasterPremium = S.PremiumMasterPremium2, T.PremiumMasterFullTermPremium = S.PremiumMasterFullTermPremium2, T.PremiumMasterDirectWrittenPremium = S.PremiumMasterDirectWrittenPremium2, T.PremiumMasterCededWrittenPremium = S.PremiumMasterCededWrittenPremium2, T.PremiumMasterNetWrittenPremium = S.PremiumMasterNetWrittenPremium2, T.PremiumMasterAgencyCommissionRate = S.PremiumMasterAgencyCommissionRate2, T.PremiumMasterAgencyDirectWrittenCommission = S.PremiumMasterAgencyDirectWrittenCommission2, T.PremiumMasterAgencyCededWrittenCommission = S.PremiumMasterAgencyCededWrittenCommission2, T.PremiumMasterAgencyNetWrittenCommission = S.PremiumMasterAgencyNetWrittenCommission2, T.PremiumMasterAuditPremium = S.PremiumMasterAuditPremium, T.PremiumMasterReturnedPremium = S.PremiumMasterReturnedPremium, T.PremiumMasterCollectionWriteOffPremium = S.PremiumMasterCollectionWriteOffPremium, T.InsuranceReferenceDimId = S.InsuranceReferenceDimId, T.SalesDivisionDimId = S.SalesDivisionDimId, T.InsuranceReferenceCoverageDimId = S.InsuranceReferenceCoverageDimID2, T.CoverageDetailDimId = S.CoverageDetailDimId2, T.ExposureAmount = S.PremiumMasterExposure2, T.PolicyAuditDimId = S.PolicyAuditDimId2, T.CreditDirectWrittenPremium = S.CreditDirectWrittenPremium, T.DebitDirectWrittenPremium = S.DebitDirectWrittenPremium, T.WrittenExposure = S.PremiumMasterWrittenExposure2, T.DeclaredEventFlag = S.DeclaredEventFlag2
),