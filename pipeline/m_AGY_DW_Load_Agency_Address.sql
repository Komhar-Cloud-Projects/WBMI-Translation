...
EXP_AGY_address_convert AS (
	...
	IFF(
	    in_ADDRESS_TYPE IS NULL, 'N/A',
	    IFF(
	        LENGTH(in_ADDRESS_TYPE)>0
	    and TRIM(in_ADDRESS_TYPE)='', 'N/A',
	        rpad(in_ADDRESS_TYPE, 3)
	    )
	) AS ADDRESS_TYPE,
	-- *INF*: iif(isnull(IN_CITY),'Not Available',iif(IS_SPACES(IN_CITY),'Not Available',IN_CITY))
	IFF(
	    IN_CITY IS NULL, 'Not Available',
	    IFF(
	        LENGTH(IN_CITY)>0 AND TRIM(IN_CITY)='', 'Not Available', IN_CITY
	    )
	) AS CITY,
	-- *INF*: iif(isnull(IN_POSTAL_CODE),'N/A',iif(IS_SPACES(IN_POSTAL_CODE),'N/A',
	-- iif(length(IN_POSTAL_CODE)=0,'N/A',IN_POSTAL_CODE)))
	IFF(
	    IN_POSTAL_CODE IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_POSTAL_CODE)>0
	    and TRIM(IN_POSTAL_CODE)='', 'N/A',
	        IFF(
	            length(IN_POSTAL_CODE) = 0, 'N/A', IN_POSTAL_CODE
	        )
	    )
	) AS POSTAL_CODE,
	-- *INF*: iif(isnull(IN_ZIP_PLUS_4),'N/A',iif(IS_SPACES(IN_ZIP_PLUS_4),'N/A',IN_ZIP_PLUS_4))
	IFF(
	    IN_ZIP_PLUS_4 IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_ZIP_PLUS_4)>0 AND TRIM(IN_ZIP_PLUS_4)='', 'N/A', IN_ZIP_PLUS_4
	    )
	) AS ZIP_PLUS_4,
	-- *INF*: iif(isnull(IN_COUNTY),'Not Available',iif(IS_SPACES(IN_COUNTY),'Not Available',IN_COUNTY))
	IFF(
	    IN_COUNTY IS NULL, 'Not Available',
	    IFF(
	        LENGTH(IN_COUNTY)>0 AND TRIM(IN_COUNTY)='', 'Not Available', IN_COUNTY
	    )
	) AS COUNTY,
	-- *INF*: iif(isnull(IN_STATE_ABBREV),'N/A',iif(IS_SPACES(IN_STATE_ABBREV),'N/A',rpad(IN_STATE_ABBREV,3)))
	IFF(
	    IN_STATE_ABBREV IS NULL, 'N/A',
	    IFF(
	        LENGTH(IN_STATE_ABBREV)>0
	    and TRIM(IN_STATE_ABBREV)='', 'N/A',
	        rpad(IN_STATE_ABBREV, 3)
	    )
	) AS STATE_ABBREV,
	-- *INF*: iif(isnull(IN_COUNTRY),'Not Available',iif(IS_SPACES(IN_COUNTRY),'Not Available',IN_COUNTRY))
	IFF(
	    IN_COUNTRY IS NULL, 'Not Available',
	    IFF(
	        LENGTH(IN_COUNTRY)>0 AND TRIM(IN_COUNTRY)='', 'Not Available', IN_COUNTRY
	    )
	) AS COUNTRY,
	...
	IFF(
	    IN_COUNTY_LOCATION IS NULL, 'Not Available',
	    IFF(
	        LENGTH(IN_COUNTY_LOCATION)>0
	    and TRIM(IN_COUNTY_LOCATION)='', 'Not Available',
	        IFF(
	            length(IN_COUNTY_LOCATION) = 0, 'Not Available', IN_COUNTY_LOCATION
	        )
	    )
	) AS COUNTY_LOCATION,
	SOURCE_SYSTEM_ID,
	...
	-- ))
	IFF(
	    (ADDRESS_LINE_1 || ADDRESS_LINE_2 || ADDRESS_LINE_3) IS NULL, 'Not Available',
	    IFF(
	        LENGTH((ADDRESS_LINE_1 || ADDRESS_LINE_2 || ADDRESS_LINE_3))>0
	        and TRIM((ADDRESS_LINE_1 || ADDRESS_LINE_2 || ADDRESS_LINE_3))='',
	        'Not Available',
	        (ADDRESS_LINE_1 || ADDRESS_LINE_2 || ADDRESS_LINE_3)
	    )
	...
	IFF(
	    STATE_CODE || AGENCY_NUM IS NULL, 'N/A',
	    IFF(
	        LENGTH(STATE_CODE || AGENCY_NUM)>0
	    and TRIM(STATE_CODE || AGENCY_NUM)='', 'N/A',
	        STATE_CODE || AGENCY_NUM
	    )
	) AS OUT_AGENCY_KEY
	FROM Agency_address_Stage
...
EXP_Agy_Address_Upd_desc AS (
	...
	DECODE(
	    TRUE,
	    address_type = v_PREV_ROW_address_type and agency_key = v_PREV_ROW_agency_key, DATEADD(SECOND,- 1,v_PREV_ROW_eff_from_date),
	    orig_eff_to_date
	) AS v_eff_to_date,
