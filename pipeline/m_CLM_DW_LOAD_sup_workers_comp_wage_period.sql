...
EXP_Default_Values AS (
	...
	IFF(wage_basis_code IS NULL,
		'N/A',
		wage_basis_code
	) AS wage_basis_code_out,
	wage_basis_desc,
	-- *INF*: IIF(ISNULL(wage_basis_desc), 'N/A', wage_basis_desc)
	IFF(wage_basis_desc IS NULL,
		'N/A',
		wage_basis_desc
	) AS wage_basis_desc_out
...
EXP_detect_changes AS (
	...
	-- 
	IFF(lkp_sup_wc_wage_period_id IS NULL,
		'NEW',
		IFF(LTRIM(RTRIM(lkp_wage_period_descript
				)
			) != ( LTRIM(RTRIM(wage_basis_desc
					)
				) 
			),
			'UPDATE',
