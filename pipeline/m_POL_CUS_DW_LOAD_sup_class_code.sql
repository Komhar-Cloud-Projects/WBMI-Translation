...
EXP_values_wbimcls AS (
	...
	TO_TIMESTAMP('12/31/2100 23:59:59', 'MM/DD/YYYY HH24:MI:SS') AS v_default_end_date,
	...
	IFF(
	    IN_inland_marine_class_code IS NULL
	    or LENGTH(IN_inland_marine_class_code)>0
	    and TRIM(IN_inland_marine_class_code)=''
	    or LENGTH(IN_inland_marine_class_code) = 0,
	    'N/A',
	    ltrim(rtrim(IN_inland_marine_class_code))
	) AS class_code,
	...
	IFF(
	    IN_inland_marine_class_description IS NULL
	    or LENGTH(IN_inland_marine_class_description)>0
	    and TRIM(IN_inland_marine_class_description)=''
	    or LENGTH(IN_inland_marine_class_description) = 0,
	    'N/A',
...
EXP_Detect_Changes_wbimcls AS (
	...
	-- 	'NOCHANGE'))
	IFF(
	    LKP_sup_class_code_id IS NULL, 'NEW',
	    IFF(
	        (ltrim(rtrim(inland_marine_class_description)) <> ltrim(rtrim(LKP_class_code_descript))),
	        'UPDATE',
	        'NOCHANGE'
...
EXP_values_tl07rx AS (
	...
	IFF(
	    IN_class_code IS NULL
	    or LENGTH(IN_class_code)>0
	    and TRIM(IN_class_code)=''
	    or LENGTH(IN_class_code) = 0,
	    'N/A',
	    ltrim(rtrim(IN_class_code))
	) AS OUT_class_code,
	...
	IFF(
	    RTRIM(LTRIM(IN_class_description_indicator)) IS NULL, v_default_str,
	...
	IFF(
	    IN_class_description_seq IS NULL
	    or LENGTH(IN_class_description_seq)>0
	    and TRIM(IN_class_description_seq)=''
	    or LENGTH(IN_class_description_seq) = 0,
	    v_default_str,
	...
	IFF(
	    RTRIM(LTRIM(IN_class_description)) IS NULL, v_default_str,
...
EXP_Detect_Changes_tl07rx AS (
	...
	IFF(
	    LKP_sup_class_code_id IS NULL, 'NEW',
	    IFF(
	        (ltrim(rtrim(class_description)) <> ltrim(rtrim(LKP_class_code_descript))), 'UPDATE',
	        'NOCHANGE'
	    )
	) AS v_Changed_Flag,
	@{pipeline().parameters.WBMI_AUDIT_CONTROL_RUN_ID} AS Audit_ID,
...
EXP_values_tl63y AS (
	...
	--          
	DECODE(
	    RTRIM(LTRIM(IN_exp_date)),
	    NULL, v_default_end_date,
	    '99999999', v_default_end_date,
	    IFF(
	        IS_DATE(IN_exp_date, 'YYYYMMDD') = 1,
	        DATEADD(DAY,1,TO_TIMESTAMP(IN_exp_date, 'YYYYMMDD')),
	        DATEADD(DAY,1,LAST_DAY(TO_TIMESTAMP(CONCAT(SUBSTR(IN_exp_date, 1, 6), '01'), 'YYYYMMDD')))
	    )
	) AS v_exp_date,
	...
	IFF(
	    RTRIM(LTRIM(IN_crime_ind)) IS NULL
	    or LENGTH(IN_crime_ind)>0
	    and TRIM(IN_crime_ind)=''
	    or LENGTH(IN_crime_ind) = 0,
	    v_default_str,
