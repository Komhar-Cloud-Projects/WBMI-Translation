...
EXP_CLAIM_TRANSACTION_VALIDATE AS (
	...
	IFF(
	    LTRIM(RTRIM(in_loss_adjustor_no)) IS NULL
	...
	DECODE(
	    TRUE,
	    LENGTH(RTRIM(UDF_DEFAULT_VALUE_FOR_STRINGS(IN_LOSS_DRAFT_NO))) < 3, 'Check',
	    UPPER(SUBSTR(UDF_DEFAULT_VALUE_FOR_STRINGS(IN_LOSS_DRAFT_NO), 1, 3)) = 'EFT', 'EFT',
	    'Check'
	) AS v_payment_method,
...
EXP_CLAIMS_TRANSACTION_DEFAULT AS (
	...
	IFF(
	    LTRIM(RTRIM(v_MEMO_PHRASE_COMMENT)) IS NULL
	    or LENGTH(LTRIM(RTRIM(v_MEMO_PHRASE_COMMENT)))>0
	    and TRIM(LTRIM(RTRIM(v_MEMO_PHRASE_COMMENT)))=''
	    or LENGTH(LTRIM(RTRIM(v_MEMO_PHRASE_COMMENT))) = 0,
	    'N/A',
	...
	-- 	,prim_payee_name)
	IFF(
	    prim_payee_name IS NULL or substr(loss_adjustor_no, 1, 1) = 'X',
	    ltrim(rtrim(LOSS_CLAIM_PAYEE_NAME)),
	    prim_payee_name
