...
EXP_Insert_CededTransactions AS (
	...
	IFF(PremiumMasterPolicyTerm IS NULL,
		'0',
		PremiumMasterPolicyTerm
	...
	IFF(ltrim(rtrim(PremiumMasterBureauStatisticalLine
			)
		) IS NULL,
		'N/A',
		PremiumMasterBureauStatisticalLine
	...
	IFF(PremiumMasterAgencyCommissionRate IS NULL,
		0.00000,
		PremiumMasterAgencyCommissionRate
	) AS o_PremiumMasterAgencyCommissionRate,
	...
	IFF(SUBSTR(comments_cs, 1, 1
		) IS NULL,
		'   ',
		SUBSTR(comments_cs, 1, 1
		)
	) AS PremiumMasterCountersignAgencyType,
	-- *INF*: IIF(ISNULL(SUBSTR(comments_cs,2,7)),'   ',SUBSTR(comments_cs,2,7))
	IFF(SUBSTR(comments_cs, 2, 7
		) IS NULL,
		'   ',
		SUBSTR(comments_cs, 2, 7
		)
	) AS PremiumMasterCountersignAgencyCode,
	-- *INF*: IIF(ISNULL(SUBSTR(comments_cs,9,2)),'   ',SUBSTR(comments_cs,9,2))
	IFF(SUBSTR(comments_cs, 9, 2
		) IS NULL,
		'   ',
		SUBSTR(comments_cs, 9, 2
		)
	) AS PremiumMasterCountersignAgencyState,
	-- *INF*: IIF(ISNULL(SUBSTR(comments_cs,11,2)),'   ',REPLACECHR(0, SUBSTR(comments_cs,11,2), ',', '0'))
	IFF(SUBSTR(comments_cs, 11, 2
		) IS NULL,
		'   ',
		REGEXP_REPLACE(SUBSTR(comments_cs, 11, 2
		),',','0','i')
