...
EXP_CleanupData AS (
	...
	IFF(AgencyEmployeeRole IS NULL, 'N/A', AgencyEmployeeRole) AS o_AgencyEmployeeRole,
	...
	IFF(MiddleName IS NULL, '', MiddleName) AS o_MiddleName,
	SQ_AgencyEmployeeStage.Suffix,
	-- *INF*: IIF(IsNull(Suffix), '', Suffix)
	IFF(Suffix IS NULL, '', Suffix) AS o_Suffix,
	SQ_AgencyEmployeeStage.NickName,
	-- *INF*: IIF(IsNull(NickName), '', NickName)
	IFF(NickName IS NULL, '', NickName) AS o_NickName,
	...
	IFF(StatusCode IS NULL, 'A', StatusCode) AS o_StatusCode,
	SQ_AgencyEmployeeStage.StatusCodeDescription,
	-- *INF*: IIF(IsNull(StatusCodeDescription), 'Active', StatusCodeDescription)
	IFF(StatusCodeDescription IS NULL, 'Active', StatusCodeDescription) AS o_StatusCodeDescription,
	SQ_AgencyEmployeeStage.ListedDate,
	-- *INF*: IIF(IsNull(ListedDate), to_date('1800-01-01', 'YYYY-MM-DD'), ListedDate)
	IFF(ListedDate IS NULL, to_date('1800-01-01', 'YYYY-MM-DD'), ListedDate) AS o_ListedDate,
...
EXP_Detect_Changes AS (
	...
	MD5(AgencyEmployeeCode || AgencyEmployeeRole || ProducerCode || LastName || FirstName || MiddleName || Suffix || NickName || PrincipalFlag || PrimaryContactFlag || PhoneNumber || FaxNumber || EmailAddress || StatusCode || StatusCodeDescription || to_char(ListedDate) || to_char(TerminatedDate) || UserID) AS v_NewHashKey,
	v_NewHashKey AS o_HashKey,
	-- *INF*: IIF(ISNULL(lkp_AgencyEmployeeAKID), 'NEW', 
	-- IIF((lkp_HashKey <> v_NewHashKey),'UPDATE', 'NOCHANGE'))
	IFF(lkp_AgencyEmployeeAKID IS NULL, 'NEW', IFF(( lkp_HashKey <> v_NewHashKey ), 'UPDATE', 'NOCHANGE')) AS v_changed_flag,
...
EXP_Assign_AKID AS (
	...
	IFF(lkp_AgencyEmployeeAKID IS NULL, NEXTVAL, lkp_AgencyEmployeeAKID) AS o_AgencyEmployeeAKID,
	AgencyAKID,
	AgencyEmployeeCode,
	AgencyEmployeeRole,
	ProducerCode,
