...
EXP_values AS (
	...
	IFF(
	    in_sic_code_number IS NULL
	    or LENGTH(in_sic_code_number)>0
	    and TRIM(in_sic_code_number)=''
	    or LENGTH(in_sic_code_number) = 0,
	    'N/A',
	    ltrim(rtrim(in_sic_code_number))
	) AS sic_code_number,
	...
	IFF(
	    in_sic_code_description IS NULL
	    or LENGTH(in_sic_code_description)>0
	    and TRIM(in_sic_code_description)=''
	    or LENGTH(in_sic_code_description) = 0,
	    'N/A',
...
EXP_Detect_Changes AS (
	...
	-- 	'NOCHANGE'))
	IFF(
	    lkp_sup_sic_code_id IS NULL, 'NEW',
	    IFF(
	        (ltrim(rtrim(sic_code_description)) <> UPPER(ltrim(rtrim(lkp_sic_code_description)))),
	        'UPDATE',
	        'NOCHANGE'
...
EXP_GetValues_DCT AS (
	...
	IFF(
	    REGEXP_INSTR(v_SICCodeDesc, v_SICCode2) = 0, v_SICCodeDesc,
