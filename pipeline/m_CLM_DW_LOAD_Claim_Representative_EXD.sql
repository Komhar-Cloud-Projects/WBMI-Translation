...
EXP_Source AS (
	...
	-- 
	IFF(
	    LENGTH(CAJ_EMP_CLIENT_ID)>0 AND TRIM(CAJ_EMP_CLIENT_ID)='' OR CAJ_EMP_CLIENT_ID IS NULL,
	    'N/A',
	    LTRIM(RTRIM(CAJ_EMP_CLIENT_ID))
	...
	-- )
	IFF(
	    LENGTH(CAJ_USER_ID)>0 AND TRIM(CAJ_USER_ID)='' OR CAJ_USER_ID IS NULL, 'N/A',
	    LTRIM(RTRIM(CAJ_USER_ID))
	...
	-- )
	IFF(
	    LENGTH(CAJ_OFFICE_ID)>0
	    and TRIM(CAJ_OFFICE_ID)=''
...
EXP_DeterminePmsId AS (
	...
	IFF(
	    SOURCE_CAJ_EMP_CLIENT_ID = 'N/A', SOURCE_CAJ_EMP_CLIENT_ID,
	...
	IFF(var_cost_center IS NULL, 'N/A', UDF_DEFAULT_VALUE_FOR_STRINGS(var_cost_center)) AS cost_center_out,
	...
	IFF(
	    var_branch_number IS NULL, 'N/A',
	    LPAD(UDF_DEFAULT_VALUE_FOR_STRINGS(var_branch_number), 3, '0')
...
EXP_Target AS (
	...
	--       -- ,LTRIM(RTRIM(v_Manager_first_name || ' ' || v_Manager_mid_name || ' ' || v_Manager_last_name)))
	IFF(
	    LENGTH(v_Manager_first_name || v_Manager_mid_name || v_Manager_last_name)>0
	    and TRIM(v_Manager_first_name || v_Manager_mid_name || v_Manager_last_name)='' = 1,
	    'N/A',
	    IFF(
	        Manager_mid_name = 'N/A',
	        LTRIM(RTRIM(v_Manager_first_name || v_Manager_mid_name || v_Manager_last_name)),
	        LTRIM(RTRIM(v_Manager_first_name || ' ' || v_Manager_mid_name || ' ' || v_Manager_last_name))
	    )
	) AS CICL_LNG_NM,
	...
	IFF(
	    LKP_SUP_CLAIM_STAFF_EMAIL_wbconnect_user_id_claim_rep.EMAIL IS NULL, 'N/A',
	...
	IFF(
	    LKP_SUP_CLAIM_STAFF_EMAIL_wbconnect_user_id_handling_officer_mgr.EMAIL IS NULL, 'N/A',
	...
	UDF_DEFAULT_VALUE_FOR_STRINGS(v_GroupType) AS GroupName_out
...
EXP_Detect_Changes AS (
	...
	-- ,'NOCHANGE'))
	IFF(
	    claim_rep_id IS NULL, 'NEW',
	    IFF(
	        ltrim(rtrim(lkp_claim_rep_first_name)) != ltrim(rtrim(CICL_FST_NM))
	        or ltrim(rtrim(lkp_claim_rep_last_name)) != ltrim(rtrim(CICL_LST_NM))
	        or ltrim(rtrim(lkp_claim_rep_mid_name)) != ltrim(rtrim(CICL_MDL_NM))
	        or ltrim(rtrim(lkp_claim_rep_name_prfx)) != ltrim(rtrim(NM_PFX))
	        or ltrim(rtrim(lkp_claim_rep_name_sfx)) != ltrim(rtrim(NM_SFX))
