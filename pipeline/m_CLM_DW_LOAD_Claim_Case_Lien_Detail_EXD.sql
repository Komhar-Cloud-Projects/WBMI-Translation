...
EXP_VALIDATE AS (
	...
	--                                                                                 
	IFF(
	    LTRIM(RTRIM(tch_claim_nbr_cld)) IS NULL
	    or LENGTH(LTRIM(RTRIM(tch_claim_nbr_cld)))>0
	    and TRIM(LTRIM(RTRIM(tch_claim_nbr_cld)))=''
	    or LENGTH(LTRIM(RTRIM(tch_claim_nbr_cld))) = 0,
	    'N/A',
	...
	--                                                                            
	IFF(
	    LTRIM(RTRIM(tch_client_id_cld)) IS NULL
	    or LENGTH(LTRIM(RTRIM(tch_client_id_cld)))>0
	    and TRIM(LTRIM(RTRIM(tch_client_id_cld)))=''
	    or LENGTH(LTRIM(RTRIM(tch_client_id_cld))) = 0,
	    'N/A',
	...
	IFF(
	    LTRIM(RTRIM(IN_lien_role)) IS NULL
	    or LENGTH(LTRIM(RTRIM(IN_lien_role)))>0
	    and TRIM(LTRIM(RTRIM(IN_lien_role)))=''
	    or LENGTH(LTRIM(RTRIM(IN_lien_role))) = 0,
	    'N/A',
...
EXP_DETECT_CHANGES AS (
	...
	-- 
	IFF(
	    old_claim_case_lien_det_id IS NULL, 'NEW',
	    IFF(
	        lien_amt <> old_lien_amt, 'UPDATE', 'NOCHANGE'
	    )
	) AS v_changed_flag,
...
EXP_Lag_eff_from_date AS (
	...
	DECODE(
	    TRUE,
	    claim_case_ak_id = v_prev_row_claim_case_ak_id and lien_client = v_prev_row_lien_client and lien_client_role_code = v_prev_row_lien_client_role_code, DATEADD(SECOND,- 1,v_prev_row_eff_from_date),
	    orig_eff_to_date
	) AS v_eff_to_date,
