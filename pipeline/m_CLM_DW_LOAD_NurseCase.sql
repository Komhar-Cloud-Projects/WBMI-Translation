...
EXP_Src_Values AS (
	...
	-- 
	-- 'N/A'))
	IFF(length(tch_claim_nbr) > 0, ltrim(rtrim(tch_claim_nbr)), IFF(length(pms_policy_sym) > 0, ltrim(rtrim(pms_policy_sym || pms_policy_num || pms_policy_mod || v_pms_date_of_loss || pms_loss_occurence)), 'N/A')) AS o_claim_nbr,
	...
	-- 
	-- 'N/A'))
	IFF(length(tch_claim_nbr) > 0, ltrim(rtrim(tch_client_id)), IFF(length(pms_policy_sym) > 0, ltrim(rtrim(pms_policy_sym || pms_policy_num || pms_policy_mod || v_pms_date_of_loss || pms_loss_occurence || pms_loss_claimant || 'CMT')), 'N/A')) AS o_client_id,
	source_system_id,
	estimated_savings_amount,
...
EXP_TargetLkp_Detect_Changes AS (
	...
	--         'UPDATE', 'NOCHANGE')
	--    )
	IFF(Lkp_NurseCaseId IS NULL, 'NEW', IFF(LTRIM(RTRIM(Lkp_claim_party_occurrence_ak_id)) != LTRIM(RTRIM(claim_party_occurrence_ak_id)) OR LTRIM(RTRIM(Lkp_clmt_nurse_manage_id)) != LTRIM(RTRIM(clmt_nurse_manage_id)) OR LTRIM(RTRIM(Lkp_EstimatedSavingsAmount)) != LTRIM(RTRIM(EstimatedSavingsAmount)), 'UPDATE', 'NOCHANGE')) AS v_ChangedFlag,
