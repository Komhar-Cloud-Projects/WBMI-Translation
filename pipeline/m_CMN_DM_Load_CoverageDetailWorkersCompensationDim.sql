...
EXP_AddMetaData AS (
	...
	IFF(
	    i_EffectiveDate IS NULL, TO_TIMESTAMP('1800-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS'),
	    i_EffectiveDate
	) AS v_EffectiveDate,
	-- *INF*: IIF(ISNULL(i_ExpirationDate), TO_DATE('2100-12-31 23:59:59','YYYY-MM-DD HH24:MI:SS'), i_ExpirationDate)
	IFF(
	    i_ExpirationDate IS NULL, TO_TIMESTAMP('2100-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS'),
	    i_ExpirationDate
	) AS v_ExpirationDate,
	...
	IFF(
	    i_ExperienceModificationEffectiveDate IS NULL,
	...
	IFF(
	    LKP_SUPCLASSIFICATIONWORKERSCOMPENSATION_i_StateCode_v_ClassCode.RatableClassIndicator IS NULL,
	...
	DECODE(
	    ...
	    lkp_EffectiveDate <> v_EffectiveDate OR lkp_ExpirationDate <> v_ExpirationDate OR lkp_CoverageGuid <> v_CoverageGuid OR LTRIM(RTRIM(lkp_NcciClassCode)) <> v_NcciClassCode OR lkp_NcciClassDescription <> v_NcciClassDescription OR lkp_PolicyPerAccidentLimit <> v_PolicyPerAccidentLimit OR lkp_PolicyPerDiseaseLimit <> v_PolicyPerDiseaseLimit OR lkp_PolicyAggregateLimit <> v_PolicyAggregateLimit OR lkp_ExperienceModificationFactor <> v_ExperienceModificationFactor OR lkp_ExperienceModificationEffectiveDate <> v_ExperienceModificationEffectiveDate OR DECODE(
	        lkp_ConsentToRateFlag,
	        'T', '1',
	        'F', '0',
	        '0'
	    ) <> v_ConsentToRateFlag OR lkp_RateOverride <> v_RateOverride OR lkp_RatableClassIndicator <> v_RatableClassIndicator OR lkp_SurchargeClassIndicator <> v_SurchargeClassIndicator OR lkp_HazardGroupCode <> v_HazardGroupCode OR DECODE(
	        lkp_AdmiraltyActFlag,
	        'T', '1',
	        'F', '0',
	        '0'
	    ) <> v_AdmiraltyActFlag OR DECODE(
	        lkp_FederalEmployersLiabilityActFlag,
	        'T', '1',
	        'F', '0',
	        '0'
	    ) <> v_FederalEmployersLiabilityActFlag OR DECODE(
	        lkp_USLongShoreAndHarborWorkersCompensationActFlag,
	        'T', '1',
	        'F', '0',
	        '0'
