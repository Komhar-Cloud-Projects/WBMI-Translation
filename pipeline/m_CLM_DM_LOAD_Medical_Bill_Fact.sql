...
EXP_Calculations AS (
	...
	ROUND(total_bill_review_red + total_network_red, 2) AS v_gross_bill_saving,
	...
	-- )
	DECODE(TRUE,
		total_bill_charge <> 0, ROUND(( v_gross_bill_saving / total_bill_charge ) * 100, 2),
		0) AS v_gross_bill_saving_perc,
	v_gross_bill_saving_perc AS gross_bill_saving_perc,
	-- *INF*: DECODE(TRUE,total_bill_charge <> 0,
	-- ROUND((total_bill_review_red / total_bill_charge) * 100,2),
	-- 0)
	DECODE(TRUE,
		total_bill_charge <> 0, ROUND(( total_bill_review_red / total_bill_charge ) * 100, 2),
		0) AS v_bill_review_saving_perc,
	v_bill_review_saving_perc AS bill_review_saving_perc,
	-- *INF*: DECODE(TRUE,total_bill_charge <> 0,
	-- ROUND((total_network_red / total_bill_charge) * 100,2),
	-- 0)
	DECODE(TRUE,
		total_bill_charge <> 0, ROUND(( total_network_red / total_bill_charge ) * 100, 2),
		0) AS v_total_network_red_perc,
	v_total_network_red_perc AS total_network_red_perc,
	-- *INF*: ROUND(v_gross_bill_saving - bill_review_cost,2)
	ROUND(v_gross_bill_saving - bill_review_cost, 2) AS v_net_bill_review_saving,
	v_net_bill_review_saving AS net_bill_review_saving,
	-- *INF*: DECODE(TRUE,total_bill_charge <> 0,
	-- ROUND((v_net_bill_review_saving / total_bill_charge) * 100,2),
	-- 0)
	DECODE(TRUE,
		total_bill_charge <> 0, ROUND(( v_net_bill_review_saving / total_bill_charge ) * 100, 2),
		0) AS v_net_billing_saving_percentage,
	...
	IFF(NOT med_bill_fact_id_exists IS NULL, IFF(LKP_claim_occurrence_dim_id <> in_claim_occurrence_dim_id OR LKP_claim_loss_date_id <> claim_loss_date_id OR LKP_claim_discovery_date_id <> claim_discovery_date_id OR LKP_claim_scripted_date_id <> claim_scripted_date_id OR LKP_source_claim_rpted_date_id <> Source_Claim_Rpted_Date_id OR LKP_claim_rpted_date_id <> claim_rpted_date_id OR LKP_claim_open_date_id <> claim_open_date_id OR LKP_claim_close_date_id <> claim_close_date_id OR LKP_claim_reopen_date_id <> claim_reopen_date_id OR LKP_claim_closed_after_reopen_date_id <> claim_closed_after_reopen_date_id OR LKP_claim_notice_only_date_id <> Claim_Notice_only_Date_ID OR LKP_claim_cat_start_date_id <> claim_cat_start_date_id OR LKP_claim_cat_end_date_id <> claim_cat_end_date_id OR LKP_claim_rep_assigned_date_id <> claim_rep_assigned_date_id OR LKP_claim_rep_unassigned_date_id <> claim_rep_unassigned_date_id OR LKP_gross_bill_saving <> v_gross_bill_saving OR LKP_gross_bill_saving_percentage <> TRUNC(v_gross_bill_saving_perc, 2) OR LKP_total_bill_review_saving <> total_bill_review_red OR LKP_bill_review_saving_percentage <> TRUNC(v_bill_review_saving_perc, 2) OR LKP_total_network_red <> total_network_red OR LKP_total_network_red_percentage <> TRUNC(v_total_network_red_perc, 2) OR LKP_total_bill_charge <> total_bill_charge OR LKP_total_recommend_pay <> total_recommend_pay OR LKP_gross_bill_review_cost <> bill_review_cost OR LKP_net_bill_review_saving <> v_net_bill_review_saving OR LKP_net_bill_saving_percentage <> LKP_net_bill_saving_percentage OR LKP_agency_dim_id <> in_agency_dim_id OR LKP_contract_cust_dim_id <> in_contract_cust_dim_id OR LKP_pol_dim_id <> in_pol_dim_id OR LKP_pol_eff_date_id <> v_pol_eff_date_id OR LKP_pol_exp_date_id <> v_pol_exp_date_id OR LKP_claimant_dim_id <> in_claimant_dim_id OR LKP_claim_rep_dim_prim_claim_rep_id <> in_claim_rep_dim_prim_claim_rep_id OR LKP_strtgc_bus_dvsn_dim_id <> strtgc_bus_dvsn_dim_id, 'UPDATE', 'NOCHANGE'), 'INSERT') AS V_Changed_Flag,
