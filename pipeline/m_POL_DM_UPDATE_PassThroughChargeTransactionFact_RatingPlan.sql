...
SQ_PassThroughChargeTransactionFact AS (
	...
	inner Join @{pipeline().parameters.SOURCE_TABLE_OWNER}.PassThroughChargeTransactionFact PTCTF
	...
	where P.edw_pol_ak_id in (SELECT
		P.edw_pol_ak_id
	FROM policy_dim P
	INNER JOIN PassThroughChargeTransactionFact PTCTF
		ON P.pol_dim_id = PTCTF.PolicyDimId
	INNER JOIN InsuranceReferenceDim IRD
		ON PTCTF.InsuranceReferenceDimId = IRD.InsuranceReferenceDimId
	GROUP BY P.edw_pol_ak_id
	HAVING COUNT(DISTINCT IRD.RatingPlanCode) > 1)
	@{pipeline().parameters.WHERE_CLAUSE_PTCTF}
	order by 1
),
EXP_PassThroughChargeTransactionFact AS (
	SELECT
	EDW_Pol_AK_id,
	...
	ProductCode,
	InsuranceReferenceLineOfBusinessCode
	FROM SQ_PassThroughChargeTransactionFact
),
LKP_RatingPlan_DCT AS (
	...
	FROM (
		...
		INNER JOIN @{pipeline().parameters.WC_DATA_MART_DATABASE_NAME}.DBO.PassThroughChargeTransactionFact PTCTF
			ON P.pol_dim_id = PTCTF.PolicyDimId
...
EXP_RatingPlanDerivation AS (
	...
	ON LKP_RatingPlan_DCT.PolicyAKID = i_PolicyAKID
	LEFT JOIN LKP_RatingPlan_PMS
	ON LKP_RatingPlan_PMS.PolicyAKID = EXP_PassThroughChargeTransactionFact.EDW_Pol_AK_id
),
EXP_PassThrChgTransFact_JNR AS (
	SELECT
	EXP_PassThroughChargeTransactionFact.PassThroughChargeTransactionFactID,
	EXP_PassThroughChargeTransactionFact.EnterpriseGroupCode,
	EXP_PassThroughChargeTransactionFact.InsuranceReferenceLegalEntityCode,
	EXP_PassThroughChargeTransactionFact.StrategicProfitCenterCode,
	EXP_PassThroughChargeTransactionFact.InsuranceSegmentCode,
	EXP_PassThroughChargeTransactionFact.PolicyOfferingCode,
	EXP_PassThroughChargeTransactionFact.ProductCode,
	EXP_PassThroughChargeTransactionFact.InsuranceReferenceLineOfBusinessCode,
	...
	FROM EXP_PassThroughChargeTransactionFact
...
FILT_PTCTF AS (
	...
	FROM EXP_PassThrChgTransFact_JNR
...
PassThroughChargeTransactionFact AS (
	MERGE INTO Shortcut_to_PassThroughChargeTransactionFact AS T
	USING UPD_PTCTF AS S
	ON T.PassThroughChargeTransactionFactID = S.PassThroughChargeTransactionFactID
