...
SQ_Claim_Representative_Occurrence_PMS_fix AS (
	...
	WHERE
	 claim_representative_occurrence.claim_rep_ak_id=-1
...
LKP_pif_4578_stage AS (
	...
	FROM (
		...
		FROM pif_4578_stage
		WHERE pif_4578_stage.loss_examiner LIKE ('X%') OR pif_4578_stage.loss_examiner IN ('QUE','QUR')
...
EXP_Claim_Representative_Occurrence_PMS_fix AS (
	...
	IFF(LKP_CLAIM_REP_V_LOSS_EXAMINER_NO.claim_rep_ak_id IS NULL, - 1, LKP_CLAIM_REP_V_LOSS_EXAMINER_NO.claim_rep_ak_id) AS CLAIM_REP_AK_ID_OP
	...
	LEFT JOIN LKP_CLAIM_REP LKP_CLAIM_REP_V_LOSS_EXAMINER_NO
	ON LKP_CLAIM_REP_V_LOSS_EXAMINER_NO.claim_rep_key = V_LOSS_EXAMINER_NO
